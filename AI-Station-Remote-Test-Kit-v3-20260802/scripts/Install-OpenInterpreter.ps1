...
$requirementsLock = Join-Path $packageRoot "requirements-lock.txt"
$expectedVersion = "0.4.3"
$expectedLiteLLMVersion = "1.41.26"
...
try {
    ...
    if (-not (Test-Path -LiteralPath $requirementsLock)) {
        throw "requirements-lock.txt is missing. Extract the complete ZIP first."
    }
    ...
    [Environment]::SetEnvironmentVariable('PIP_ONLY_BINARY', 'litellm', 'Process')
    ...
    & $venvPython -m pip install --disable-pip-version-check --require-virtualenv --prefer-binary --only-binary=litellm -r $requirementsLock
    ...
    $installedLiteLLMVersion = & $venvPython -c "import importlib.metadata; print(importlib.metadata.version('litellm'))"
    if ($LASTEXITCODE -ne 0 -or $installedLiteLLMVersion -ne $expectedLiteLLMVersion) {
        throw "Installed LiteLLM version '$installedLiteLLMVersion' does not match '$expectedLiteLLMVersion'."
    }
    ...
    if ($LASTEXITCODE -ne 0) { throw "The installed dependency set failed pip check." }
    ...
    @(
        ...
        "LiteLLM: $installedLiteLLMVersion (wheel only)",
