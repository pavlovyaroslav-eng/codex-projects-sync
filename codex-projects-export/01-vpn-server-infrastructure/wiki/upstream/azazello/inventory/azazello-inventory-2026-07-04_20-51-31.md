...
Generated: 2026-07-04T20:51:31+00:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 20:51:32 up 28 days,  9:31,  1 user,  load average: 5.00, 2.19, 1.32
```
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 23736 bytes 5860194 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 23736 bytes 1681931 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		iifname "lo" counter packets 6603849 bytes 3954922773 accept
		ct state related,established counter packets 156083638 bytes 131320165594 accept
		ct state invalid counter packets 5699 bytes 374260 jump ufw-logging-deny
		ct state invalid counter packets 5699 bytes 374260 drop
		...
		ip protocol icmp icmp type echo-request counter packets 11928 bytes 691342 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 1238389 bytes 277350005 jump ufw-not-local
		...
		counter packets 1238389 bytes 277350005 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 6603853 bytes 3954922989 accept
		ct state related,established counter packets 164884866 bytes 191668702794 accept
		counter packets 1333815 bytes 184544514 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		...
		udp dport 67 counter packets 457480 bytes 150054086 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 76446 bytes 23266971 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 23736 bytes 1524755 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 23363 bytes 1952643 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 170290 bytes 11622916 accept
		ip protocol udp ct state new counter packets 1153520 bytes 171941457 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 44983606 bytes 43946494012 jump f2b-ufw-portscan
		iifname "tun79" counter packets 362715 bytes 451526685 accept
		counter packets 538018978 bytes 501992485577 jump ufw-before-logging-input
		counter packets 538018978 bytes 501992485577 jump ufw-before-input
		counter packets 4804415 bytes 925696477 jump ufw-after-input
		counter packets 2348600 bytes 129979252 jump ufw-after-logging-input
		counter packets 2348600 bytes 129979252 jump ufw-reject-input
		counter packets 2348600 bytes 129979252 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 10441 bytes 1991672 accept
		counter packets 490979284 bytes 474316097081 jump ufw-before-logging-output
		counter packets 490979284 bytes 474316097081 jump ufw-before-output
		counter packets 4837514 bytes 688071592 jump ufw-after-output
		counter packets 4837514 bytes 688071592 jump ufw-after-logging-output
		counter packets 4837514 bytes 688071592 jump ufw-reject-output
		counter packets 4837514 bytes 688071592 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 51561362 bytes 127040946727 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 61949429 bytes 11453750452 accept
		...
		counter packets 258365179 bytes 297970631302 jump DOCKER-USER
		counter packets 258365179 bytes 297970631302 jump DOCKER-FORWARD
	...
	chain DOCKER {
		ip daddr 172.17.0.3 iifname != "docker0" oifname "docker0" tcp dport 443 counter packets 2958 bytes 183380 accept
	...
	chain DOCKER-FORWARD {
		counter packets 258365179 bytes 297970631302 jump DOCKER-CT
		counter packets 129475664 bytes 151295167603 jump DOCKER-INTERNAL
		counter packets 129475664 bytes 151295167603 jump DOCKER-BRIDGE
		iifname "br-566ebeb0f4f1" counter packets 0 bytes 0 accept
		iifname "amn0" counter packets 128018408 bytes 150944712538 accept
		iifname "docker0" counter packets 1430445 bytes 346620807 accept
	}
	...
	chain DOCKER-BRIDGE {
		...
		oifname "docker0" counter packets 2958 bytes 183380 jump DOCKER
	}

	chain DOCKER-CT {
		oifname "br-566ebeb0f4f1" ct state related,established counter packets 0 bytes 0 accept
		oifname "amn0" ct state related,established counter packets 127785474 bytes 146302670097 accept
		oifname "docker0" ct state related,established counter packets 1104041 bytes 372793602 accept
	}
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 4926 bytes 328338 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 4926 bytes 328338 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 534547 bytes 173357905 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 704376 bytes 104020596 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 534013 bytes 173329409 return
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 41 | 2026-07-04 20:51:28 |
...
| `/etc/x-ui` | directory | root:root | 755 | 4096 | 2026-07-04 20:51:37 |
| `/etc/3x-ui` | missing | - | - | - | - |
