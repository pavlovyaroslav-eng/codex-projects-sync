...
Generated: 2026-09-23T04:30:08+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:09 up 9 days,  1:17,  0 user,  load average: 0.39, 0.24, 0.25
```
...
udp   UNCONN 0      0                  *:33374            *:*    users:(("rw-core",pid=3110,fd=12))                         
...
-A f2b-ufw-portscan -s 216.180.246.144/32 -j REJECT --reject-with icmp-port-unreachable
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 39121 bytes 8923967 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 39121 bytes 3604007 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		...
		ct state related,established counter packets 140869529 bytes 151893835993 accept
		ct state invalid counter packets 494 bytes 62654 jump ufw-logging-deny
		ct state invalid counter packets 494 bytes 62654 drop
		...
		ip protocol icmp icmp type echo-request counter packets 4963 bytes 303722 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 1311688 bytes 267131889 jump ufw-not-local
		...
		counter packets 1311688 bytes 267131889 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 1932 bytes 318825 accept
		ct state related,established counter packets 145752599 bytes 157290703420 accept
		counter packets 2076159 bytes 323322905 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		udp dport 137 counter packets 112 bytes 8952 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 8 bytes 1616 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 75 bytes 3492 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 960 bytes 44080 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 573152 bytes 187993352 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 8 bytes 224 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 102323 bytes 33187229 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 39120 bytes 2386330 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 39120 bytes 3584188 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 932510 bytes 55951600 accept
		ip protocol udp ct state new counter packets 1140647 bytes 267080398 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 2456658 bytes 6023799457 jump f2b-ufw-portscan
		iifname "tun79" counter packets 0 bytes 0 accept
		counter packets 142188606 bytes 152161653083 jump ufw-before-logging-input
		counter packets 142188606 bytes 152161653083 jump ufw-before-input
		counter packets 932770 bytes 244247991 jump ufw-after-input
		counter packets 256132 bytes 23009046 jump ufw-after-logging-input
		counter packets 256132 bytes 23009046 jump ufw-reject-input
		counter packets 256132 bytes 23009046 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 0 bytes 0 accept
		counter packets 147830690 bytes 157614345150 jump ufw-before-logging-output
		counter packets 147830690 bytes 157614345150 jump ufw-before-output
		counter packets 2076159 bytes 323322905 jump ufw-after-output
		counter packets 2076159 bytes 323322905 jump ufw-after-logging-output
		counter packets 2076159 bytes 323322905 jump ufw-reject-output
		counter packets 2076159 bytes 323322905 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 193910951 bytes 192506349759 jump DOCKER-USER
		counter packets 193910952 bytes 192506349819 jump DOCKER-FORWARD
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 2340914 bytes 2866811189 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 3180931 bytes 1636503850 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 494 bytes 62654 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 494 bytes 62654 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 676638 bytes 221238945 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 636201 bytes 45951902 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 675487 bytes 221179987 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 205054 bytes 12056208 accept
		udp dport 46539 counter packets 61 bytes 79367 accept
		udp dport 39425 counter packets 158 bytes 14738 accept
		tcp dport 80 counter packets 139927 bytes 8241146 accept
		udp dport 1194 counter packets 79 bytes 3932 accept
		tcp dport 8443 counter packets 31102 bytes 1825104 accept
		udp dport 21194 counter packets 22 bytes 3432 accept
		iifname "tun79" counter packets 0 bytes 0 accept
		udp dport 33328 counter packets 1 bytes 389 accept
		tcp dport 52000 counter packets 197 bytes 10724 accept
		udp dport 51820 counter packets 1 bytes 214 accept
		tcp dport 25 counter packets 1837 bytes 104480 accept
		...
		ip saddr 93.183.106.203 tcp dport 2222 counter packets 34 bytes 2040 accept
	...
	chain DOCKER {
		ip daddr 172.29.172.2 iifname != "amn0" oifname "amn0" udp dport 39425 counter packets 12567 bytes 1351250 accept
```
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 49 | 2026-09-23 04:30:07 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-09-22 14:18:19 |
...
total 476K
drwxr-xr-x  2 root root   4.0K Sep 23 03:10 .
