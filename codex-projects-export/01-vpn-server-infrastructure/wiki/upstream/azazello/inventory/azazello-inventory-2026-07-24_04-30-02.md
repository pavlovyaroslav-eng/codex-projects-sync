...
Generated: 2026-07-24T04:30:02+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:02 up 47 days, 14:09,  0 user,  load average: 0.41, 0.49, 0.39
```
...
-A f2b-ufw-portscan -s 104.83.4.43/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 17.253.122.200/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 91.108.4.168/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 91.108.4.150/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 185.46.220.130/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 151.101.67.6/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 188.254.122.180/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 45.225.135.171/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 2.16.16.147/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 85.217.149.65/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 104.83.5.153/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 17.248.213.66/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 71.18.255.145/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 78.156.235.143/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 173.194.151.98/32 -j REJECT --reject-with icmp-port-unreachable
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 25187 bytes 10260329 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 25187 bytes 1591262 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		iifname "lo" counter packets 26855687 bytes 14337430115 accept
		ct state related,established counter packets 351010283 bytes 241930044131 accept
		ct state invalid counter packets 26624 bytes 2131222 jump ufw-logging-deny
		ct state invalid counter packets 26624 bytes 2131222 drop
		...
		ip protocol icmp icmp type echo-request counter packets 47621 bytes 2741805 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 4655089 bytes 1624881439 jump ufw-not-local
		...
		counter packets 4655089 bytes 1624881439 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 26856035 bytes 14337461349 accept
		ct state related,established counter packets 432033204 bytes 498160606532 accept
		counter packets 2317937 bytes 285236770 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		udp dport 137 counter packets 340 bytes 30938 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 3 bytes 2383 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 289 bytes 15036 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 1769 bytes 89460 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 1912364 bytes 627257436 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 3 bytes 2384 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 320956 bytes 97236191 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 25187 bytes 6563091 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 25187 bytes 1764462 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 394963 bytes 25210128 accept
		ip protocol udp ct state new counter packets 1877277 bytes 255594658 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 14736352 bytes 12986025424 jump f2b-ufw-portscan
		iifname "tun79" counter packets 1090750 bytes 1359553160 accept
		counter packets 756670779 bytes 624336210315 jump ufw-before-logging-input
		counter packets 756670779 bytes 624336210315 jump ufw-before-input
		counter packets 7425615 bytes 1572445186 jump ufw-after-input
		counter packets 3268623 bytes 225452038 jump ufw-after-logging-input
		counter packets 3268623 bytes 225452038 jump ufw-reject-input
		counter packets 3268623 bytes 225452038 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 31413 bytes 6024246 accept
		counter packets 779363926 bytes 791291231435 jump ufw-before-logging-output
		counter packets 779363926 bytes 791291231435 jump ufw-before-output
		counter packets 5821636 bytes 788763848 jump ufw-after-output
		counter packets 5821636 bytes 788763848 jump ufw-after-logging-output
		counter packets 5821636 bytes 788763848 jump ufw-reject-output
		counter packets 5821636 bytes 788763848 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 156922206 bytes 369107744188 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 188167348 bytes 41278310822 accept
		...
		counter packets 719157543 bytes 773619017606 jump DOCKER-USER
		counter packets 719157543 bytes 773619017606 jump DOCKER-FORWARD
	...
	chain DOCKER {
		ip daddr 172.29.172.2 iifname != "amn0" oifname "amn0" udp dport 39425 counter packets 20451 bytes 3993339 accept
		ip daddr 172.17.0.3 iifname != "docker0" oifname "docker0" tcp dport 443 counter packets 39443 bytes 2379356 accept
	...
	chain DOCKER-FORWARD {
		counter packets 719157543 bytes 773619017606 jump DOCKER-CT
		counter packets 361378520 bytes 384000856209 jump DOCKER-INTERNAL
		counter packets 361378520 bytes 384000856209 jump DOCKER-BRIDGE
		iifname "br-566ebeb0f4f1" counter packets 0 bytes 0 accept
		iifname "amn0" counter packets 268471526 bytes 347086257582 accept
		iifname "docker0" counter packets 92711904 bytes 36896054717 accept
	}

	chain DOCKER-BRIDGE {
		oifname "br-566ebeb0f4f1" counter packets 0 bytes 0 jump DOCKER
		oifname "amn0" counter packets 58537 bytes 10243232 jump DOCKER
		oifname "docker0" counter packets 136001 bytes 8269068 jump DOCKER
	}

	chain DOCKER-CT {
		oifname "br-566ebeb0f4f1" ct state related,established counter packets 0 bytes 0 accept
		oifname "amn0" ct state related,established counter packets 277072258 bytes 337079808288 accept
		oifname "docker0" ct state related,established counter packets 80706765 bytes 52538353109 accept
	}
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 4403 bytes 296888 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 4403 bytes 296888 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 2235724 bytes 724633828 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 2421632 bytes 900376844 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 2233457 bytes 724504595 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 13759 bytes 810629 accept
		udp dport 46539 counter packets 309956 bytes 430918842 accept
		udp dport 39425 counter packets 204 bytes 19124 accept
		tcp dport 80 counter packets 53734 bytes 3202360 accept
		udp dport 1194 counter packets 68 bytes 5433 accept
		tcp dport 8443 counter packets 3125 bytes 180994 accept
		...
		tcp dport 52000 counter packets 215 bytes 10912 accept
		udp dport 51820 counter packets 0 bytes 0 accept
		tcp dport 25 counter packets 618 bytes 34288 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 43 | 2026-07-24 04:30:06 |
...
| `/etc/x-ui` | directory | root:root | 755 | 4096 | 2026-07-24 04:29:43 |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 124K
drwxr-xr-x  2 root root   4.0K Jul 24 03:10 .
