...
Generated: 2026-09-08T04:30:02+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:02 up 37 days,  1:17,  0 user,  load average: 0.81, 0.56, 0.54
```
...
udp   UNCONN 5056   0                  *:24443            *:*    users:(("rw-core",pid=2921938,fd=7))                         
udp   UNCONN 0      0                  *:36877            *:*    users:(("rw-core",pid=2921938,fd=22))                        
udp   UNCONN 0      0              [::1]:8388          [::]:*    users:(("ss-server",pid=1113,fd=7))                          
udp   UNCONN 0      0               [::]:39425         [::]:*    users:(("docker-proxy",pid=3859403,fd=7))                    
udp   UNCONN 0      0                  *:47679            *:*    users:(("rw-core",pid=2921938,fd=20))                        
udp   UNCONN 0      0               [::]:51820         [::]:*                                                                 
udp   UNCONN 0      0                  *:60507            *:*    users:(("rw-core",pid=2921938,fd=19))                        
...
-A f2b-ufw-portscan -s 138.199.43.188/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 85.217.140.6/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 172.217.17.74/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 169.58.251.213/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 193.84.88.27/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 121.41.165.233/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 121.41.167.163/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 81.163.50.246/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 103.249.70.31/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 3.101.240.24/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 3.42.1.30/32 -j REJECT --reject-with icmp-port-unreachable
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 20972 bytes 5361597 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 20972 bytes 1583828 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		...
		ct state related,established counter packets 53003802 bytes 51699585864 accept
		ct state invalid counter packets 397 bytes 32350 jump ufw-logging-deny
		ct state invalid counter packets 397 bytes 32350 drop
		...
		ip protocol icmp icmp type echo-request counter packets 2447 bytes 140807 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 1001772 bytes 338441358 jump ufw-not-local
		...
		counter packets 1001772 bytes 338441358 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 203166 bytes 86146464 accept
		ct state related,established counter packets 56019500 bytes 52155992549 accept
		counter packets 1175625 bytes 128431939 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		udp dport 137 counter packets 71 bytes 5753 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 50 bytes 2284 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 272 bytes 13320 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 365599 bytes 119916076 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 2 bytes 56 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 60817 bytes 18307599 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 20971 bytes 1267862 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 20972 bytes 1588374 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 560278 bytes 33616680 accept
		ip protocol udp ct state new counter packets 613630 bytes 94663808 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp tcp dport 52000 counter packets 3235 bytes 749217 jump f2b-sshd
		ip protocol tcp counter packets 13926652 bytes 29463451461 jump f2b-ufw-portscan
		iifname "tun79" counter packets 0 bytes 0 accept
		counter packets 362507325 bytes 373804545479 jump ufw-before-logging-input
		counter packets 362507325 bytes 373804545479 jump ufw-before-input
		counter packets 4344886 bytes 1131501785 jump ufw-after-input
		counter packets 1094805 bytes 81023777 jump ufw-after-logging-input
		counter packets 1094805 bytes 81023777 jump ufw-reject-input
		counter packets 1094805 bytes 81023777 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 0 bytes 0 accept
		counter packets 312617648 bytes 250280203801 jump ufw-before-logging-output
		counter packets 312617648 bytes 250280203801 jump ufw-before-output
		counter packets 2124824 bytes 221554895 jump ufw-after-output
		counter packets 2124824 bytes 221554895 jump ufw-after-logging-output
		counter packets 2124824 bytes 221554895 jump ufw-reject-output
		counter packets 2124824 bytes 221554895 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 1649105757 bytes 1993487547465 jump DOCKER-USER
		counter packets 1649105764 bytes 1993487548679 jump DOCKER-FORWARD
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 7842123 bytes 9686248572 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 9901088 bytes 4421408597 accept
	...
	chain DOCKER {
		ip daddr 172.29.172.2 iifname != "amn0" oifname "amn0" udp dport 39425 counter packets 1650 bytes 242853 accept
		ip daddr 172.17.0.2 iifname != "docker0" oifname "docker0" tcp dport 443 counter packets 414795 bytes 25560958 accept
	...
	chain DOCKER-FORWARD {
		counter packets 1649105764 bytes 1993487548679 jump DOCKER-CT
		counter packets 831464253 bytes 1008983630575 jump DOCKER-INTERNAL
		counter packets 831464253 bytes 1008983630575 jump DOCKER-BRIDGE
		iifname "br-566ebeb0f4f1" counter packets 0 bytes 0 accept
		iifname "amn0" counter packets 618068497 bytes 873070027230 accept
		iifname "docker0" counter packets 195206427 bytes 121775150922 accept
	}

	chain DOCKER-BRIDGE {
		oifname "br-566ebeb0f4f1" counter packets 22 bytes 1320 jump DOCKER
		oifname "amn0" counter packets 31321 bytes 5242253 jump DOCKER
		oifname "docker0" counter packets 414795 bytes 25560958 jump DOCKER
	}

	chain DOCKER-CT {
		oifname "br-566ebeb0f4f1" ct state related,established counter packets 0 bytes 0 accept
		oifname "amn0" ct state related,established counter packets 635331609 bytes 846558611828 accept
		oifname "docker0" ct state related,established counter packets 182309902 bytes 137945306276 accept
	}
	...
	chain f2b-ufw-portscan {
		ip saddr 185.165.170.59 counter packets 2 bytes 112 reject
		ip saddr 5.83.129.207 counter packets 52 bytes 2080 reject
		ip saddr 89.144.35.106 counter packets 83 bytes 3320 reject
		ip saddr 85.93.10.7 counter packets 74 bytes 2960 reject
		ip saddr 85.217.140.37 counter packets 29 bytes 1508 reject
		ip saddr 85.217.149.53 counter packets 45 bytes 2340 reject
		ip saddr 85.217.140.33 counter packets 30 bytes 1560 reject
		ip saddr 85.217.140.49 counter packets 138 bytes 7176 reject
		ip saddr 85.217.140.2 counter packets 25 bytes 1300 reject
		ip saddr 85.217.140.35 counter packets 25 bytes 1300 reject
		ip saddr 85.217.149.72 counter packets 86 bytes 4472 reject
		ip saddr 85.217.149.74 counter packets 149 bytes 7748 reject
		ip saddr 138.199.43.188 counter packets 123 bytes 7380 reject
		ip saddr 85.217.140.6 counter packets 83 bytes 4316 reject
		ip saddr 172.217.17.74 counter packets 83 bytes 18606 reject
		ip saddr 85.217.140.31 counter packets 36 bytes 1872 reject
		ip saddr 85.217.149.70 counter packets 153 bytes 7956 reject
		ip saddr 85.217.149.37 counter packets 94 bytes 4888 reject
		ip saddr 85.217.140.11 counter packets 243 bytes 12636 reject
		ip saddr 85.217.140.48 counter packets 165 bytes 8580 reject
		ip saddr 85.217.149.73 counter packets 286 bytes 14872 reject
		ip saddr 85.217.140.30 counter packets 179 bytes 9308 reject
		ip saddr 85.217.149.50 counter packets 77 bytes 4004 reject
		ip saddr 85.217.140.19 counter packets 91 bytes 4732 reject
		ip saddr 85.217.149.63 counter packets 359 bytes 18668 reject
		ip saddr 31.14.32.6 counter packets 220 bytes 11440 reject
		ip saddr 85.217.140.39 counter packets 581 bytes 30212 reject
		ip saddr 169.58.251.213 counter packets 11 bytes 652 reject
		ip saddr 85.217.140.22 counter packets 137 bytes 7124 reject
		ip saddr 85.217.149.51 counter packets 222 bytes 11544 reject
		ip saddr 85.217.140.43 counter packets 450 bytes 23400 reject
		ip saddr 85.217.140.34 counter packets 277 bytes 14404 reject
		ip saddr 85.217.149.68 counter packets 565 bytes 29380 reject
		ip saddr 85.217.140.52 counter packets 631 bytes 32812 reject
		ip saddr 85.217.140.47 counter packets 526 bytes 27352 reject
		ip saddr 85.217.140.9 counter packets 564 bytes 29328 reject
		ip saddr 193.84.88.27 counter packets 15 bytes 884 reject
		ip saddr 85.217.140.41 counter packets 585 bytes 30420 reject
		ip saddr 85.217.149.67 counter packets 737 bytes 38324 reject
		ip saddr 85.217.149.64 counter packets 619 bytes 32188 reject
		ip saddr 121.41.165.233 counter packets 97 bytes 3880 reject
		ip saddr 85.217.140.8 counter packets 667 bytes 34684 reject
		ip saddr 85.93.10.49 counter packets 527 bytes 21080 reject
		ip saddr 85.217.140.44 counter packets 634 bytes 32968 reject
		ip saddr 85.217.140.27 counter packets 181 bytes 9412 reject
		ip saddr 143.42.238.18 counter packets 591 bytes 26004 reject
		ip saddr 85.217.140.4 counter packets 751 bytes 39052 reject
		ip saddr 85.217.140.40 counter packets 711 bytes 36972 reject
```
...
remnanode                    remnawave/node:3.4.1             Up 4 days     
mtproto-telegram             telegrammessenger/proxy:latest   Up 5 weeks    0.0.0.0:9443->443/tcp, [::]:9443->443/tcp
amnezia-awg2                 amnezia-awg2                     Up 10 hours   0.0.0.0:39425->39425/udp, [::]:39425->39425/udp
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 48 | 2026-09-08 04:29:48 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-09-07 23:06:20 |
...
total 388K
drwxr-xr-x  2 root root   4.0K Sep  8 03:10 .
