...
Generated: 2026-08-31T04:30:02+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:02 up 29 days,  1:17,  0 user,  load average: 0.29, 0.25, 0.23
```
...
-A f2b-ufw-portscan -s 2.16.16.149/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 17.253.73.204/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 151.101.64.84/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 103.127.196.38/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 103.236.108.190/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 139.59.236.63/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 151.101.194.167/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 151.101.2.167/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 165.232.143.119/32 -j REJECT --reject-with icmp-port-unreachable
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 125521 bytes 44513083 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 125521 bytes 7725408 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		iifname "lo" counter packets 31104874 bytes 16374068605 accept
		ct state related,established counter packets 244931945 bytes 274459795228 accept
		ct state invalid counter packets 23846 bytes 1392389 jump ufw-logging-deny
		ct state invalid counter packets 23846 bytes 1392389 drop
		...
		ip protocol icmp icmp type echo-request counter packets 19891 bytes 1240628 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 6532845 bytes 3713153614 jump ufw-not-local
		...
		counter packets 6532845 bytes 3713153614 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 31104874 bytes 16374068605 accept
		ct state related,established counter packets 204829473 bytes 165639190078 accept
		counter packets 854799 bytes 86232904 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		udp dport 137 counter packets 519 bytes 43773 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 731 bytes 167198 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 271 bytes 12788 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 1790 bytes 87812 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 2174160 bytes 713123506 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 1 bytes 28 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 369064 bytes 109208932 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 125495 bytes 7795009 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 125520 bytes 8928257 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 208835 bytes 13538355 accept
		ip protocol udp ct state new counter packets 620600 bytes 70374816 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 1634791 bytes 1127684844 jump f2b-ufw-portscan
		iifname "tun79" counter packets 0 bytes 0 accept
		counter packets 282613401 bytes 294549650464 jump ufw-before-logging-input
		counter packets 282613401 bytes 294549650464 jump ufw-before-input
		counter packets 3472364 bytes 893514117 jump ufw-after-input
		counter packets 925828 bytes 70870080 jump ufw-after-logging-input
		counter packets 925828 bytes 70870080 jump ufw-reject-input
		counter packets 925828 bytes 70870080 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 0 bytes 0 accept
		counter packets 236789146 bytes 182099491587 jump ufw-before-logging-output
		counter packets 236789146 bytes 182099491587 jump ufw-before-output
		counter packets 854799 bytes 86232904 jump ufw-after-output
		counter packets 854799 bytes 86232904 jump ufw-after-logging-output
		counter packets 854799 bytes 86232904 jump ufw-reject-output
		counter packets 854799 bytes 86232904 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 1409368957 bytes 1725750965466 jump DOCKER-USER
		counter packets 1409368964 bytes 1725750966680 jump DOCKER-FORWARD
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 6776018 bytes 8313445096 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 8449489 bytes 3763535207 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 17491 bytes 1097787 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 17491 bytes 1097787 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 2546536 bytes 822644037 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 3988612 bytes 2890628747 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 2544233 bytes 822524867 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 490424 bytes 23785457 accept
		udp dport 46539 counter packets 1990416 bytes 2767471319 accept
		udp dport 39425 counter packets 683 bytes 68341 accept
		tcp dport 80 counter packets 474268 bytes 22182011 accept
		udp dport 1194 counter packets 288 bytes 12110 accept
		tcp dport 8443 counter packets 99471 bytes 5845173 accept
		udp dport 21194 counter packets 53 bytes 8092 accept
		iifname "tun79" counter packets 0 bytes 0 accept
		udp dport 33328 counter packets 1 bytes 438 accept
		tcp dport 52000 counter packets 747 bytes 38512 accept
		udp dport 51820 counter packets 1 bytes 436 accept
		tcp dport 25 counter packets 4124 bytes 227308 accept
	...
	chain DOCKER {
		ip daddr 172.29.172.2 iifname != "amn0" oifname "amn0" udp dport 39425 counter packets 24002 bytes 4349277 accept
		ip daddr 172.17.0.2 iifname != "docker0" oifname "docker0" tcp dport 443 counter packets 324018 bytes 20029741 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 47 | 2026-08-31 04:30:09 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-08-30 22:00:10 |
| `/root/cert` | directory | root:root | 755 | 4096 | 2025-03-17 17:20:58 |
| `/etc/x-ui` | directory | root:root | 755 | 4096 | 2026-08-31 04:30:10 |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 352K
drwxr-xr-x  2 root root   4.0K Aug 31 03:10 .
drwxr-xr-x 18 root syslog 4.0K Aug 30 00:00 ..
