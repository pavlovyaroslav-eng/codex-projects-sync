...
Generated: 2026-09-15T04:30:08+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:08 up 1 day,  1:17,  0 user,  load average: 0.47, 0.24, 0.20
```
...
-A f2b-ufw-portscan -s 222.167.35.115/32 -j REJECT --reject-with icmp-port-unreachable
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 4561 bytes 977728 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 4560 bytes 370611 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		...
		ct state related,established counter packets 14500772 bytes 15954104263 accept
		ct state invalid counter packets 84 bytes 25570 jump ufw-logging-deny
		ct state invalid counter packets 84 bytes 25570 drop
		...
		ip protocol icmp icmp type echo-request counter packets 339 bytes 18011 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 195801 bytes 36737438 jump ufw-not-local
		...
		counter packets 195801 bytes 36737438 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 80 bytes 5920 accept
		ct state related,established counter packets 16280039 bytes 16779085369 accept
		counter packets 263643 bytes 35183077 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		udp dport 137 counter packets 11 bytes 858 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 4 bytes 196 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 121 bytes 5628 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 79654 bytes 26126512 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 13627 bytes 4153210 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 4560 bytes 297540 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 4560 bytes 376020 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 106355 bytes 6381268 accept
		ip protocol udp ct state new counter packets 156913 bytes 28760298 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 3485637 bytes 9056489373 jump f2b-ufw-portscan
		iifname "tun79" counter packets 0 bytes 0 accept
		counter packets 14697076 bytes 15990891202 jump ufw-before-logging-input
		counter packets 14697076 bytes 15990891202 jump ufw-before-input
		counter packets 112291 bytes 31556231 jump ufw-after-input
		counter packets 18874 bytes 1269827 jump ufw-after-logging-input
		counter packets 18874 bytes 1269827 jump ufw-reject-input
		counter packets 18874 bytes 1269827 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 0 bytes 0 accept
		counter packets 16543762 bytes 16814274366 jump ufw-before-logging-output
		counter packets 16543762 bytes 16814274366 jump ufw-before-output
		counter packets 263643 bytes 35183077 jump ufw-after-output
		counter packets 263643 bytes 35183077 jump ufw-after-logging-output
		counter packets 263643 bytes 35183077 jump ufw-reject-output
		counter packets 263643 bytes 35183077 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 17207875 bytes 14426274785 jump DOCKER-USER
		counter packets 17207876 bytes 14426274845 jump DOCKER-FORWARD
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 301719 bytes 372475109 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 404150 bytes 202791426 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 84 bytes 25570 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 84 bytes 25570 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 93417 bytes 30286404 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 102517 bytes 6457482 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 93284 bytes 30279956 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 37824 bytes 2230900 accept
		udp dport 46539 counter packets 0 bytes 0 accept
		udp dport 39425 counter packets 100 bytes 9028 accept
		tcp dport 80 counter packets 44044 bytes 2596614 accept
		udp dport 1194 counter packets 6 bytes 237 accept
		tcp dport 8443 counter packets 960 bytes 56320 accept
		udp dport 21194 counter packets 20 bytes 3120 accept
		iifname "tun79" counter packets 0 bytes 0 accept
		udp dport 33328 counter packets 0 bytes 0 accept
		tcp dport 52000 counter packets 26 bytes 1432 accept
		udp dport 51820 counter packets 0 bytes 0 accept
		tcp dport 25 counter packets 276 bytes 16220 accept
		...
		ip saddr 185.71.196.110 udp dport 24443 counter packets 202 bytes 264216 accept
	}
	...
	chain f2b-ufw-portscan {
		ip saddr 15.235.149.124 counter packets 31 bytes 1836 reject
```
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 49 | 2026-09-15 04:29:58 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-09-14 22:52:47 |
