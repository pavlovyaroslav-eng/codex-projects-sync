...
Generated: 2026-08-27T04:30:01+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:02 up 25 days,  1:17,  0 user,  load average: 0.33, 0.28, 0.44
```
...
-A f2b-ufw-portscan -s 194.34.236.55/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 45.33.14.197/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 10.8.1.10/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 2.23.97.24/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 2.19.197.96/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 18.199.237.58/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 45.131.48.51/32 -j REJECT --reject-with icmp-port-unreachable
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 108241 bytes 38984849 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 108241 bytes 6675727 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		iifname "lo" counter packets 26813324 bytes 14061616304 accept
		ct state related,established counter packets 220134128 bytes 251949371288 accept
		ct state invalid counter packets 22867 bytes 1319328 jump ufw-logging-deny
		ct state invalid counter packets 22867 bytes 1319328 drop
		...
		ip protocol icmp icmp type echo-request counter packets 17138 bytes 1059563 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 5506926 bytes 3210184065 jump ufw-not-local
		...
		counter packets 5506926 bytes 3210184065 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 26813324 bytes 14061616304 accept
		ct state related,established counter packets 181713806 bytes 152123785055 accept
		counter packets 758384 bytes 78086161 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		udp dport 137 counter packets 454 bytes 38278 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 731 bytes 167198 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 233 bytes 11032 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 1580 bytes 77604 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 1870964 bytes 613675516 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 318446 bytes 93953446 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 108240 bytes 6712989 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 108240 bytes 7735428 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 181986 bytes 11927415 accept
		ip protocol udp ct state new counter packets 552567 bytes 63999620 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 82394114 bytes 96330857218 jump f2b-ufw-portscan
		iifname "tun79" counter packets 0 bytes 0 accept
		counter packets 252494383 bytes 269223550548 jump ufw-before-logging-input
		counter packets 252494383 bytes 269223550548 jump ufw-before-input
		counter packets 3043815 bytes 774358353 jump ufw-after-input
		counter packets 851407 bytes 66435279 jump ufw-after-logging-input
		counter packets 851407 bytes 66435279 jump ufw-reject-input
		counter packets 851407 bytes 66435279 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 0 bytes 0 accept
		counter packets 209285514 bytes 166263487520 jump ufw-before-logging-output
		counter packets 209285514 bytes 166263487520 jump ufw-before-output
		counter packets 758384 bytes 78086161 jump ufw-after-output
		counter packets 758384 bytes 78086161 jump ufw-after-logging-output
		counter packets 758384 bytes 78086161 jump ufw-reject-output
		counter packets 758384 bytes 78086161 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 1231187392 bytes 1513800163248 jump DOCKER-USER
		counter packets 1231187399 bytes 1513800164462 jump DOCKER-FORWARD
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 5696685 bytes 6925186737 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 7103116 bytes 3163965083 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 16557 bytes 1026694 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 16557 bytes 1026694 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 2192408 bytes 707923074 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 3316530 bytes 2502364950 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 2190396 bytes 707819115 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 336202 bytes 17221906 accept
		udp dport 46539 counter packets 1727154 bytes 2398295008 accept
		udp dport 39425 counter packets 636 bytes 63499 accept
		tcp dport 80 counter packets 307144 bytes 14858899 accept
		udp dport 1194 counter packets 243 bytes 10157 accept
		tcp dport 8443 counter packets 87259 bytes 5126445 accept
		udp dport 21194 counter packets 44 bytes 6736 accept
		iifname "tun79" counter packets 0 bytes 0 accept
		udp dport 33328 counter packets 1 bytes 438 accept
		tcp dport 52000 counter packets 660 bytes 33872 accept
		udp dport 51820 counter packets 1 bytes 436 accept
		tcp dport 25 counter packets 3762 bytes 208016 accept
	...
	chain DOCKER {
		ip daddr 172.29.172.2 iifname != "amn0" oifname "amn0" udp dport 39425 counter packets 20964 bytes 3731265 accept
		ip daddr 172.17.0.2 iifname != "docker0" oifname "docker0" tcp dport 443 counter packets 283949 bytes 17569508 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 48 | 2026-08-27 04:30:12 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-08-26 20:14:04 |
| `/root/cert` | directory | root:root | 755 | 4096 | 2025-03-17 17:20:58 |
| `/etc/x-ui` | directory | root:root | 755 | 4096 | 2026-08-27 04:30:10 |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 312K
drwxr-xr-x  2 root root   4.0K Aug 27 03:10 .
