...
Generated: 2026-08-24T04:30:02+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:03 up 22 days,  1:17,  0 user,  load average: 0.33, 0.20, 0.20
```
...
tcp   LISTEN 0      100          0.0.0.0:25         0.0.0.0:*    users:(("master",pid=3101833,fd=13),("smtpd",pid=679057,fd=6))
...
-A f2b-ufw-portscan -s 34.36.102.187/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 3.167.37.119/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 45.33.12.122/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 66.228.62.150/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 85.26.177.68/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 13.36.104.214/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 194.35.227.224/32 -j REJECT --reject-with icmp-port-unreachable
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 95281 bytes 35297230 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 95281 bytes 5886280 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		iifname "lo" counter packets 23603160 bytes 12335655983 accept
		ct state related,established counter packets 218142250 bytes 250219359438 accept
		ct state invalid counter packets 16320 bytes 1007924 jump ufw-logging-deny
		ct state invalid counter packets 16320 bytes 1007924 drop
		...
		ip protocol icmp icmp type echo-request counter packets 15382 bytes 953320 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 4581271 bytes 2809054292 jump ufw-not-local
		...
		counter packets 4581271 bytes 2809054292 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 23603160 bytes 12335655983 accept
		ct state related,established counter packets 178718555 bytes 150498543103 accept
		counter packets 696936 bytes 72991412 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		udp dport 137 counter packets 425 bytes 36016 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 731 bytes 167198 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 205 bytes 9760 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 1383 bytes 67888 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 1646953 bytes 540199985 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 281173 bytes 82930892 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 95281 bytes 5978530 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 95280 bytes 6838301 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 164425 bytes 10713399 accept
		ip protocol udp ct state new counter packets 510259 bytes 60260180 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 78206332 bytes 94097416359 jump f2b-ufw-portscan
		iifname "tun79" counter packets 0 bytes 0 accept
		counter packets 246358383 bytes 265366030957 jump ufw-before-logging-input
		counter packets 246358383 bytes 265366030957 jump ufw-before-input
		counter packets 2555172 bytes 677331549 jump ufw-after-input
		counter packets 624302 bytes 53919810 jump ufw-after-logging-input
		counter packets 624302 bytes 53919810 jump ufw-reject-input
		counter packets 624302 bytes 53919810 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 0 bytes 0 accept
		counter packets 203018651 bytes 162907190498 jump ufw-before-logging-output
		counter packets 203018651 bytes 162907190498 jump ufw-before-output
		counter packets 696936 bytes 72991412 jump ufw-after-output
		counter packets 696936 bytes 72991412 jump ufw-after-logging-output
		counter packets 696936 bytes 72991412 jump ufw-reject-output
		counter packets 696936 bytes 72991412 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 1074569624 bytes 1327388498746 jump DOCKER-USER
		counter packets 1074569631 bytes 1327388499960 jump DOCKER-FORWARD
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 5084044 bytes 6231487422 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 6358663 bytes 2828636181 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 13688 bytes 869339 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 13688 bytes 869339 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 1930870 bytes 623411739 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 2652159 bytes 2185733262 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 1929112 bytes 623321030 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 231016 bytes 12418901 accept
		udp dport 46539 counter packets 1516936 bytes 2104744190 accept
		udp dport 39425 counter packets 625 bytes 62591 accept
		tcp dport 80 counter packets 203356 bytes 10155968 accept
		udp dport 1194 counter packets 216 bytes 9072 accept
		tcp dport 8443 counter packets 69886 bytes 4103630 accept
		udp dport 21194 counter packets 44 bytes 6736 accept
		iifname "tun79" counter packets 0 bytes 0 accept
		udp dport 33328 counter packets 1 bytes 438 accept
		tcp dport 52000 counter packets 566 bytes 29481 accept
		udp dport 51820 counter packets 1 bytes 436 accept
		tcp dport 25 counter packets 3447 bytes 191000 accept
	...
	chain DOCKER {
		ip daddr 172.29.172.2 iifname != "amn0" oifname "amn0" udp dport 39425 counter packets 19590 bytes 3579304 accept
		ip daddr 172.17.0.2 iifname != "docker0" oifname "docker0" tcp dport 443 counter packets 241226 bytes 14927405 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 47 | 2026-08-24 04:30:16 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-08-24 01:55:07 |
| `/root/cert` | directory | root:root | 755 | 4096 | 2025-03-17 17:20:58 |
| `/etc/x-ui` | directory | root:root | 755 | 4096 | 2026-08-24 04:30:10 |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 300K
drwxr-xr-x  2 root root   4.0K Aug 24 03:10 .
