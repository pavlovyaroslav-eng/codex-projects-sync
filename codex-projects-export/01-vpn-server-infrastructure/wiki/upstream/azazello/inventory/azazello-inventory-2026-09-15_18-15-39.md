...
Generated: 2026-09-15T18:15:39+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 18:15:39 up 1 day, 15:02,  1 user,  load average: 1.07, 0.75, 0.70
```
...
udp   UNCONN 0      0                  *:53101            *:*    users:(("rw-core",pid=3110,fd=21))                     
udp   UNCONN 0      0                  *:24443            *:*    users:(("rw-core",pid=3110,fd=7))                      
udp   UNCONN 0      0                  *:59421            *:*    users:(("rw-core",pid=3110,fd=43))                     
...
udp   UNCONN 0      0                  *:33950            *:*    users:(("rw-core",pid=3110,fd=10))                     
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 7037 bytes 1454598 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 7037 bytes 573839 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		...
		ct state related,established counter packets 22804282 bytes 26005428763 accept
		...
		counter packets 301215 bytes 56579187 jump ufw-not-local
		...
		counter packets 301215 bytes 56579187 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 80 bytes 5920 accept
		ct state related,established counter packets 22835380 bytes 26827153898 accept
		counter packets 378754 bytes 53274844 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		...
		udp dport 67 counter packets 123159 bytes 40396096 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 1 bytes 28 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 20970 bytes 6437125 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 7036 bytes 455961 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 7037 bytes 583205 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 155262 bytes 9315688 accept
		ip protocol udp ct state new counter packets 222930 bytes 43890993 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 6246110 bytes 15174441975 jump f2b-ufw-portscan
		iifname "tun79" counter packets 0 bytes 0 accept
		counter packets 23106215 bytes 26062078920 jump ufw-before-logging-input
		counter packets 23106215 bytes 26062078920 jump ufw-before-input
		counter packets 172727 bytes 48742235 jump ufw-after-input
		counter packets 28366 bytes 1897412 jump ufw-after-logging-input
		counter packets 28366 bytes 1897412 jump ufw-reject-input
		counter packets 28366 bytes 1897412 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 0 bytes 0 accept
		counter packets 23214214 bytes 26880434662 jump ufw-before-logging-output
		counter packets 23214214 bytes 26880434662 jump ufw-before-output
		counter packets 378754 bytes 53274844 jump ufw-after-output
		counter packets 378754 bytes 53274844 jump ufw-after-logging-output
		counter packets 378754 bytes 53274844 jump ufw-reject-output
		counter packets 378754 bytes 53274844 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 27164726 bytes 23306582738 jump DOCKER-USER
		counter packets 27164727 bytes 23306582798 jump DOCKER-FORWARD
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 431680 bytes 525910549 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 587007 bytes 304873809 accept
	...
	chain ufw-skip-to-policy-input {
		counter packets 144361 bytes 46844823 drop
	}
	...
	chain ufw-user-input {
		...
		tcp dport 52000 counter packets 52 bytes 2948 accept
