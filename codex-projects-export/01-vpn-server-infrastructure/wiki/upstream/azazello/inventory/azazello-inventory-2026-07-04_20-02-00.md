...
Generated: 2026-07-04T20:02:00+00:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 20:02:01 up 28 days,  8:41,  1 user,  load average: 2.90, 1.47, 1.11
```
...
udp   UNCONN 5056   0            0.0.0.0:21194      0.0.0.0:*    users:(("openvpn",pid=709029,fd=5))                        
udp   UNCONN 0      0              *%wg0:56508            *:*    users:(("xray-linux-amd6",pid=712801,fd=901))              
...
udp   UNCONN 0      0              *%wg0:35379            *:*    users:(("xray-linux-amd6",pid=712801,fd=427))              
...
-A f2b-ufw-portscan -s 185.222.106.14/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 178.217.160.136/32 -j REJECT --reject-with icmp-port-unreachable
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 23588 bytes 5831524 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 23587 bytes 1672746 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		iifname "lo" counter packets 6566678 bytes 3931286982 accept
		ct state related,established counter packets 155040166 bytes 130931879367 accept
		ct state invalid counter packets 5660 bytes 372040 jump ufw-logging-deny
		ct state invalid counter packets 5660 bytes 372040 drop
		...
		ip protocol icmp icmp type echo-request counter packets 11684 bytes 677326 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 1226272 bytes 275796663 jump ufw-not-local
		...
		counter packets 1226272 bytes 275796663 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 6566682 bytes 3931287198 accept
		ct state related,established counter packets 163211416 bytes 189529148432 accept
		counter packets 1332968 bytes 184487797 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		...
		tcp dport 445 counter packets 414 bytes 21052 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 454943 bytes 149221950 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 76043 bytes 23154915 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 23588 bytes 1516697 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 23215 bytes 1942834 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 169970 bytes 11603716 accept
		ip protocol udp ct state new counter packets 1153015 bytes 171905616 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 44918320 bytes 43919525129 jump f2b-ufw-portscan
		iifname "tun79" counter packets 356676 bytes 443880582 accept
		counter packets 536925935 bytes 501578993981 jump ufw-before-logging-input
		counter packets 536925935 bytes 501578993981 jump ufw-before-input
		counter packets 4792409 bytes 924268205 jump ufw-after-input
		counter packets 2339536 bytes 129495276 jump ufw-after-logging-input
		counter packets 2339536 bytes 129495276 jump ufw-reject-input
		counter packets 2339536 bytes 129495276 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 10232 bytes 1951718 accept
		counter packets 489267816 bytes 472152850211 jump ufw-before-logging-output
		counter packets 489267816 bytes 472152850211 jump ufw-before-output
		counter packets 4836667 bytes 688014875 jump ufw-after-output
		counter packets 4836667 bytes 688014875 jump ufw-after-logging-output
		counter packets 4836667 bytes 688014875 jump ufw-reject-output
		counter packets 4836667 bytes 688014875 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 50721978 bytes 125089154680 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 60939558 bytes 11171907576 accept
		...
		counter packets 257896069 bytes 297623411517 jump DOCKER-USER
		counter packets 257896069 bytes 297623411517 jump DOCKER-FORWARD
	...
	chain DOCKER {
		ip daddr 172.17.0.3 iifname != "docker0" oifname "docker0" tcp dport 443 counter packets 2690 bytes 166656 accept
	...
	chain DOCKER-FORWARD {
		counter packets 257896069 bytes 297623411517 jump DOCKER-CT
		counter packets 129224180 bytes 151118619395 jump DOCKER-INTERNAL
		counter packets 129224180 bytes 151118619395 jump DOCKER-BRIDGE
		iifname "br-566ebeb0f4f1" counter packets 0 bytes 0 accept
		iifname "amn0" counter packets 127905297 bytes 150802964632 accept
		iifname "docker0" counter packets 1292340 bytes 311837229 accept
	}
	...
	chain DOCKER-BRIDGE {
		...
		oifname "docker0" counter packets 2690 bytes 166656 jump DOCKER
	}

	chain DOCKER-CT {
		oifname "br-566ebeb0f4f1" ct state related,established counter packets 0 bytes 0 accept
		oifname "amn0" ct state related,established counter packets 127672292 bytes 146165253483 accept
		oifname "docker0" ct state related,established counter packets 999597 bytes 339538639 accept
	}
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 4887 bytes 326118 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 4887 bytes 326118 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 531605 bytes 172413609 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 695199 bytes 103411446 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 531073 bytes 172385217 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 151010 bytes 9503084 accept
		udp dport 46539 counter packets 43702 bytes 50424571 accept
		udp dport 39425 counter packets 75 bytes 7699 accept
		tcp dport 80 counter packets 56273 bytes 3360285 accept
		udp dport 1194 counter packets 33 bytes 1323 accept
		...
		tcp dport 52000 counter packets 104 bytes 5692 accept
...
mtproto-telegram             telegrammessenger/proxy:latest   Up 8 hours   0.0.0.0:9443->443/tcp, [::]:9443->443/tcp
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 41 | 2026-07-04 20:01:54 |
...
| `/etc/x-ui` | directory | root:root | 755 | 4096 | 2026-07-04 20:02:07 |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 8.0K
drwxr-xr-x  2 root root   4.0K Jul  4 12:46 .
drwxr-xr-x 16 root syslog 4.0K Jul  4 12:46 ..
```
