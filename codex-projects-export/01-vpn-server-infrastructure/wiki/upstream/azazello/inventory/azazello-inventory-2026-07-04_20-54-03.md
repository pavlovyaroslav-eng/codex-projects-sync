...
Generated: 2026-07-04T20:54:03+00:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 20:54:04 up 28 days,  9:33,  1 user,  load average: 4.87, 3.18, 1.82
```
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 23744 bytes 5862240 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 23744 bytes 1682426 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		iifname "lo" counter packets 6605756 bytes 3956132790 accept
		ct state related,established counter packets 156113367 bytes 131326846976 accept
		ct state invalid counter packets 5700 bytes 374300 jump ufw-logging-deny
		ct state invalid counter packets 5700 bytes 374300 drop
		...
		ip protocol icmp icmp type echo-request counter packets 11938 bytes 691920 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 1238978 bytes 277422714 jump ufw-not-local
		...
		counter packets 1238978 bytes 277422714 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 6605760 bytes 3956133006 accept
		ct state related,established counter packets 164934771 bytes 191732520340 accept
		counter packets 1333859 bytes 184547364 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		...
		udp dport 67 counter packets 457612 bytes 150097382 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 76469 bytes 23273966 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 23744 bytes 1525163 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 23371 bytes 1953160 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 170308 bytes 11623996 accept
		ip protocol udp ct state new counter packets 1153546 bytes 171943227 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 44986293 bytes 43947840800 jump f2b-ufw-portscan
		iifname "tun79" counter packets 362895 bytes 451752351 accept
		counter packets 538051214 bytes 502000450303 jump ufw-before-logging-input
		counter packets 538051214 bytes 502000450303 jump ufw-before-input
		counter packets 4805004 bytes 925769186 jump ufw-after-input
		counter packets 2349034 bytes 130001670 jump ufw-after-logging-input
		counter packets 2349034 bytes 130001670 jump ufw-reject-input
		counter packets 2349034 bytes 130001670 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 10448 bytes 1993040 accept
		counter packets 491031140 bytes 474381127494 jump ufw-before-logging-output
		counter packets 491031140 bytes 474381127494 jump ufw-before-output
		counter packets 4837558 bytes 688074442 jump ufw-after-output
		counter packets 4837558 bytes 688074442 jump ufw-after-logging-output
		counter packets 4837558 bytes 688074442 jump ufw-reject-output
		counter packets 4837558 bytes 688074442 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 51588704 bytes 127099226538 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 61978327 bytes 11457385107 accept
		...
		counter packets 258384382 bytes 297980971886 jump DOCKER-USER
		counter packets 258384382 bytes 297980971886 jump DOCKER-FORWARD
	...
	chain DOCKER {
		ip daddr 172.17.0.3 iifname != "docker0" oifname "docker0" tcp dport 443 counter packets 2960 bytes 183500 accept
	...
	chain DOCKER-FORWARD {
		counter packets 258384382 bytes 297980971886 jump DOCKER-CT
		counter packets 129485999 bytes 151300441346 jump DOCKER-INTERNAL
		counter packets 129485999 bytes 151300441346 jump DOCKER-BRIDGE
		iifname "br-566ebeb0f4f1" counter packets 0 bytes 0 accept
		iifname "amn0" counter packets 128022704 bytes 150949589405 accept
		iifname "docker0" counter packets 1436482 bytes 347017563 accept
	}
	...
	chain DOCKER-BRIDGE {
		...
		oifname "docker0" counter packets 2960 bytes 183500 jump DOCKER
	}

	chain DOCKER-CT {
		oifname "br-566ebeb0f4f1" ct state related,established counter packets 0 bytes 0 accept
		oifname "amn0" ct state related,established counter packets 127789905 bytes 146307428046 accept
		oifname "docker0" ct state related,established counter packets 1108478 bytes 373102494 accept
	}
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 4927 bytes 328378 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 4927 bytes 328378 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 534702 bytes 173408196 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 704810 bytes 104043014 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 534168 bytes 173379700 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 151023 bytes 9503844 accept
		udp dport 46539 counter packets 43786 bytes 50548173 accept
		udp dport 39425 counter packets 75 bytes 7699 accept
		tcp dport 80 counter packets 56286 bytes 3360933 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 41 | 2026-07-04 20:54:04 |
...
| `/etc/x-ui` | directory | root:root | 755 | 4096 | 2026-07-04 20:54:07 |
| `/etc/3x-ui` | missing | - | - | - | - |
