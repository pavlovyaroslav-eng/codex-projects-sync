...
Generated: 2026-07-29T04:30:02+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:02 up 52 days, 14:09,  0 user,  load average: 0.34, 0.42, 0.35
```
...
-A f2b-ufw-portscan -s 62.48.127.128/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 216.180.246.236/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 93.186.237.7/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 199.232.18.13/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 85.217.140.24/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 193.200.10.38/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 192.178.25.163/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 95.163.52.67/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 93.185.164.136/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 216.146.31.1/32 -j REJECT --reject-with icmp-port-unreachable
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 46787 bytes 18827855 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 46787 bytes 2980012 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		iifname "lo" counter packets 32204855 bytes 17202060447 accept
		ct state related,established counter packets 363723441 bytes 253301866789 accept
		ct state invalid counter packets 30526 bytes 2429171 jump ufw-logging-deny
		ct state invalid counter packets 30526 bytes 2429171 drop
		...
		ip protocol icmp icmp type echo-request counter packets 50842 bytes 2928187 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 5969474 bytes 2289463404 jump ufw-not-local
		...
		counter packets 5969474 bytes 2289463404 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 32205203 bytes 17202091681 accept
		ct state related,established counter packets 444201454 bytes 506312005167 accept
		counter packets 2528577 bytes 304776175 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		udp dport 137 counter packets 393 bytes 35288 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 3 bytes 2383 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 327 bytes 16880 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 2089 bytes 105232 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 2291530 bytes 751623884 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 3 bytes 2384 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 387708 bytes 116799650 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 46787 bytes 13261292 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 46787 bytes 3369626 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 443216 bytes 28105308 accept
		ip protocol udp ct state new counter packets 2032919 bytes 271574938 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 5033921 bytes 3265384973 jump f2b-ufw-portscan
		iifname "tun79" counter packets 1090750 bytes 1359553160 accept
		counter packets 776054613 bytes 639237729601 jump ufw-before-logging-input
		counter packets 776054613 bytes 639237729601 jump ufw-before-input
		counter packets 8327542 bytes 1799790079 jump ufw-after-input
		counter packets 3724221 bytes 308845058 jump ufw-after-logging-input
		counter packets 3724221 bytes 308845058 jump ufw-reject-input
		counter packets 3724221 bytes 308845058 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 31413 bytes 6024246 accept
		counter packets 797091984 bytes 802326799807 jump ufw-before-logging-output
		counter packets 797091984 bytes 802326799807 jump ufw-before-output
		counter packets 6032276 bytes 808303253 jump ufw-after-output
		counter packets 6032276 bytes 808303253 jump ufw-after-logging-output
		counter packets 6032276 bytes 808303253 jump ufw-reject-output
		counter packets 6032276 bytes 808303253 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 158257563 bytes 370745554470 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 189988975 bytes 42011024935 accept
		...
		counter packets 938444255 bytes 1035853284394 jump DOCKER-USER
		counter packets 938444255 bytes 1035853284394 jump DOCKER-FORWARD
	...
	chain DOCKER {
		ip daddr 172.29.172.2 iifname != "amn0" oifname "amn0" udp dport 39425 counter packets 31144 bytes 6129851 accept
		ip daddr 172.17.0.3 iifname != "docker0" oifname "docker0" tcp dport 443 counter packets 82417 bytes 5011390 accept
	...
	chain DOCKER-FORWARD {
		counter packets 938444255 bytes 1035853284394 jump DOCKER-CT
		counter packets 470283328 bytes 514979039004 jump DOCKER-INTERNAL
		counter packets 470283328 bytes 514979039004 jump DOCKER-BRIDGE
		iifname "br-566ebeb0f4f1" counter packets 0 bytes 0 accept
		iifname "amn0" counter packets 355218825 bytes 471098490688 accept
		iifname "docker0" counter packets 114815745 bytes 43857235808 accept
	}

	chain DOCKER-BRIDGE {
		oifname "br-566ebeb0f4f1" counter packets 0 bytes 0 jump DOCKER
		oifname "amn0" counter packets 69230 bytes 12379744 jump DOCKER
		oifname "docker0" counter packets 178975 bytes 10901102 jump DOCKER
	}

	chain DOCKER-CT {
		oifname "br-566ebeb0f4f1" ct state related,established counter packets 0 bytes 0 accept
		oifname "amn0" ct state related,established counter packets 368032427 bytes 457449886031 accept
		oifname "docker0" ct state related,established counter packets 100128500 bytes 63424359359 accept
	}
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 7873 bytes 566187 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 7873 bytes 566187 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 2682053 bytes 868585701 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 3290084 bytes 1421027516 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 2679390 bytes 868435888 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 66336 bytes 3912618 accept
		udp dport 46539 counter packets 624827 bytes 862385506 accept
		udp dport 39425 counter packets 385 bytes 36357 accept
		tcp dport 80 counter packets 88200 bytes 5243703 accept
		udp dport 1194 counter packets 115 bytes 7801 accept
		tcp dport 8443 counter packets 12945 bytes 758530 accept
		...
		tcp dport 52000 counter packets 339 bytes 17203 accept
		udp dport 51820 counter packets 2 bytes 1600 accept
		tcp dport 25 counter packets 986 bytes 54736 accept
...
mtproto-telegram             telegrammessenger/proxy:latest   Up 11 days   0.0.0.0:9443->443/tcp, [::]:9443->443/tcp
amnezia-awg2                 amnezia-awg2                     Up 11 days   0.0.0.0:39425->39425/udp, [::]:39425->39425/udp
signal-tls-proxy-certbot-1   certbot/certbot                  Up 11 days   80/tcp, 443/tcp
```
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 43 | 2026-07-29 04:30:19 |
...
| `/etc/x-ui` | directory | root:root | 755 | 4096 | 2026-07-29 04:29:43 |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 160K
drwxr-xr-x  2 root root   4.0K Jul 29 03:10 .
