...
Generated: 2026-09-15T18:14:43+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 18:14:44 up 1 day, 15:01,  1 user,  load average: 1.19, 0.69, 0.67
```
...
udp   UNCONN 0      0                  *:32856            *:*    users:(("rw-core",pid=3110,fd=15))                     
udp   UNCONN 0      0              [::1]:8388          [::]:*    users:(("ss-server",pid=1090,fd=7))                    
udp   UNCONN 0      0                  *:47430            *:*    users:(("rw-core",pid=3110,fd=38))                     
udp   UNCONN 0      0               [::]:39425         [::]:*    users:(("docker-proxy",pid=2667,fd=7))                 
udp   UNCONN 0      0               [::]:51820         [::]:*                                                           
udp   UNCONN 0      0                  *:33402            *:*    users:(("rw-core",pid=3110,fd=34))                     
udp   UNCONN 0      0                  *:58074            *:*    users:(("rw-core",pid=3110,fd=9))                      
udp   UNCONN 0      0                  *:51943            *:*    users:(("rw-core",pid=3110,fd=23))                     
udp   UNCONN 0      0                  *:60190            *:*    users:(("rw-core",pid=3110,fd=35))                     
udp   UNCONN 0      0                  *:35980            *:*    users:(("rw-core",pid=3110,fd=22))                     
udp   UNCONN 0      0                  *:50335            *:*    users:(("rw-core",pid=3110,fd=67))                     
udp   UNCONN 0      0                  *:60818            *:*    users:(("rw-core",pid=3110,fd=29))                     
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 7034 bytes 1454438 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 7034 bytes 573646 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		...
		ct state related,established counter packets 22802640 bytes 26004550270 accept
		ct state invalid counter packets 117 bytes 27815 jump ufw-logging-deny
		ct state invalid counter packets 117 bytes 27815 drop
		...
		ip protocol icmp icmp type echo-request counter packets 521 bytes 37235 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 301095 bytes 56556793 jump ufw-not-local
		...
		counter packets 301095 bytes 56556793 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 80 bytes 5920 accept
		ct state related,established counter packets 22833624 bytes 26826639007 accept
		counter packets 378624 bytes 53254753 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		...
		tcp dport 139 counter packets 13 bytes 608 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 197 bytes 9204 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 123110 bytes 40380024 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 1 bytes 28 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 20961 bytes 6434295 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 7034 bytes 455877 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 7034 bytes 582967 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 155196 bytes 9311728 accept
		ip protocol udp ct state new counter packets 222867 bytes 43874946 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 6245027 bytes 15173854286 jump f2b-ufw-portscan
		iifname "tun79" counter packets 0 bytes 0 accept
		counter packets 23104453 bytes 26061178033 jump ufw-before-logging-input
		counter packets 23104453 bytes 26061178033 jump ufw-before-input
		counter packets 172659 bytes 48722909 jump ufw-after-input
		counter packets 28356 bytes 1896988 jump ufw-after-logging-input
		counter packets 28356 bytes 1896988 jump ufw-reject-input
		counter packets 28356 bytes 1896988 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 0 bytes 0 accept
		counter packets 23212328 bytes 26879899680 jump ufw-before-logging-output
		counter packets 23212328 bytes 26879899680 jump ufw-before-output
		counter packets 378624 bytes 53254753 jump ufw-after-output
		counter packets 378624 bytes 53254753 jump ufw-after-logging-output
		counter packets 378624 bytes 53254753 jump ufw-reject-output
		counter packets 378624 bytes 53254753 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 27160773 bytes 23306302292 jump DOCKER-USER
		counter packets 27160774 bytes 23306302352 jump DOCKER-FORWARD
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 431666 bytes 525907175 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 586975 bytes 304869699 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 117 bytes 27815 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 117 bytes 27815 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 144303 bytes 46825921 drop
	}
	...
	chain f2b-ufw-portscan {
		ip saddr 85.217.140.13 counter packets 55 bytes 2860 reject
```
...
remnanode                    remnawave/node:3.4.1             Up 39 hours   
mtproto-telegram             telegrammessenger/proxy:latest   Up 39 hours   0.0.0.0:9443->443/tcp, [::]:9443->443/tcp
amnezia-awg2                 amnezia-awg2                     Up 39 hours   0.0.0.0:39425->39425/udp, [::]:39425->39425/udp
signal-tls-proxy-certbot-1   certbot/certbot                  Up 39 hours   80/tcp, 443/tcp
```
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 49 | 2026-09-15 18:14:48 |
