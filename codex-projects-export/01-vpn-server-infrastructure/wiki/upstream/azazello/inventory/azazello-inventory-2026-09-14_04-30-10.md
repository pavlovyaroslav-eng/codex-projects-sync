...
Generated: 2026-09-14T04:30:10+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:10 up  1:17,  0 user,  load average: 0.79, 0.33, 0.26
```
...
udp   UNCONN 0      0                  *:55135            *:*    users:(("rw-core",pid=3110,fd=16))                        
...
udp   UNCONN 0      0                  *:37976            *:*    users:(("rw-core",pid=3110,fd=12))                        
...
tcp   LISTEN 0      100          0.0.0.0:25         0.0.0.0:*    users:(("smtpd",pid=13742,fd=6),("master",pid=1895,fd=13))
...
-A f2b-ufw-portscan -s 89.10.217.121/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 81.45.66.148/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 62.210.142.170/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 193.148.56.66/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 162.55.220.65/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 148.113.198.174/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 143.42.238.18/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 113.57.146.66/32 -j REJECT --reject-with icmp-port-unreachable
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 241 bytes 59118 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 240 bytes 17940 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		...
		ct state related,established counter packets 90435 bytes 100616254 accept
		ct state invalid counter packets 11 bytes 4499 jump ufw-logging-deny
		ct state invalid counter packets 11 bytes 4499 drop
		...
		ip protocol icmp icmp type echo-request counter packets 11 bytes 376 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 9294 bytes 1999409 jump ufw-not-local
		...
		counter packets 9294 bytes 1999409 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 80 bytes 5920 accept
		ct state related,established counter packets 89725 bytes 102819229 accept
		counter packets 3054 bytes 603137 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		...
		tcp dport 445 counter packets 4 bytes 184 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 4113 bytes 1349064 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 694 bytes 218668 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 240 bytes 18296 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 240 bytes 18972 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 976 bytes 58560 accept
		ip protocol udp ct state new counter packets 2046 bytes 538225 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 45500 bytes 83926828 jump f2b-ufw-portscan
		iifname "tun79" counter packets 0 bytes 0 accept
		counter packets 99831 bytes 102626458 jump ufw-before-logging-input
		counter packets 99831 bytes 102626458 jump ufw-before-input
		counter packets 5576 bytes 1627723 jump ufw-after-input
		counter packets 765 bytes 59807 jump ufw-after-logging-input
		counter packets 765 bytes 59807 jump ufw-reject-input
		counter packets 765 bytes 59807 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 0 bytes 0 accept
		counter packets 92859 bytes 103428286 jump ufw-before-logging-output
		counter packets 92859 bytes 103428286 jump ufw-before-output
		counter packets 3054 bytes 603137 jump ufw-after-output
		counter packets 3054 bytes 603137 jump ufw-after-logging-output
		counter packets 3054 bytes 603137 jump ufw-reject-output
		counter packets 3054 bytes 603137 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 350897 bytes 42812317 jump DOCKER-USER
		counter packets 350898 bytes 42812377 jump DOCKER-FORWARD
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 8322 bytes 7137907 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 13823 bytes 4702984 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 11 bytes 4499 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 11 bytes 4499 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 4811 bytes 1567916 drop
	}
	...
	chain ufw-user-input {
		...
		tcp dport 25 counter packets 3 bytes 140 accept
		...
		ip saddr 185.71.196.110 udp dport 24443 counter packets 118 bytes 154344 accept
	}
	...
	chain f2b-ufw-portscan {
		ip saddr 172.217.118.4 counter packets 14 bytes 12929 reject
```
...
remnanode                    remnawave/node:3.4.1             Up About an hour   
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 49 | 2026-09-14 04:29:52 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-09-13 21:22:03 |
...
total 428K
drwxr-xr-x  2 root root   4.0K Sep 14 03:10 .
