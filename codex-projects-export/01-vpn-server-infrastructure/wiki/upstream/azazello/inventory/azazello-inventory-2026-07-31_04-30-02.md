...
Generated: 2026-07-31T04:30:02+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:02 up 54 days, 14:09,  0 user,  load average: 0.15, 0.28, 0.41
```
...
-A f2b-ufw-portscan -s 2.19.126.147/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 208.87.242.93/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 31.13.72.48/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 78.156.235.161/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 104.83.5.136/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 185.201.2.7/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 91.198.230.5/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 172.217.118.4/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 151.101.129.91/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 95.213.56.4/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 142.250.147.108/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 8.218.68.146/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 71.204.84.55/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 45.76.107.103/32 -j REJECT --reject-with icmp-port-unreachable
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 55427 bytes 22209981 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 55427 bytes 3538873 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		iifname "lo" counter packets 34344139 bytes 18354655419 accept
		ct state related,established counter packets 367744040 bytes 256397049373 accept
		ct state invalid counter packets 31942 bytes 2548161 jump ufw-logging-deny
		ct state invalid counter packets 31942 bytes 2548161 drop
		...
		ip protocol icmp icmp type echo-request counter packets 51952 bytes 2991520 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 6433246 bytes 2531074281 jump ufw-not-local
		...
		counter packets 6433246 bytes 2531074281 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 34344510 bytes 18354688493 accept
		ct state related,established counter packets 448031020 bytes 508755094259 accept
		counter packets 2634581 bytes 314864025 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		udp dport 137 counter packets 416 bytes 37082 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 3 bytes 2383 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 344 bytes 17700 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 2212 bytes 111268 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 2442693 bytes 801205348 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 3 bytes 2384 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 412879 bytes 124413676 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 55426 bytes 13767108 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 55427 bytes 4012362 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 458236 bytes 29026490 accept
		ip protocol udp ct state new counter packets 2122144 bytes 280525754 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 7983548 bytes 4895913837 jump f2b-ufw-portscan
		iifname "tun79" counter packets 1090750 bytes 1359553160 accept
		counter packets 782680794 bytes 643727300357 jump ufw-before-logging-input
		counter packets 782680794 bytes 643727300357 jump ufw-before-input
		counter packets 8615053 bytes 1863046459 jump ufw-after-input
		counter packets 3835235 bytes 314897298 jump ufw-after-logging-input
		counter packets 3835235 bytes 314897298 jump ufw-reject-input
		counter packets 3835235 bytes 314897298 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 31413 bytes 6024246 accept
		counter packets 803166861 bytes 805932573561 jump ufw-before-logging-output
		counter packets 803166861 bytes 805932573561 jump ufw-before-output
		counter packets 6138280 bytes 818391103 jump ufw-after-output
		counter packets 6138280 bytes 818391103 jump ufw-after-logging-output
		counter packets 6138280 bytes 818391103 jump ufw-reject-output
		counter packets 6138280 bytes 818391103 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 158722950 bytes 371336067588 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 190624699 bytes 42238211115 accept
		...
		counter packets 1035143865 bytes 1148934836652 jump DOCKER-USER
		counter packets 1035143865 bytes 1148934836652 jump DOCKER-FORWARD
	...
	chain DOCKER {
		ip daddr 172.29.172.2 iifname != "amn0" oifname "amn0" udp dport 39425 counter packets 34421 bytes 6683125 accept
		ip daddr 172.17.0.3 iifname != "docker0" oifname "docker0" tcp dport 443 counter packets 92928 bytes 5650503 accept
	...
	chain DOCKER-FORWARD {
		counter packets 1035143865 bytes 1148934836652 jump DOCKER-CT
		counter packets 518501998 bytes 571702719429 jump DOCKER-INTERNAL
		counter packets 518501998 bytes 571702719429 jump DOCKER-BRIDGE
		iifname "br-566ebeb0f4f1" counter packets 0 bytes 0 accept
		iifname "amn0" counter packets 395718449 bytes 526452867425 accept
		iifname "docker0" counter packets 122521003 bytes 45225347109 accept
	}

	chain DOCKER-BRIDGE {
		oifname "br-566ebeb0f4f1" counter packets 0 bytes 0 jump DOCKER
		oifname "amn0" counter packets 72507 bytes 12933018 jump DOCKER
		oifname "docker0" counter packets 189486 bytes 11540215 jump DOCKER
	}

	chain DOCKER-CT {
		oifname "br-566ebeb0f4f1" ct state related,established counter packets 0 bytes 0 accept
		oifname "amn0" ct state related,established counter packets 410049569 bytes 511211010102 accept
		oifname "docker0" ct state related,established counter packets 106592298 bytes 66021107121 accept
	}
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 9233 bytes 682901 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 9233 bytes 682901 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 2858550 bytes 925789841 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 3577522 bytes 1605442903 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 2855724 bytes 925631378 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 90737 bytes 5371366 accept
		udp dport 46539 counter packets 752404 bytes 1037845282 accept
		udp dport 39425 counter packets 402 bytes 38171 accept
		tcp dport 80 counter packets 106562 bytes 6335412 accept
		udp dport 1194 counter packets 146 bytes 9213 accept
		tcp dport 8443 counter packets 18422 bytes 1078506 accept
		...
		tcp dport 52000 counter packets 475 bytes 23413 accept
		udp dport 51820 counter packets 2 bytes 1600 accept
		tcp dport 25 counter packets 1246 bytes 69588 accept
...
mtproto-telegram             telegrammessenger/proxy:latest   Up 13 days   0.0.0.0:9443->443/tcp, [::]:9443->443/tcp
amnezia-awg2                 amnezia-awg2                     Up 13 days   0.0.0.0:39425->39425/udp, [::]:39425->39425/udp
signal-tls-proxy-certbot-1   certbot/certbot                  Up 13 days   80/tcp, 443/tcp
```
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 44 | 2026-07-31 04:29:54 |
...
| `/etc/x-ui` | directory | root:root | 755 | 4096 | 2026-07-31 04:29:43 |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 168K
drwxr-xr-x  2 root root   4.0K Jul 31 03:10 .
