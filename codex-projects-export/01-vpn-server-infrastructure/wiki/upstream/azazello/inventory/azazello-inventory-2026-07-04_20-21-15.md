...
Generated: 2026-07-04T20:21:15+00:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 20:21:15 up 28 days,  9:01,  1 user,  load average: 0.73, 0.88, 0.96
```
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 23645 bytes 5842878 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 23645 bytes 1676305 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		iifname "lo" counter packets 6581175 bytes 3940471326 accept
		ct state related,established counter packets 155440451 bytes 131024304108 accept
		ct state invalid counter packets 5674 bytes 372687 jump ufw-logging-deny
		ct state invalid counter packets 5674 bytes 372687 drop
		...
		ip protocol icmp icmp type echo-request counter packets 11779 bytes 682748 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 1231190 bytes 276458905 jump ufw-not-local
		...
		counter packets 1231190 bytes 276458905 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 6581179 bytes 3940471542 accept
		ct state related,established counter packets 163915237 bytes 190456235351 accept
		counter packets 1333340 bytes 184513416 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		...
		udp dport 67 counter packets 455939 bytes 149548638 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 76191 bytes 23189596 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 23645 bytes 1519657 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 23272 bytes 1946598 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 170113 bytes 11612296 accept
		ip protocol udp ct state new counter packets 1153236 bytes 171922027 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 44948982 bytes 43930442357 jump f2b-ufw-portscan
		iifname "tun79" counter packets 359119 bytes 446925567 accept
		counter packets 537345744 bytes 501681271377 jump ufw-before-logging-input
		counter packets 537345744 bytes 501681271377 jump ufw-before-input
		counter packets 4797239 bytes 924823841 jump ufw-after-input
		counter packets 2343222 bytes 129689543 jump ufw-after-logging-input
		counter packets 2343222 bytes 129689543 jump ufw-reject-input
		counter packets 2343222 bytes 129689543 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 10310 bytes 1966670 accept
		counter packets 489986506 bytes 473089147093 jump ufw-before-logging-output
		counter packets 489986506 bytes 473089147093 jump ufw-before-output
		counter packets 4837039 bytes 688040494 jump ufw-after-output
		counter packets 4837039 bytes 688040494 jump ufw-after-logging-output
		counter packets 4837039 bytes 688040494 jump ufw-reject-output
		counter packets 4837039 bytes 688040494 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 51064835 bytes 125935933702 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 61321104 bytes 11223225392 accept
		...
		counter packets 258171240 bytes 297894946472 jump DOCKER-USER
		counter packets 258171240 bytes 297894946472 jump DOCKER-FORWARD
	...
	chain DOCKER {
		ip daddr 172.17.0.3 iifname != "docker0" oifname "docker0" tcp dport 443 counter packets 2728 bytes 168996 accept
	...
	chain DOCKER-FORWARD {
		counter packets 258171240 bytes 297894946472 jump DOCKER-CT
		counter packets 129367708 bytes 151256541956 jump DOCKER-INTERNAL
		counter packets 129367708 bytes 151256541956 jump DOCKER-BRIDGE
		iifname "br-566ebeb0f4f1" counter packets 0 bytes 0 accept
		iifname "amn0" counter packets 127990465 bytes 150914224489 accept
		iifname "docker0" counter packets 1350662 bytes 338497593 accept
	}
	...
	chain DOCKER-BRIDGE {
		...
		oifname "docker0" counter packets 2728 bytes 168996 jump DOCKER
	}

	chain DOCKER-CT {
		oifname "br-566ebeb0f4f1" ct state related,established counter packets 0 bytes 0 accept
		oifname "amn0" ct state related,established counter packets 127757143 bytes 146272927832 accept
		oifname "docker0" ct state related,established counter packets 1046389 bytes 365476684 accept
	}
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 4901 bytes 326765 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 4901 bytes 326765 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 532749 bytes 172774978 drop
	}
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 41 | 2026-07-04 20:21:21 |
...
| `/etc/x-ui` | directory | root:root | 755 | 4096 | 2026-07-04 20:21:22 |
| `/etc/3x-ui` | missing | - | - | - | - |
