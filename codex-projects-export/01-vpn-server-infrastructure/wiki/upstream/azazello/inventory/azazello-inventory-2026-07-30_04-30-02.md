...
Generated: 2026-07-30T04:30:02+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:02 up 53 days, 14:09,  0 user,  load average: 0.68, 0.47, 0.40
```
...
-A f2b-ufw-portscan -s 153.75.247.172/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 45.95.201.227/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 185.107.193.173/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 173.194.10.167/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 45.131.151.100/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 51.159.105.145/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 45.131.49.73/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 87.229.16.30/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 44.192.201.131/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 59.28.34.72/32 -j REJECT --reject-with icmp-port-unreachable
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 51107 bytes 20677390 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 51107 bytes 3263428 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		iifname "lo" counter packets 33274038 bytes 17778178558 accept
		ct state related,established counter packets 365342573 bytes 254653500016 accept
		ct state invalid counter packets 31246 bytes 2488847 jump ufw-logging-deny
		ct state invalid counter packets 31246 bytes 2488847 drop
		...
		ip protocol icmp icmp type echo-request counter packets 51428 bytes 2960449 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 6216112 bytes 2426782300 jump ufw-not-local
		...
		counter packets 6216112 bytes 2426782300 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 33274409 bytes 17778211632 accept
		ct state related,established counter packets 445759667 bytes 507417507348 accept
		counter packets 2603751 bytes 311865694 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		udp dport 137 counter packets 399 bytes 35756 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 3 bytes 2383 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 335 bytes 17256 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 2148 bytes 108088 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 2367026 bytes 776386572 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 3 bytes 2384 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 400311 bytes 120622095 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 51107 bytes 13514626 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 51107 bytes 3691618 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 451243 bytes 28586928 accept
		ip protocol udp ct state new counter packets 2098992 bytes 278064183 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 6545414 bytes 4123210413 jump f2b-ufw-portscan
		iifname "tun79" counter packets 1090750 bytes 1359553160 accept
		counter packets 778990872 bytes 641302891773 jump ufw-before-logging-input
		counter packets 778990872 bytes 641302891773 jump ufw-before-input
		counter packets 8474204 bytes 1831540025 jump ufw-after-input
		counter packets 3782711 bytes 312006171 jump ufw-after-logging-input
		counter packets 3782711 bytes 312006171 jump ufw-reject-input
		counter packets 3782711 bytes 312006171 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 31413 bytes 6024246 accept
		counter packets 799794577 bytes 804015511458 jump ufw-before-logging-output
		counter packets 799794577 bytes 804015511458 jump ufw-before-output
		counter packets 6107450 bytes 815392772 jump ufw-after-output
		counter packets 6107450 bytes 815392772 jump ufw-after-logging-output
		counter packets 6107450 bytes 815392772 jump ufw-reject-output
		counter packets 6107450 bytes 815392772 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 158506476 bytes 371068667837 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 190329723 bytes 42140572236 accept
		...
		counter packets 985511857 bytes 1089413989749 jump DOCKER-USER
		counter packets 985511857 bytes 1089413989749 jump DOCKER-FORWARD
	...
	chain DOCKER {
		ip daddr 172.29.172.2 iifname != "amn0" oifname "amn0" udp dport 39425 counter packets 32485 bytes 6366009 accept
		ip daddr 172.17.0.3 iifname != "docker0" oifname "docker0" tcp dport 443 counter packets 87730 bytes 5338154 accept
	...
	chain DOCKER-FORWARD {
		counter packets 985511857 bytes 1089413989749 jump DOCKER-CT
		counter packets 493720051 bytes 541731095372 jump DOCKER-INTERNAL
		counter packets 493720051 bytes 541731095372 jump DOCKER-BRIDGE
		iifname "br-566ebeb0f4f1" counter packets 0 bytes 0 accept
		iifname "amn0" counter packets 374796782 bytes 497182967948 accept
		iifname "docker0" counter packets 118667857 bytes 44524251994 accept
	}

	chain DOCKER-BRIDGE {
		oifname "br-566ebeb0f4f1" counter packets 0 bytes 0 jump DOCKER
		oifname "amn0" counter packets 70571 bytes 12615902 jump DOCKER
		oifname "docker0" counter packets 184288 bytes 11227866 jump DOCKER
	}

	chain DOCKER-CT {
		oifname "br-566ebeb0f4f1" ct state related,established counter packets 0 bytes 0 accept
		oifname "amn0" ct state related,established counter packets 388354750 bytes 482848913640 accept
		oifname "docker0" ct state related,established counter packets 103437056 bytes 64833980737 accept
	}
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 8565 bytes 624743 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 8565 bytes 624743 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 2770225 bytes 897174534 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 3448623 bytes 1529761279 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 2767489 bytes 897021021 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 84098 bytes 4975634 accept
		udp dport 46539 counter packets 700865 bytes 966531438 accept
		udp dport 39425 counter packets 396 bytes 37542 accept
		tcp dport 80 counter packets 93333 bytes 5544714 accept
		udp dport 1194 counter packets 124 bytes 8159 accept
		tcp dport 8443 counter packets 13861 bytes 810682 accept
		...
		tcp dport 52000 counter packets 388 bytes 19359 accept
		udp dport 51820 counter packets 2 bytes 1600 accept
		tcp dport 25 counter packets 1044 bytes 57876 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 44 | 2026-07-30 04:30:17 |
...
| `/etc/x-ui` | directory | root:root | 755 | 4096 | 2026-07-30 04:30:19 |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 164K
drwxr-xr-x  2 root root   4.0K Jul 30 03:10 .
