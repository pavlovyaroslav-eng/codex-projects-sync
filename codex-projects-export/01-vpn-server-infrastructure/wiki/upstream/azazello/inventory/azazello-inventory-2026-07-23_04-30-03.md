...
Generated: 2026-07-23T04:30:03+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:03 up 46 days, 14:09,  0 user,  load average: 0.20, 0.19, 0.25
```
...
-A f2b-ufw-portscan -s 143.20.58.7/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 216.180.246.27/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 85.217.140.51/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 124.232.143.99/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 2.16.16.156/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 2.23.97.240/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 188.128.76.129/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 109.95.79.207/32 -j REJECT --reject-with icmp-port-unreachable
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 20867 bytes 8330356 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 20867 bytes 1317698 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		iifname "lo" counter packets 25789045 bytes 13767984803 accept
		ct state related,established counter packets 350045113 bytes 241229612629 accept
		ct state invalid counter packets 26122 bytes 2100971 jump ufw-logging-deny
		ct state invalid counter packets 26122 bytes 2100971 drop
		...
		ip protocol icmp icmp type echo-request counter packets 47124 bytes 2712485 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 4416145 bytes 1495613862 jump ufw-not-local
		...
		counter packets 4416145 bytes 1495613862 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 25789393 bytes 13768016037 accept
		ct state related,established counter packets 431038954 bytes 497620307381 accept
		counter packets 2287832 bytes 282913286 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		udp dport 137 counter packets 330 bytes 30158 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 3 bytes 2383 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 275 bytes 14380 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 1705 bytes 86376 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 1836832 bytes 602482940 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 3 bytes 2384 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 308357 bytes 93433258 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 20867 bytes 4630600 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 20867 bytes 1461146 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 387761 bytes 24778008 accept
		ip protocol udp ct state new counter packets 1855623 bytes 253805827 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 13392441 bytes 12234263278 jump f2b-ufw-portscan
		iifname "tun79" counter packets 1090750 bytes 1359553160 accept
		counter packets 754399024 bytes 622937006353 jump ufw-before-logging-input
		counter packets 754399024 bytes 622937006353 jump ufw-before-input
		counter packets 7254379 bytes 1525911417 jump ufw-after-input
		counter packets 3185606 bytes 207500218 jump ufw-after-logging-input
		counter packets 3185606 bytes 207500218 jump ufw-reject-input
		counter packets 3185606 bytes 207500218 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 31413 bytes 6024246 accept
		counter packets 777272929 bytes 790179163488 jump ufw-before-logging-output
		counter packets 777272929 bytes 790179163488 jump ufw-before-output
		counter packets 5791531 bytes 786440364 jump ufw-after-output
		counter packets 5791531 bytes 786440364 jump ufw-after-logging-output
		counter packets 5791531 bytes 786440364 jump ufw-reject-output
		counter packets 5791531 bytes 786440364 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 156741298 bytes 368952623400 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 187920707 bytes 41180650383 accept
		...
		counter packets 680714151 bytes 727192193009 jump DOCKER-USER
		counter packets 680714151 bytes 727192193009 jump DOCKER-FORWARD
	...
	chain DOCKER {
		ip daddr 172.29.172.2 iifname != "amn0" oifname "amn0" udp dport 39425 counter packets 17674 bytes 3407639 accept
		ip daddr 172.17.0.3 iifname != "docker0" oifname "docker0" tcp dport 443 counter packets 35226 bytes 2126081 accept
	...
	chain DOCKER-FORWARD {
		counter packets 680714151 bytes 727192193009 jump DOCKER-CT
		counter packets 341876383 bytes 360430328320 jump DOCKER-INTERNAL
		counter packets 341876383 bytes 360430328320 jump DOCKER-BRIDGE
		iifname "br-566ebeb0f4f1" counter packets 0 bytes 0 accept
		iifname "amn0" counter packets 253322001 bytes 325045406512 accept
		iifname "docker0" counter packets 88366286 bytes 35367216873 accept
	}

	chain DOCKER-BRIDGE {
		oifname "br-566ebeb0f4f1" counter packets 0 bytes 0 jump DOCKER
		oifname "amn0" counter packets 55760 bytes 9657532 jump DOCKER
		oifname "docker0" counter packets 131784 bytes 8015793 jump DOCKER
	}

	chain DOCKER-CT {
		oifname "br-566ebeb0f4f1" ct state related,established counter packets 0 bytes 0 accept
		oifname "amn0" ct state related,established counter packets 261567440 bytes 315744177930 accept
		oifname "docker0" ct state related,established counter packets 77270328 bytes 51017686759 accept
	}
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 3905 bytes 266821 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 3905 bytes 266821 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 2147505 bytes 696051879 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 2270819 bytes 799686696 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 2145326 bytes 695927166 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 10383 bytes 611701 accept
		udp dport 46539 counter packets 251196 bytes 348712697 accept
		udp dport 39425 counter packets 163 bytes 15423 accept
		tcp dport 80 counter packets 49297 bytes 2942718 accept
		udp dport 1194 counter packets 56 bytes 4839 accept
		tcp dport 8443 counter packets 2130 bytes 122590 accept
		...
		tcp dport 52000 counter packets 206 bytes 10384 accept
		udp dport 51820 counter packets 0 bytes 0 accept
		tcp dport 25 counter packets 542 bytes 30020 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 42 | 2026-07-23 04:30:13 |
...
| `/etc/x-ui` | directory | root:root | 755 | 4096 | 2026-07-23 04:29:43 |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 120K
drwxr-xr-x  2 root root   4.0K Jul 23 03:10 .
