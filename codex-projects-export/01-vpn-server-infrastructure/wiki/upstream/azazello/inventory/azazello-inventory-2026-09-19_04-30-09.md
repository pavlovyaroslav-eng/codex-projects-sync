...
Generated: 2026-09-19T04:30:09+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:10 up 5 days,  1:17,  0 user,  load average: 1.42, 0.41, 0.25
```
...
udp   UNCONN 0      0                  *:35648            *:*    users:(("rw-core",pid=3110,fd=11))                         
...
-A f2b-ufw-portscan -s 34.54.122.72/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 141.98.50.74/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 78.128.114.166/32 -j REJECT --reject-with icmp-port-unreachable
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 21841 bytes 4578795 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 21841 bytes 2034566 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		...
		ct state related,established counter packets 78146611 bytes 82570385838 accept
		ct state invalid counter packets 298 bytes 46775 jump ufw-logging-deny
		ct state invalid counter packets 298 bytes 46775 drop
		...
		ip protocol icmp icmp type echo-request counter packets 2694 bytes 171349 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 852082 bytes 165327212 jump ufw-not-local
		...
		counter packets 852082 bytes 165327212 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 1922 bytes 317445 accept
		ct state related,established counter packets 82530630 bytes 85665874256 accept
		counter packets 1083579 bytes 168812612 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		udp dport 137 counter packets 47 bytes 3666 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 5 bytes 1010 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 40 bytes 1876 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 582 bytes 26492 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 366867 bytes 120332096 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 5 bytes 140 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 62613 bytes 19445186 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 21840 bytes 1330987 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 21840 bytes 2018913 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 477763 bytes 28665748 accept
		ip protocol udp ct state new counter packets 604138 bytes 139985640 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 17575173 bytes 45673901298 jump f2b-ufw-portscan
		iifname "tun79" counter packets 0 bytes 0 accept
		counter packets 79003607 bytes 82736248619 jump ufw-before-logging-input
		counter packets 79003607 bytes 82736248619 jump ufw-before-input
		counter packets 572619 bytes 148301633 jump ufw-after-input
		counter packets 142460 bytes 8491167 jump ufw-after-logging-input
		counter packets 142460 bytes 8491167 jump ufw-reject-input
		counter packets 142460 bytes 8491167 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 0 bytes 0 accept
		counter packets 83616131 bytes 85835004313 jump ufw-before-logging-output
		counter packets 83616131 bytes 85835004313 jump ufw-before-output
		counter packets 1083579 bytes 168812612 jump ufw-after-output
		counter packets 1083579 bytes 168812612 jump ufw-after-logging-output
		counter packets 1083579 bytes 168812612 jump ufw-reject-output
		counter packets 1083579 bytes 168812612 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 101125601 bytes 96021838146 jump DOCKER-USER
		counter packets 101125602 bytes 96021838206 jump DOCKER-FORWARD
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 1134647 bytes 1399081924 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 1553563 bytes 765198741 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 298 bytes 46775 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 298 bytes 46775 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 430159 bytes 139810466 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 422604 bytes 25551056 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 429478 bytes 139776156 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 163721 bytes 9622716 accept
		udp dport 46539 counter packets 60 bytes 78960 accept
		udp dport 39425 counter packets 158 bytes 14738 accept
		tcp dport 80 counter packets 95504 bytes 5625826 accept
		udp dport 1194 counter packets 47 bytes 2523 accept
		tcp dport 8443 counter packets 18175 bytes 1066706 accept
		udp dport 21194 counter packets 20 bytes 3120 accept
		iifname "tun79" counter packets 0 bytes 0 accept
		udp dport 33328 counter packets 0 bytes 0 accept
		tcp dport 52000 counter packets 103 bytes 5824 accept
		udp dport 51820 counter packets 1 bytes 214 accept
		tcp dport 25 counter packets 1212 bytes 69296 accept
	...
	chain f2b-ufw-portscan {
		ip saddr 85.217.140.13 counter packets 107 bytes 5564 reject
```
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 49 | 2026-09-19 04:30:07 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-09-19 01:14:39 |
...
total 448K
drwxr-xr-x  2 root root   4.0K Sep 19 03:10 .
