...
Generated: 2026-09-25T04:30:09+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:09 up 11 days,  1:17,  0 user,  load average: 0.37, 0.25, 0.27
```
...
udp   UNCONN 0      0                  *:43085            *:*    users:(("rw-core",pid=3110,fd=26))                         
udp   UNCONN 0      0                  *:32870            *:*    users:(("rw-core",pid=3110,fd=28))                         
udp   UNCONN 0      0                  *:41080            *:*    users:(("rw-core",pid=3110,fd=22))                         
udp   UNCONN 0      0              [::1]:8388          [::]:*    users:(("ss-server",pid=1090,fd=7))                        
udp   UNCONN 0      0                  *:39311            *:*    users:(("rw-core",pid=3110,fd=23))                         
udp   UNCONN 0      0               [::]:39425         [::]:*    users:(("docker-proxy",pid=2667,fd=7))                     
udp   UNCONN 0      0               [::]:51820         [::]:*                                                               
udp   UNCONN 0      0                  *:50118            *:*    users:(("rw-core",pid=3110,fd=27))                         
udp   UNCONN 0      0                  *:46288            *:*    users:(("rw-core",pid=3110,fd=17))                         
udp   UNCONN 0      0                  *:42537            *:*    users:(("rw-core",pid=3110,fd=12))                         
...
-A f2b-ufw-portscan -s 112.124.56.43/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 45.145.42.42/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 142.250.109.95/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 85.217.140.49/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 87.236.16.58/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 23.139.36.16/32 -j REJECT --reject-with icmp-port-unreachable
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 47761 bytes 10286067 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 47761 bytes 4415644 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		...
		ct state related,established counter packets 180565303 bytes 193586299938 accept
		ct state invalid counter packets 599 bytes 69790 jump ufw-logging-deny
		ct state invalid counter packets 599 bytes 69790 drop
		...
		ip protocol icmp icmp type echo-request counter packets 5687 bytes 343899 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 1750756 bytes 326635076 jump ufw-not-local
		...
		counter packets 1750756 bytes 326635076 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 1932 bytes 318825 accept
		ct state related,established counter packets 185983687 bytes 200427278521 accept
		counter packets 2656578 bytes 416767652 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		udp dport 137 counter packets 136 bytes 10824 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 11 bytes 2222 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 88 bytes 4096 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 1093 bytes 50556 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 676146 bytes 221775272 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 11 bytes 308 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 122352 bytes 40148647 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 47760 bytes 2909958 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 47760 bytes 4403159 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 1202401 bytes 72145060 accept
		ip protocol udp ct state new counter packets 1450484 bytes 344262151 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 11470624 bytes 30267970958 jump f2b-ufw-portscan
		iifname "tun79" counter packets 0 bytes 0 accept
		counter packets 182324277 bytes 193913667528 jump ufw-before-logging-input
		counter packets 182324277 bytes 193913667528 jump ufw-before-input
		counter packets 1111954 bytes 288424072 jump ufw-after-input
		counter packets 312117 bytes 26432147 jump ufw-after-logging-input
		counter packets 312117 bytes 26432147 jump ufw-reject-input
		counter packets 312117 bytes 26432147 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 0 bytes 0 accept
		counter packets 188642197 bytes 200844364998 jump ufw-before-logging-output
		counter packets 188642197 bytes 200844364998 jump ufw-before-output
		counter packets 2656578 bytes 416767652 jump ufw-after-output
		counter packets 2656578 bytes 416767652 jump ufw-after-logging-output
		counter packets 2656578 bytes 416767652 jump ufw-reject-output
		counter packets 2656578 bytes 416767652 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 246264790 bytes 250748063010 jump DOCKER-USER
		counter packets 246264791 bytes 250748063070 jump DOCKER-FORWARD
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 2855127 bytes 3549589104 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 3891342 bytes 1967553087 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 599 bytes 69790 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 599 bytes 69790 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 799837 bytes 261991925 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 952248 bytes 64712295 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 798508 bytes 261922781 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 353446 bytes 20779440 accept
		udp dport 46539 counter packets 64 bytes 83171 accept
		udp dport 39425 counter packets 181 bytes 17057 accept
		tcp dport 80 counter packets 175448 bytes 10335548 accept
		udp dport 1194 counter packets 99 bytes 4884 accept
		tcp dport 8443 counter packets 106872 bytes 6293725 accept
		udp dport 21194 counter packets 24 bytes 3728 accept
		iifname "tun79" counter packets 0 bytes 0 accept
		udp dport 33328 counter packets 1 bytes 389 accept
		tcp dport 52000 counter packets 230 bytes 12576 accept
		udp dport 51820 counter packets 1 bytes 214 accept
		tcp dport 25 counter packets 1927 bytes 109348 accept
		...
		ip saddr 185.71.196.110 udp dport 24443 counter packets 433 bytes 566364 accept
	}
	...
	chain DOCKER {
		ip daddr 172.29.172.2 iifname != "amn0" oifname "amn0" udp dport 39425 counter packets 16818 bytes 1788815 accept
```
...
remnanode                    remnawave/node:3.4.1             Up 11 days   
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 47 | 2026-09-25 04:29:51 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-09-24 23:05:09 |
...
total 484K
drwxr-xr-x  2 root root   4.0K Sep 25 03:10 .
