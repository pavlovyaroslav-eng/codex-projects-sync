...
Generated: 2026-09-03T04:30:03+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:03 up 32 days,  1:17,  0 user,  load average: 0.82, 0.53, 0.39
```
...
tcp   LISTEN 0      4096         0.0.0.0:52000      0.0.0.0:*    users:(("sshd",pid=2803004,fd=3),("systemd",pid=1,fd=147))   
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 138481 bytes 49613395 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 138481 bytes 8513933 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		iifname "lo" counter packets 34320843 bytes 18113012716 accept
		ct state related,established counter packets 265886107 bytes 298602621856 accept
		ct state invalid counter packets 23999 bytes 1407478 jump ufw-logging-deny
		ct state invalid counter packets 23999 bytes 1407478 drop
		...
		ip protocol icmp icmp type echo-request counter packets 21980 bytes 1372615 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 7128981 bytes 4144112977 jump ufw-not-local
		...
		counter packets 7128981 bytes 4144112977 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 34320843 bytes 18113012716 accept
		ct state related,established counter packets 219085928 bytes 179056642651 accept
		counter packets 937340 bytes 92088711 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		udp dport 137 counter packets 556 bytes 46654 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 731 bytes 167198 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 294 bytes 13828 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 1972 bytes 96788 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 2399817 bytes 787138702 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 2 bytes 56 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 406710 bytes 120496333 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 138455 bytes 8595136 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 138480 bytes 9818052 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 232322 bytes 14947575 accept
		ip protocol udp ct state new counter packets 678637 bytes 74731925 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp tcp dport 52000 counter packets 238 bytes 65518 jump f2b-sshd
		ip protocol tcp counter packets 490976 bytes 283756819 jump f2b-ufw-portscan
		iifname "tun79" counter packets 0 bytes 0 accept
		counter packets 307381910 bytes 320862527642 jump ufw-before-logging-input
		counter packets 307381910 bytes 320862527642 jump ufw-before-input
		counter packets 3791945 bytes 982126773 jump ufw-after-input
		counter packets 981863 bytes 74167214 jump ufw-after-logging-input
		counter packets 981863 bytes 74167214 jump ufw-reject-input
		counter packets 981863 bytes 74167214 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 0 bytes 0 accept
		counter packets 254344111 bytes 197261744078 jump ufw-before-logging-output
		counter packets 254344111 bytes 197261744078 jump ufw-before-output
		counter packets 937340 bytes 92088711 jump ufw-after-output
		counter packets 937340 bytes 92088711 jump ufw-after-logging-output
		counter packets 937340 bytes 92088711 jump ufw-reject-output
		counter packets 937340 bytes 92088711 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 1539660225 bytes 1883664868003 jump DOCKER-USER
		counter packets 1539660232 bytes 1883664869217 jump DOCKER-FORWARD
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 7320164 bytes 9032386882 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 9198606 bytes 4082159601 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 17644 bytes 1112876 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 17644 bytes 1112876 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 2810082 bytes 907959559 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 4321443 bytes 3236285307 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 2807538 bytes 907827670 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 501346 bytes 24429053 accept
		udp dport 46539 counter packets 2235036 bytes 3107938917 accept
		udp dport 39425 counter packets 772 bytes 76950 accept
		tcp dport 80 counter packets 491493 bytes 23193194 accept
		udp dport 1194 counter packets 320 bytes 13424 accept
		tcp dport 8443 counter packets 102715 bytes 6035995 accept
		udp dport 21194 counter packets 59 bytes 9012 accept
		iifname "tun79" counter packets 0 bytes 0 accept
		udp dport 33328 counter packets 1 bytes 438 accept
		tcp dport 52000 counter packets 856 bytes 44409 accept
		udp dport 51820 counter packets 1 bytes 436 accept
		tcp dport 25 counter packets 4432 bytes 244076 accept
	...
	chain DOCKER {
		ip daddr 172.29.172.2 iifname != "amn0" oifname "amn0" udp dport 39425 counter packets 25467 bytes 4534764 accept
		ip daddr 172.17.0.2 iifname != "docker0" oifname "docker0" tcp dport 443 counter packets 352219 bytes 21746686 accept
```
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 46 | 2026-09-03 04:29:56 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-09-02 14:42:56 |
| `/root/cert` | directory | root:root | 755 | 4096 | 2025-03-17 17:20:58 |
| `/etc/x-ui` | directory | root:root | 755 | 4096 | 2026-09-03 04:30:10 |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 364K
drwxr-xr-x  2 root root   4.0K Sep  3 03:10 .
drwxr-xr-x 18 root syslog 4.0K Sep  2 20:51 ..
