...
Generated: 2026-08-03T04:30:02+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:02 up 1 day,  1:17,  0 user,  load average: 0.17, 0.46, 0.55
```
...
-A f2b-ufw-portscan -s 172.182.211.18/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 177.10.12.176/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 157.240.0.61/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 157.240.30.54/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 94.154.43.214/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 85.217.140.49/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 45.33.109.18/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 172.170.180.133/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 157.230.149.167/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 143.198.23.100/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 143.198.129.238/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 85.217.140.39/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 23.217.48.55/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 104.234.6.243/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 172.203.207.242/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 74.235.90.81/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 212.90.203.171/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 195.14.107.145/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 195.14.106.70/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 52.225.97.22/32 -j REJECT --reject-with icmp-port-unreachable
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 4561 bytes 1898000 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 4561 bytes 284158 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		iifname "lo" counter packets 1102975 bytes 537529684 accept
		ct state related,established counter packets 3013939 bytes 2033199797 accept
		ct state invalid counter packets 899 bytes 58626 jump ufw-logging-deny
		ct state invalid counter packets 899 bytes 58626 drop
		...
		ip protocol icmp icmp type echo-request counter packets 855 bytes 52671 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 203831 bytes 120351900 jump ufw-not-local
		...
		counter packets 203831 bytes 120351900 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 1102975 bytes 537529684 accept
		ct state related,established counter packets 2839149 bytes 2281021228 accept
		counter packets 35948 bytes 4685719 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		udp dport 137 counter packets 144 bytes 12150 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 25 bytes 1116 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 71 bytes 3256 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 80366 bytes 26360048 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 13314 bytes 4049372 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 4560 bytes 272026 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 4560 bytes 320073 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 6692 bytes 401520 accept
		ip protocol udp ct state new counter packets 28254 bytes 4204161 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 1713783 bytes 1009033623 jump f2b-ufw-portscan
		iifname "tun79" counter packets 0 bytes 0 accept
		counter packets 4322499 bytes 2691192678 jump ufw-before-logging-input
		counter packets 4322499 bytes 2691192678 jump ufw-before-input
		counter packets 123886 bytes 32033878 jump ufw-after-input
		counter packets 29966 bytes 1607936 jump ufw-after-logging-input
		counter packets 29966 bytes 1607936 jump ufw-reject-input
		counter packets 29966 bytes 1607936 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 0 bytes 0 accept
		counter packets 3978072 bytes 2823236631 jump ufw-before-logging-output
		counter packets 3978072 bytes 2823236631 jump ufw-before-output
		counter packets 35948 bytes 4685719 jump ufw-after-output
		counter packets 35948 bytes 4685719 jump ufw-after-logging-output
		counter packets 35948 bytes 4685719 jump ufw-reject-output
		counter packets 35948 bytes 4685719 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 52437012 bytes 58850082737 jump DOCKER-USER
		counter packets 52437019 bytes 58850083951 jump DOCKER-FORWARD
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 294606 bytes 373826670 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 397781 bytes 169573015 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 879 bytes 57652 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 879 bytes 57652 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 93920 bytes 30425942 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 110028 bytes 89931968 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 93803 bytes 30419932 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 7735 bytes 456750 accept
		udp dport 46539 counter packets 64089 bytes 87383898 accept
		udp dport 39425 counter packets 59 bytes 5599 accept
		tcp dport 80 counter packets 1090 bytes 62255 accept
		udp dport 1194 counter packets 30 bytes 992 accept
		tcp dport 8443 counter packets 6519 bytes 382508 accept
		udp dport 21194 counter packets 21 bytes 3276 accept
		iifname "tun79" counter packets 0 bytes 0 accept
		udp dport 33328 counter packets 0 bytes 0 accept
		tcp dport 52000 counter packets 14 bytes 776 accept
		udp dport 51820 counter packets 0 bytes 0 accept
		tcp dport 25 counter packets 383 bytes 21668 accept
	...
	chain f2b-ufw-portscan {
		ip saddr 45.225.135.171 counter packets 66 bytes 2640 reject
		ip saddr 2.22.113.123 counter packets 9 bytes 1831 reject
		ip saddr 5.83.129.207 counter packets 141 bytes 5640 reject
```
...
mtproto-telegram             telegrammessenger/proxy:latest   Up 25 hours   0.0.0.0:9443->443/tcp, [::]:9443->443/tcp
amnezia-awg2                 amnezia-awg2                     Up 25 hours   0.0.0.0:39425->39425/udp, [::]:39425->39425/udp
signal-tls-proxy-certbot-1   certbot/certbot                  Up 25 hours   80/tcp, 443/tcp
```
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 45 | 2026-08-03 04:29:52 |
...
| `/etc/x-ui` | directory | root:root | 755 | 4096 | 2026-08-03 04:30:19 |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 184K
drwxr-xr-x  2 root root   4.0K Aug  3 03:10 .
