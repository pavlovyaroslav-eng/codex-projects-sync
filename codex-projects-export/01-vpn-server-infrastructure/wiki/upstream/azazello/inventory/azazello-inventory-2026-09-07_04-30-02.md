...
Generated: 2026-09-07T04:30:02+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:02 up 36 days,  1:17,  0 user,  load average: 0.27, 0.26, 0.29
```
...
udp   UNCONN 0      0                  *:56151            *:*    users:(("rw-core",pid=2921938,fd=16))                        
udp   UNCONN 0      0                  *:46186            *:*    users:(("rw-core",pid=2921938,fd=25))                        
...
-A f2b-sshd -j RETURN
-A f2b-ufw-portscan -s 64.124.145.4/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 206.223.236.65/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 160.187.146.74/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 85.93.10.7/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 85.217.149.67/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 47.94.165.91/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 121.41.167.96/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 47.95.194.74/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 47.95.195.112/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 121.40.47.185/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 89.144.35.106/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 86.51.83.253/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 112.124.57.196/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 47.95.209.196/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 158.69.127.181/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 86.51.82.252/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 193.148.56.10/32 -j REJECT --reject-with icmp-port-unreachable
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 16652 bytes 4371298 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 16652 bytes 1271114 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		...
		ct state related,established counter packets 41995998 bytes 40082431842 accept
		ct state invalid counter packets 354 bytes 29156 jump ufw-logging-deny
		ct state invalid counter packets 354 bytes 29156 drop
		...
		ip protocol icmp icmp type echo-request counter packets 1927 bytes 110258 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 780525 bytes 279653401 jump ufw-not-local
		...
		counter packets 780525 bytes 279653401 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 203166 bytes 86146464 accept
		ct state related,established counter packets 45152021 bytes 40332212410 accept
		counter packets 774694 bytes 88627200 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		udp dport 137 counter packets 62 bytes 5052 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 23 bytes 1076 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 210 bytes 10368 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 290275 bytes 95209900 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 1 bytes 28 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 48344 bytes 14533941 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 16651 bytes 1003558 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 16652 bytes 1272292 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 363451 bytes 21807060 accept
		ip protocol udp ct state new counter packets 409864 bytes 66697711 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp tcp dport 52000 counter packets 2438 bytes 567566 jump f2b-sshd
		ip protocol tcp counter packets 10578945 bytes 21771916887 jump f2b-ufw-portscan
		iifname "tun79" counter packets 0 bytes 0 accept
		counter packets 351277711 bytes 362128569757 jump ufw-before-logging-input
		counter packets 351277711 bytes 362128569757 jump ufw-before-input
		counter packets 4234469 bytes 1101477191 jump ufw-after-input
		counter packets 1072284 bytes 79483906 jump ufw-after-logging-input
		counter packets 1072284 bytes 79483906 jump ufw-reject-input
		counter packets 1072284 bytes 79483906 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 0 bytes 0 accept
		counter packets 301349238 bytes 238416618923 jump ufw-before-logging-output
		counter packets 301349238 bytes 238416618923 jump ufw-before-output
		counter packets 1723893 bytes 181750156 jump ufw-after-output
		counter packets 1723893 bytes 181750156 jump ufw-after-logging-output
		counter packets 1723893 bytes 181750156 jump ufw-reject-output
		counter packets 1723893 bytes 181750156 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 1626865679 bytes 1969938774836 jump DOCKER-USER
		counter packets 1626865686 bytes 1969938776050 jump DOCKER-FORWARD
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 7650213 bytes 9480202529 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 9629828 bytes 4275958252 accept
	...
	chain DOCKER {
		ip daddr 172.29.172.2 iifname != "amn0" oifname "amn0" udp dport 39425 counter packets 28811 bytes 4915322 accept
		ip daddr 172.17.0.2 iifname != "docker0" oifname "docker0" tcp dport 443 counter packets 396895 bytes 24471536 accept
	...
	chain DOCKER-FORWARD {
		counter packets 1626865686 bytes 1969938776050 jump DOCKER-CT
		counter packets 820256384 bytes 997012583952 jump DOCKER-INTERNAL
		counter packets 820256384 bytes 997012583952 jump DOCKER-BRIDGE
		iifname "br-566ebeb0f4f1" counter packets 0 bytes 0 accept
		iifname "amn0" counter packets 615627385 bytes 870357915115 accept
		iifname "docker0" counter packets 186923250 bytes 112869129155 accept
	}

	chain DOCKER-BRIDGE {
		oifname "br-566ebeb0f4f1" counter packets 22 bytes 1320 jump DOCKER
		oifname "amn0" counter packets 28811 bytes 4915322 jump DOCKER
		oifname "docker0" counter packets 396895 bytes 24471536 jump DOCKER
	}

	chain DOCKER-CT {
		oifname "br-566ebeb0f4f1" ct state related,established counter packets 0 bytes 0 accept
		oifname "amn0" ct state related,established counter packets 632840156 bytes 843899877581 accept
		oifname "docker0" ct state related,established counter packets 173769146 bytes 129026314517 accept
	}
	...
	chain f2b-ufw-portscan {
		ip saddr 64.124.145.4 counter packets 0 bytes 0 reject
		ip saddr 206.223.236.65 counter packets 4 bytes 224 reject
		ip saddr 85.217.140.24 counter packets 6 bytes 312 reject
		ip saddr 85.217.149.52 counter packets 10 bytes 520 reject
		ip saddr 85.217.140.20 counter packets 19 bytes 988 reject
		ip saddr 85.217.140.1 counter packets 18 bytes 936 reject
		ip saddr 85.217.140.28 counter packets 15 bytes 780 reject
		ip saddr 85.217.140.37 counter packets 27 bytes 1404 reject
		ip saddr 160.187.146.74 counter packets 76 bytes 3344 reject
		ip saddr 85.217.149.71 counter packets 71 bytes 3692 reject
		ip saddr 85.217.140.18 counter packets 73 bytes 3796 reject
		ip saddr 85.217.140.49 counter packets 96 bytes 4992 reject
		ip saddr 85.217.140.33 counter packets 20 bytes 1040 reject
		ip saddr 85.217.140.5 counter packets 134 bytes 6968 reject
		ip saddr 85.217.140.7 counter packets 36 bytes 1872 reject
		ip saddr 85.217.149.74 counter packets 142 bytes 7384 reject
		ip saddr 85.217.149.72 counter packets 213 bytes 11076 reject
		ip saddr 85.217.149.70 counter packets 276 bytes 14352 reject
		ip saddr 85.217.140.29 counter packets 114 bytes 5928 reject
		ip saddr 85.217.149.53 counter packets 107 bytes 5564 reject
		ip saddr 85.217.140.11 counter packets 280 bytes 14560 reject
		ip saddr 85.217.149.37 counter packets 128 bytes 6656 reject
		ip saddr 85.217.140.48 counter packets 147 bytes 7644 reject
		ip saddr 85.217.140.32 counter packets 78 bytes 4056 reject
		ip saddr 85.217.140.6 counter packets 242 bytes 12584 reject
		ip saddr 85.217.140.35 counter packets 172 bytes 8944 reject
		ip saddr 5.83.129.207 counter packets 222 bytes 8880 reject
		ip saddr 85.217.149.73 counter packets 388 bytes 20176 reject
		ip saddr 85.217.140.31 counter packets 120 bytes 6240 reject
		ip saddr 85.217.149.63 counter packets 376 bytes 19552 reject
		ip saddr 85.217.140.19 counter packets 127 bytes 6604 reject
		ip saddr 85.217.140.39 counter packets 527 bytes 27404 reject
		ip saddr 85.217.140.30 counter packets 277 bytes 14404 reject
		ip saddr 1.1.1.1 counter packets 15 bytes 1092 reject
		ip saddr 85.217.140.34 counter packets 260 bytes 13520 reject
		ip saddr 85.217.149.68 counter packets 524 bytes 27248 reject
		ip saddr 85.217.140.47 counter packets 623 bytes 32396 reject
		ip saddr 85.217.140.9 counter packets 523 bytes 27196 reject
		ip saddr 85.217.140.52 counter packets 476 bytes 24752 reject
		ip saddr 85.217.140.22 counter packets 241 bytes 12532 reject
		ip saddr 31.14.32.6 counter packets 316 bytes 16432 reject
		ip saddr 85.217.149.64 counter packets 538 bytes 27976 reject
		ip saddr 85.93.10.7 counter packets 385 bytes 15400 reject
		ip saddr 85.217.149.67 counter packets 539 bytes 28028 reject
		ip saddr 47.94.165.91 counter packets 91 bytes 3640 reject
		ip saddr 121.41.167.96 counter packets 127 bytes 5080 reject
		ip saddr 47.95.194.74 counter packets 80 bytes 3200 reject
		ip saddr 47.95.195.112 counter packets 145 bytes 5800 reject
```
...
amnezia-awg2                 amnezia-awg2                     Up 5 weeks   0.0.0.0:39425->39425/udp, [::]:39425->39425/udp
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 47 | 2026-09-07 04:29:39 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-09-06 14:46:30 |
...
total 384K
drwxr-xr-x  2 root root   4.0K Sep  7 03:10 .
