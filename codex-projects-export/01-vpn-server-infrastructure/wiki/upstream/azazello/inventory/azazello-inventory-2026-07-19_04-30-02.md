...
Generated: 2026-07-19T04:30:02+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:02 up 42 days, 14:09,  0 user,  load average: 0.64, 0.36, 0.35
```
...
-A f2b-ufw-portscan -s 179.43.175.236/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 217.148.142.94/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 109.105.209.14/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 109.105.209.13/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 45.79.207.71/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 103.167.90.119/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 45.33.109.8/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 157.240.253.2/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 193.228.162.77/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 167.82.86.13/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 2.21.33.24/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 52.92.18.42/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 45.153.33.70/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 23.50.131.32/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 2.16.10.90/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 151.101.131.6/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 95.100.146.11/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 174.138.11.157/32 -j REJECT --reject-with icmp-port-unreachable
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 3587 bytes 1213820 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 3587 bytes 226897 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		iifname "lo" counter packets 21509265 bytes 11507316635 accept
		ct state related,established counter packets 338761706 bytes 231374801916 accept
		ct state invalid counter packets 22550 bytes 1863867 jump ufw-logging-deny
		ct state invalid counter packets 22550 bytes 1863867 drop
		...
		ip protocol icmp icmp type echo-request counter packets 44640 bytes 2573216 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 3535479 bytes 1028418857 jump ufw-not-local
		...
		counter packets 3535479 bytes 1028418857 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 21509269 bytes 11507316851 accept
		ct state related,established counter packets 420464814 bytes 490171569091 accept
		counter packets 2162627 bytes 272648423 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		udp dport 137 counter packets 298 bytes 25509 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 246 bytes 12880 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 1428 bytes 72464 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 1532918 bytes 502797750 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 257578 bytes 77956922 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 3586 bytes 211297 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 3587 bytes 249275 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 356456 bytes 22899714 accept
		ip protocol udp ct state new counter packets 1764843 bytes 245691238 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 6122189 bytes 7152286900 jump f2b-ufw-portscan
		iifname "tun79" counter packets 1090750 bytes 1359553160 accept
		counter packets 737949115 bytes 610353956094 jump ufw-before-logging-input
		counter packets 737949115 bytes 610353956094 jump ufw-before-input
		counter packets 6642051 bytes 1366802442 jump ufw-after-input
		counter packets 2928315 bytes 163577597 jump ufw-after-logging-input
		counter packets 2928315 bytes 163577597 jump ufw-reject-input
		counter packets 2928315 bytes 163577597 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 31413 bytes 6024246 accept
		counter packets 762293460 bytes 780459461149 jump ufw-before-logging-output
		counter packets 762293460 bytes 780459461149 jump ufw-before-output
		counter packets 5666326 bytes 776175501 jump ufw-after-output
		counter packets 5666326 bytes 776175501 jump ufw-after-logging-output
		counter packets 5666326 bytes 776175501 jump ufw-reject-output
		counter packets 5666326 bytes 776175501 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 155697117 bytes 367640841017 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 186503655 bytes 40569731652 accept
		...
		counter packets 513592758 bytes 541767389846 jump DOCKER-USER
		counter packets 513592758 bytes 541767389846 jump DOCKER-FORWARD
	...
	chain DOCKER {
		ip daddr 172.29.172.2 iifname != "amn0" oifname "amn0" udp dport 39425 counter packets 1683 bytes 239827 accept
		ip daddr 172.17.0.3 iifname != "docker0" oifname "docker0" tcp dport 443 counter packets 12615 bytes 765579 accept
	...
	chain DOCKER-FORWARD {
		counter packets 513592758 bytes 541767389846 jump DOCKER-CT
		counter packets 258909354 bytes 268388538705 jump DOCKER-INTERNAL
		counter packets 258909354 bytes 268388538705 jump DOCKER-BRIDGE
		iifname "br-566ebeb0f4f1" counter packets 0 bytes 0 accept
		iifname "amn0" counter packets 187562755 bytes 237063895415 accept
		iifname "docker0" counter packets 71197105 bytes 31311466669 accept
	}

	chain DOCKER-BRIDGE {
		oifname "br-566ebeb0f4f1" counter packets 0 bytes 0 jump DOCKER
		oifname "amn0" counter packets 39769 bytes 6489720 jump DOCKER
		oifname "docker0" counter packets 109173 bytes 6655291 jump DOCKER
	}

	chain DOCKER-CT {
		oifname "br-566ebeb0f4f1" ct state related,established counter packets 0 bytes 0 accept
		oifname "amn0" ct state related,established counter packets 192595231 bytes 230182279618 accept
		oifname "docker0" ct state related,established counter packets 62088173 bytes 43196571523 accept
	}
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 635 bytes 48538 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 635 bytes 48538 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 1792468 bytes 580865525 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 1744846 bytes 447651069 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 1790633 bytes 580767788 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 1075 bytes 62264 accept
		udp dport 46539 counter packets 31697 bytes 43534366 accept
		udp dport 39425 counter packets 6 bytes 580 accept
		tcp dport 80 counter packets 12501 bytes 747792 accept
		udp dport 1194 counter packets 22 bytes 1185 accept
		tcp dport 8443 counter packets 156 bytes 8672 accept
		...
		tcp dport 52000 counter packets 94 bytes 4891 accept
		udp dport 51820 counter packets 0 bytes 0 accept
		tcp dport 25 counter packets 84 bytes 4592 accept
...
mtproto-telegram             telegrammessenger/proxy:latest   Up 47 hours   0.0.0.0:9443->443/tcp, [::]:9443->443/tcp
amnezia-awg2                 amnezia-awg2                     Up 47 hours   0.0.0.0:39425->39425/udp, [::]:39425->39425/udp
signal-tls-proxy-certbot-1   certbot/certbot                  Up 46 hours   80/tcp, 443/tcp
```
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 41 | 2026-07-19 04:30:12 |
...
| `/etc/x-ui` | directory | root:root | 755 | 4096 | 2026-07-19 04:30:19 |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 104K
drwxr-xr-x  2 root root   4.0K Jul 19 03:30 .
