...
Generated: 2026-08-10T04:30:03+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:03 up 8 days,  1:17,  0 user,  load average: 0.62, 1.17, 0.86
```
...
-A f2b-ufw-portscan -s 183.47.101.67/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 212.63.79.180/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 47.99.112.241/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 178.34.51.16/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 37.19.194.246/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 39.100.84.3/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 104.96.141.44/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 120.55.61.72/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 47.95.211.222/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 47.96.73.30/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 39.100.77.183/32 -j REJECT --reject-with icmp-port-unreachable
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 34801 bytes 13609082 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 34801 bytes 2152310 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		iifname "lo" counter packets 8557757 bytes 4347028883 accept
		ct state related,established counter packets 47354140 bytes 43169106502 accept
		ct state invalid counter packets 6894 bytes 402905 jump ufw-logging-deny
		ct state invalid counter packets 6894 bytes 402905 drop
		...
		ip protocol icmp icmp type echo-request counter packets 5247 bytes 324428 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 1505340 bytes 943442451 jump ufw-not-local
		...
		counter packets 1505340 bytes 943442451 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 8557757 bytes 4347028883 accept
		ct state related,established counter packets 42455984 bytes 27341855126 accept
		counter packets 269275 bytes 30341466 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		udp dport 137 counter packets 204 bytes 17046 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 84 bytes 3860 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 481 bytes 23064 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 597149 bytes 195864572 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 106671 bytes 30526060 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 34800 bytes 2165680 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 34800 bytes 2476625 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 56159 bytes 3379620 accept
		ip protocol udp ct state new counter packets 205804 bytes 26358651 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 4740125 bytes 3490988749 jump f2b-ufw-portscan
		iifname "tun79" counter packets 0 bytes 0 accept
		counter packets 57429378 bytes 48460305169 jump ufw-before-logging-input
		counter packets 57429378 bytes 48460305169 jump ufw-before-input
		counter packets 883409 bytes 236570287 jump ufw-after-input
		counter packets 178820 bytes 10135685 jump ufw-after-logging-input
		counter packets 178820 bytes 10135685 jump ufw-reject-input
		counter packets 178820 bytes 10135685 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 0 bytes 0 accept
		counter packets 51283016 bytes 31719225475 jump ufw-before-logging-output
		counter packets 51283016 bytes 31719225475 jump ufw-before-output
		counter packets 269275 bytes 30341466 jump ufw-after-output
		counter packets 269275 bytes 30341466 jump ufw-after-logging-output
		counter packets 269275 bytes 30341466 jump ufw-reject-output
		counter packets 269275 bytes 30341466 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 369131278 bytes 435720389711 jump DOCKER-USER
		counter packets 369131285 bytes 435720390925 jump DOCKER-FORWARD
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 1919290 bytes 2407334372 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 2402554 bytes 1040266361 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 5974 bytes 357581 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 5974 bytes 357581 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 704589 bytes 226434602 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 801386 bytes 717040183 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 703954 bytes 226402268 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 38544 bytes 2279890 accept
		udp dport 46539 counter packets 510722 bytes 700307945 accept
		udp dport 39425 counter packets 226 bytes 22494 accept
		tcp dport 80 counter packets 42600 bytes 2514506 accept
		udp dport 1194 counter packets 85 bytes 3350 accept
		tcp dport 8443 counter packets 27558 bytes 1618179 accept
		udp dport 21194 counter packets 26 bytes 4008 accept
		iifname "tun79" counter packets 0 bytes 0 accept
		udp dport 33328 counter packets 0 bytes 0 accept
		tcp dport 52000 counter packets 223 bytes 11032 accept
		udp dport 51820 counter packets 0 bytes 0 accept
		tcp dport 25 counter packets 1942 bytes 110460 accept
	...
	chain DOCKER {
		ip daddr 172.29.172.2 iifname != "amn0" oifname "amn0" udp dport 39425 counter packets 12710 bytes 2491141 accept
		ip daddr 172.17.0.2 iifname != "docker0" oifname "docker0" tcp dport 443 counter packets 69613 bytes 4290505 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 46 | 2026-08-10 04:29:57 |
...
| `/etc/x-ui` | directory | root:root | 755 | 4096 | 2026-08-10 04:30:10 |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 220K
drwxr-xr-x  2 root root   4.0K Aug 10 03:10 .
