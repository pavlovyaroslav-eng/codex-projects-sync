...
Generated: 2026-09-09T04:30:02+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:02 up 38 days,  1:17,  0 user,  load average: 0.37, 0.35, 0.27
```
...
udp   UNCONN 0      0                  *:59646            *:*    users:(("rw-core",pid=2921938,fd=60))                        
...
-A f2b-sshd -j RETURN
-A f2b-ufw-portscan -s 185.165.170.59/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 89.144.35.106/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 5.83.129.207/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 121.41.171.40/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 34.21.212.221/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 34.96.154.222/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 173.2.184.203/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 47.94.164.194/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 85.93.10.49/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 47.95.212.72/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 47.94.98.187/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 121.199.173.234/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 121.43.55.59/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 121.41.169.123/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 39.100.83.194/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 85.217.140.26/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 112.124.56.24/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 121.41.166.225/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 162.35.165.228/32 -j REJECT --reject-with icmp-port-unreachable
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 25292 bytes 6255679 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 25292 bytes 1920136 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		...
		ct state related,established counter packets 70293294 bytes 70414345793 accept
		ct state invalid counter packets 488 bytes 64060 jump ufw-logging-deny
		ct state invalid counter packets 488 bytes 64060 drop
		...
		ip protocol icmp icmp type echo-request counter packets 2818 bytes 162814 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 1235722 bytes 404110615 jump ufw-not-local
		...
		counter packets 1235722 bytes 404110615 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 203166 bytes 86146464 accept
		ct state related,established counter packets 74890059 bytes 71732227576 accept
		counter packets 1576307 bytes 171850385 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		udp dport 137 counter packets 116 bytes 9694 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 2 bytes 404 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 60 bytes 2776 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 342 bytes 16720 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 440659 bytes 144535756 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 2 bytes 56 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 73228 bytes 22094169 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 25291 bytes 1525369 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 25292 bytes 1929608 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 733654 bytes 44019240 accept
		ip protocol udp ct state new counter packets 840572 bytes 127624752 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp tcp dport 52000 counter packets 3516 bytes 807345 jump f2b-sshd
		ip protocol tcp counter packets 18222139 bytes 40688336643 jump f2b-ufw-portscan
		iifname "tun79" counter packets 0 bytes 0 accept
		counter packets 380031229 bytes 392585028382 jump ufw-before-logging-input
		counter packets 380031229 bytes 392585028382 jump ufw-before-input
		counter packets 4456136 bytes 1161280681 jump ufw-after-input
		counter packets 1118457 bytes 82388186 jump ufw-after-logging-input
		counter packets 1118457 bytes 82388186 jump ufw-reject-input
		counter packets 1118457 bytes 82388186 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 0 bytes 0 accept
		counter packets 331888889 bytes 269899857274 jump ufw-before-logging-output
		counter packets 331888889 bytes 269899857274 jump ufw-before-output
		counter packets 2525506 bytes 264973341 jump ufw-after-output
		counter packets 2525506 bytes 264973341 jump ufw-after-logging-output
		counter packets 2525506 bytes 264973341 jump ufw-reject-output
		counter packets 2525506 bytes 264973341 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 1667322568 bytes 2010109536374 jump DOCKER-USER
		counter packets 1667322575 bytes 2010109537588 jump DOCKER-FORWARD
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 8132798 bytes 10111678415 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 10273535 bytes 4563500446 accept
	...
	chain DOCKER {
		ip daddr 172.29.172.2 iifname != "amn0" oifname "amn0" udp dport 39425 counter packets 2351 bytes 327724 accept
		ip daddr 172.17.0.2 iifname != "docker0" oifname "docker0" tcp dport 443 counter packets 433054 bytes 26671895 accept
	...
	chain DOCKER-FORWARD {
		counter packets 1667322575 bytes 2010109537588 jump DOCKER-CT
		counter packets 841221758 bytes 1017639310040 jump DOCKER-INTERNAL
		counter packets 841221758 bytes 1017639310040 jump DOCKER-BRIDGE
		iifname "br-566ebeb0f4f1" counter packets 0 bytes 0 accept
		iifname "amn0" counter packets 620911835 bytes 876479191474 accept
		iifname "docker0" counter packets 201438512 bytes 126452948643 accept
	}

	chain DOCKER-BRIDGE {
		oifname "br-566ebeb0f4f1" counter packets 22 bytes 1320 jump DOCKER
		oifname "amn0" counter packets 32022 bytes 5327124 jump DOCKER
		oifname "docker0" counter packets 433054 bytes 26671895 jump DOCKER
	}

	chain DOCKER-CT {
		oifname "br-566ebeb0f4f1" ct state related,established counter packets 0 bytes 0 accept
		oifname "amn0" ct state related,established counter packets 638219655 bytes 849898573531 accept
		oifname "docker0" ct state related,established counter packets 187881162 bytes 142571654017 accept
	}
	...
	chain f2b-ufw-portscan {
		ip saddr 185.165.170.59 counter packets 36 bytes 2140 reject
		ip saddr 89.144.35.106 counter packets 30 bytes 1200 reject
		ip saddr 5.83.129.207 counter packets 40 bytes 1600 reject
		ip saddr 194.34.158.240 counter packets 45 bytes 22999 reject
		ip saddr 31.14.32.6 counter packets 1 bytes 52 reject
		ip saddr 87.240.137.208 counter packets 1 bytes 115 reject
		ip saddr 85.217.140.7 counter packets 22 bytes 1144 reject
		ip saddr 121.41.171.40 counter packets 53 bytes 2120 reject
		ip saddr 34.21.212.221 counter packets 11 bytes 644 reject
		ip saddr 34.96.154.222 counter packets 36 bytes 2112 reject
		ip saddr 85.217.149.50 counter packets 41 bytes 2132 reject
		ip saddr 85.217.140.18 counter packets 52 bytes 2704 reject
		ip saddr 85.217.149.70 counter packets 164 bytes 8528 reject
		ip saddr 85.217.149.51 counter packets 94 bytes 4888 reject
		ip saddr 85.217.149.73 counter packets 154 bytes 8008 reject
		ip saddr 85.217.140.11 counter packets 281 bytes 14612 reject
		ip saddr 85.217.140.48 counter packets 328 bytes 17056 reject
		ip saddr 85.217.149.63 counter packets 249 bytes 12948 reject
		ip saddr 173.2.184.203 counter packets 0 bytes 0 reject
		ip saddr 85.217.140.39 counter packets 309 bytes 16068 reject
		ip saddr 85.217.149.68 counter packets 330 bytes 17160 reject
		ip saddr 85.217.140.41 counter packets 370 bytes 19240 reject
		ip saddr 85.217.149.65 counter packets 404 bytes 21008 reject
		ip saddr 85.217.149.67 counter packets 378 bytes 19656 reject
		ip saddr 85.217.140.9 counter packets 403 bytes 20956 reject
		ip saddr 85.217.140.23 counter packets 313 bytes 16276 reject
		ip saddr 85.217.140.43 counter packets 402 bytes 20904 reject
		ip saddr 85.217.140.52 counter packets 457 bytes 23764 reject
		ip saddr 85.217.140.47 counter packets 442 bytes 22984 reject
		ip saddr 47.94.164.194 counter packets 89 bytes 3560 reject
		ip saddr 85.217.149.64 counter packets 602 bytes 31304 reject
		ip saddr 85.217.140.20 counter packets 306 bytes 15912 reject
		ip saddr 85.217.140.12 counter packets 556 bytes 28912 reject
		ip saddr 85.217.140.40 counter packets 679 bytes 35308 reject
		ip saddr 85.217.140.44 counter packets 596 bytes 30992 reject
		ip saddr 85.217.140.8 counter packets 579 bytes 30108 reject
		ip saddr 85.93.10.49 counter packets 28 bytes 1120 reject
		ip saddr 47.95.212.72 counter packets 111 bytes 4440 reject
		ip saddr 85.217.140.53 counter packets 638 bytes 33176 reject
		ip saddr 172.217.118.4 counter packets 31937 bytes 1916108 reject
		ip saddr 85.217.140.10 counter packets 744 bytes 38688 reject
		ip saddr 85.217.140.4 counter packets 723 bytes 37596 reject
		ip saddr 85.217.140.24 counter packets 265 bytes 13780 reject
		ip saddr 85.217.140.15 counter packets 703 bytes 36556 reject
		ip saddr 185.2.80.252 counter packets 433 bytes 22516 reject
		ip saddr 85.217.140.13 counter packets 752 bytes 39104 reject
		ip saddr 85.217.149.52 counter packets 419 bytes 21788 reject
		ip saddr 85.217.140.45 counter packets 850 bytes 44200 reject
```
...
remnanode                    remnawave/node:3.4.1             Up 5 days     
mtproto-telegram             telegrammessenger/proxy:latest   Up 5 weeks    0.0.0.0:9443->443/tcp, [::]:9443->443/tcp
amnezia-awg2                 amnezia-awg2                     Up 34 hours   0.0.0.0:39425->39425/udp, [::]:39425->39425/udp
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 48 | 2026-09-09 04:30:08 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-09-08 21:55:52 |
...
total 392K
drwxr-xr-x  2 root root   4.0K Sep  9 03:10 .
