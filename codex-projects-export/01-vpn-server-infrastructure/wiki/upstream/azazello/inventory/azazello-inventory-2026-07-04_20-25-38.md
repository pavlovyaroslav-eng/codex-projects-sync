...
Generated: 2026-07-04T20:25:38+00:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 20:25:38 up 28 days,  9:05,  1 user,  load average: 1.02, 0.84, 0.92
```
...
udp   UNCONN 0      6528         0.0.0.0:21194      0.0.0.0:*    users:(("openvpn",pid=709029,fd=5))                        
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 23658 bytes 5845881 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 23658 bytes 1677105 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		iifname "lo" counter packets 6584544 bytes 3942580524 accept
		ct state related,established counter packets 155556943 bytes 131081779317 accept
		ct state invalid counter packets 5679 bytes 372935 jump ufw-logging-deny
		ct state invalid counter packets 5679 bytes 372935 drop
		...
		ip protocol icmp icmp type echo-request counter packets 11804 bytes 684193 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 1232288 bytes 276588310 jump ufw-not-local
		...
		counter packets 1232288 bytes 276588310 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 6584548 bytes 3942580740 accept
		ct state related,established counter packets 164083729 bytes 190663272542 accept
		counter packets 1333414 bytes 184518228 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		...
		udp dport 67 counter packets 456171 bytes 149624734 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 76225 bytes 23199210 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 23658 bytes 1520333 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 23285 bytes 1947456 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 170140 bytes 11613916 accept
		ip protocol udp ct state new counter packets 1153282 bytes 171925135 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 44956762 bytes 43933099080 jump f2b-ufw-portscan
		iifname "tun79" counter packets 359693 bytes 447669988 accept
		counter packets 537466733 bytes 501740986882 jump ufw-before-logging-input
		counter packets 537466733 bytes 501740986882 jump ufw-before-input
		counter packets 4798333 bytes 924953042 jump ufw-after-input
		counter packets 2344050 bytes 129733034 jump ufw-after-logging-input
		counter packets 2344050 bytes 129733034 jump ufw-reject-input
		counter packets 2344050 bytes 129733034 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 10333 bytes 1970914 accept
		counter packets 490158441 bytes 473298298294 jump ufw-before-logging-output
		counter packets 490158441 bytes 473298298294 jump ufw-before-output
		counter packets 4837113 bytes 688045306 jump ufw-after-output
		counter packets 4837113 bytes 688045306 jump ufw-after-logging-output
		counter packets 4837113 bytes 688045306 jump ufw-reject-output
		counter packets 4837113 bytes 688045306 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 51148007 bytes 126123996635 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 61431570 bytes 11268345831 accept
		...
		counter packets 258191409 bytes 297897502484 jump DOCKER-USER
		counter packets 258191409 bytes 297897502484 jump DOCKER-FORWARD
	...
	chain DOCKER {
		ip daddr 172.17.0.3 iifname != "docker0" oifname "docker0" tcp dport 443 counter packets 2779 bytes 172184 accept
	...
	chain DOCKER-FORWARD {
		counter packets 258191409 bytes 297897502484 jump DOCKER-CT
		counter packets 129379329 bytes 151257909349 jump DOCKER-INTERNAL
		counter packets 129379329 bytes 151257909349 jump DOCKER-BRIDGE
		iifname "br-566ebeb0f4f1" counter packets 0 bytes 0 accept
		iifname "amn0" counter packets 127990588 bytes 150914240978 accept
		iifname "docker0" counter packets 1362109 bytes 339845309 accept
	}
	...
	chain DOCKER-BRIDGE {
		...
		oifname "docker0" counter packets 2779 bytes 172184 jump DOCKER
	}

	chain DOCKER-CT {
		oifname "br-566ebeb0f4f1" ct state related,established counter packets 0 bytes 0 accept
		oifname "amn0" ct state related,established counter packets 127757243 bytes 146272941015 accept
		oifname "docker0" ct state related,established counter packets 1054837 bytes 366652120 accept
	}
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 4906 bytes 327013 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 4906 bytes 327013 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 533015 bytes 172860688 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 699805 bytes 103756014 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 532483 bytes 172832296 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 151018 bytes 9503544 accept
		udp dport 46539 counter packets 43775 bytes 50530365 accept
		udp dport 39425 counter packets 75 bytes 7699 accept
		tcp dport 80 counter packets 56284 bytes 3360841 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 41 | 2026-07-04 20:25:22 |
...
| `/etc/x-ui` | directory | root:root | 755 | 4096 | 2026-07-04 20:25:47 |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 8.0K
drwxr-xr-x  2 root root   4.0K Jul  4 12:46 .
drwxr-xr-x 17 root syslog 4.0K Jul  4 20:25 ..
```
