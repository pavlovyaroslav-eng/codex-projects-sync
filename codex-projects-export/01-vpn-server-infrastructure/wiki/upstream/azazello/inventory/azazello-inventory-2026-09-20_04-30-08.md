...
Generated: 2026-09-20T04:30:08+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:09 up 6 days,  1:17,  0 user,  load average: 0.54, 0.46, 0.36
```
...
tcp   LISTEN 0      100          0.0.0.0:25         0.0.0.0:*    users:(("smtpd",pid=1257610,fd=6),("master",pid=1249347,fd=13))
...
tcp   LISTEN 0      4096         0.0.0.0:52000      0.0.0.0:*    users:(("sshd",pid=1249032,fd=3),("systemd",pid=1,fd=171))     
...
-A f2b-ufw-portscan -s 76.9.201.198/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 54.38.196.162/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 216.238.124.78/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 216.180.246.241/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 216.180.246.230/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 198.251.81.159/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 172.93.0.251/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 104.243.42.83/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 103.249.70.34/32 -j REJECT --reject-with icmp-port-unreachable
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 26161 bytes 5693548 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 26160 bytes 2418592 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		iifname "lo" counter packets 1922 bytes 317445 accept
		ct state related,established counter packets 92767824 bytes 97818584464 accept
		ct state invalid counter packets 327 bytes 48244 jump ufw-logging-deny
		ct state invalid counter packets 327 bytes 48244 drop
		...
		ip protocol icmp icmp type echo-request counter packets 3548 bytes 227050 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 966981 bytes 196435907 jump ufw-not-local
		...
		counter packets 966981 bytes 196435907 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 1922 bytes 317445 accept
		ct state related,established counter packets 97675226 bytes 101439696428 accept
		counter packets 1284555 bytes 202562118 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		udp dport 137 counter packets 70 bytes 5676 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 5 bytes 1010 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 46 bytes 2152 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 732 bytes 33356 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 418373 bytes 137226008 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 6 bytes 168 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 72506 bytes 22893262 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 26160 bytes 1595412 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 26160 bytes 2407763 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 565355 bytes 33921268 accept
		ip protocol udp ct state new counter packets 717212 bytes 168452701 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 76741 bytes 446225759 jump f2b-ufw-portscan
		iifname "tun79" counter packets 0 bytes 0 accept
		counter packets 93740602 bytes 98015613110 jump ufw-before-logging-input
		counter packets 93740602 bytes 98015613110 jump ufw-before-input
		counter packets 673152 bytes 178567129 jump ufw-after-input
		counter packets 181414 bytes 18405497 jump ufw-after-logging-input
		counter packets 181414 bytes 18405497 jump ufw-reject-input
		counter packets 181414 bytes 18405497 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 0 bytes 0 accept
		counter packets 98961703 bytes 101642575991 jump ufw-before-logging-output
		counter packets 98961703 bytes 101642575991 jump ufw-before-output
		counter packets 1284555 bytes 202562118 jump ufw-after-output
		counter packets 1284555 bytes 202562118 jump ufw-after-logging-output
		counter packets 1284555 bytes 202562118 jump ufw-reject-output
		counter packets 1284555 bytes 202562118 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 120730789 bytes 116807913389 jump DOCKER-USER
		counter packets 120730790 bytes 116807913449 jump DOCKER-FORWARD
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 1441883 bytes 1755249254 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 1978360 bytes 1019748461 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 327 bytes 48244 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 327 bytes 48244 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 491738 bytes 160161632 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 476090 bytes 36316649 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 490891 bytes 160119258 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 175768 bytes 10331584 accept
		udp dport 46539 counter packets 61 bytes 79367 accept
		udp dport 39425 counter packets 158 bytes 14738 accept
		tcp dport 80 counter packets 96332 bytes 5673753 accept
		udp dport 1194 counter packets 53 bytes 2775 accept
		tcp dport 8443 counter packets 19332 bytes 1134346 accept
		udp dport 21194 counter packets 20 bytes 3120 accept
		iifname "tun79" counter packets 0 bytes 0 accept
		udp dport 33328 counter packets 1 bytes 389 accept
		tcp dport 52000 counter packets 151 bytes 8116 accept
		udp dport 51820 counter packets 1 bytes 214 accept
		tcp dport 25 counter packets 1489 bytes 84660 accept
		...
		ip saddr 93.183.106.203 tcp dport 2222 counter packets 24 bytes 1440 accept
		ip saddr 185.71.196.110 udp dport 24443 counter packets 407 bytes 532356 accept
	}
	...
	chain DOCKER {
		ip daddr 172.29.172.2 iifname != "amn0" oifname "amn0" udp dport 39425 counter packets 6878 bytes 729092 accept
```
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 47 | 2026-09-20 04:30:02 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-09-19 18:24:26 |
...
drwxrwxr-x 17 root syslog 4.0K Sep 20 00:00 ..
