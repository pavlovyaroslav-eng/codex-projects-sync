...
Generated: 2026-08-06T04:30:03+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:04 up 4 days,  1:17,  0 user,  load average: 3.05, 1.33, 0.73
```
...
udp   UNCONN 0      0              *%wg0:45012            *:*    users:(("xray-linux-amd6",pid=840772,fd=37))               
udp   UNCONN 0      0              *%wg0:35003            *:*    users:(("xray-linux-amd6",pid=840772,fd=35))               
...
-A f2b-ufw-portscan -s 173.194.10.134/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 45.56.72.13/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 50.116.30.65/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 35.172.123.124/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 45.131.48.147/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 217.119.129.227/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 91.108.4.151/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 87.209.132.177/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 178.217.160.16/32 -j REJECT --reject-with icmp-port-unreachable

table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 17522 bytes 6794037 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 17522 bytes 1083608 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		iifname "lo" counter packets 4284095 bytes 2158162487 accept
		ct state related,established counter packets 5996027 bytes 4196101285 accept
		ct state invalid counter packets 3855 bytes 219614 jump ufw-logging-deny
		ct state invalid counter packets 3855 bytes 219614 drop
		...
		ip protocol icmp icmp type echo-request counter packets 2870 bytes 176274 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 802713 bytes 472172418 jump ufw-not-local
		...
		counter packets 802713 bytes 472172418 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 4284095 bytes 2158162487 accept
		ct state related,established counter packets 6158431 bytes 4422454444 accept
		counter packets 118812 bytes 13032529 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		udp dport 137 counter packets 168 bytes 14022 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 50 bytes 2248 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 248 bytes 11820 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 296090 bytes 97117220 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 56344 bytes 15471621 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 17520 bytes 1075878 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 17520 bytes 1215195 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 26483 bytes 1588980 accept
		ip protocol udp ct state new counter packets 88877 bytes 11171895 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 5813001 bytes 3177262089 jump f2b-ufw-portscan
		iifname "tun79" counter packets 0 bytes 0 accept
		counter packets 11089560 bytes 6826832078 jump ufw-before-logging-input
		counter packets 11089560 bytes 6826832078 jump ufw-before-input
		counter packets 477098 bytes 119331412 jump ufw-after-input
		counter packets 124198 bytes 6714481 jump ufw-after-logging-input
		counter packets 124198 bytes 6714481 jump ufw-reject-input
		counter packets 124198 bytes 6714481 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 0 bytes 0 accept
		counter packets 10561338 bytes 6593649460 jump ufw-before-logging-output
		counter packets 10561338 bytes 6593649460 jump ufw-before-output
		counter packets 118812 bytes 13032529 jump ufw-after-output
		counter packets 118812 bytes 13032529 jump ufw-after-logging-output
		counter packets 118812 bytes 13032529 jump ufw-reject-output
		counter packets 118812 bytes 13032529 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 180504057 bytes 208401198601 jump DOCKER-USER
		counter packets 180504064 bytes 208401199815 jump DOCKER-FORWARD
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 1066183 bytes 1321989953 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 1358591 bytes 604548661 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 3330 bytes 192087 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 3330 bytes 192087 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 352900 bytes 112616931 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 450157 bytes 359573093 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 352556 bytes 112599325 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 30907 bytes 1823215 accept
		udp dport 46539 counter packets 256352 bytes 348763446 accept
		udp dport 39425 counter packets 122 bytes 11973 accept
		tcp dport 80 counter packets 12997 bytes 760051 accept
		udp dport 1194 counter packets 57 bytes 2236 accept
		tcp dport 8443 counter packets 23615 bytes 1388977 accept
		udp dport 21194 counter packets 21 bytes 3276 accept
		iifname "tun79" counter packets 0 bytes 0 accept
		udp dport 33328 counter packets 0 bytes 0 accept
		tcp dport 52000 counter packets 124 bytes 6224 accept
		udp dport 51820 counter packets 0 bytes 0 accept
		tcp dport 25 counter packets 1415 bytes 81308 accept
	...
	chain f2b-ufw-portscan {
		ip saddr 173.194.10.134 counter packets 3 bytes 156 reject
		ip saddr 17.253.122.197 counter packets 0 bytes 0 reject
		ip saddr 46.228.223.174 counter packets 3 bytes 156 reject
```
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 47 | 2026-08-06 04:30:24 |
...
| `/etc/x-ui` | directory | root:root | 755 | 4096 | 2026-08-06 04:30:45 |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 196K
drwxr-xr-x  2 root root   4.0K Aug  6 03:10 .
