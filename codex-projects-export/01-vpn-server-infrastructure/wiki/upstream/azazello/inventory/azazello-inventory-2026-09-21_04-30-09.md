...
Generated: 2026-09-21T04:30:09+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:09 up 7 days,  1:17,  0 user,  load average: 0.47, 0.26, 0.26
```
...
udp   UNCONN 0      0                  *:55171            *:*    users:(("rw-core",pid=3110,fd=24))                         
udp   UNCONN 0      0              [::1]:8388          [::]:*    users:(("ss-server",pid=1090,fd=7))                        
udp   UNCONN 0      0                  *:51540            *:*    users:(("rw-core",pid=3110,fd=31))                         
udp   UNCONN 0      0               [::]:39425         [::]:*    users:(("docker-proxy",pid=2667,fd=7))                     
udp   UNCONN 0      0               [::]:51820         [::]:*                                                               
udp   UNCONN 0      0                  *:46359            *:*    users:(("rw-core",pid=3110,fd=26))                         
udp   UNCONN 0      0                  *:40217            *:*    users:(("rw-core",pid=3110,fd=23))                         
...
-A f2b-ufw-portscan -s 138.197.195.248/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 17.253.122.199/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 51.75.53.4/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 2.23.231.137/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 16.5.0.239/32 -j REJECT --reject-with icmp-port-unreachable
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 30481 bytes 6734184 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 30481 bytes 2816445 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		...
		ct state related,established counter packets 110038718 bytes 116523578213 accept
		ct state invalid counter packets 386 bytes 54213 jump ufw-logging-deny
		ct state invalid counter packets 386 bytes 54213 drop
		...
		ip protocol icmp icmp type echo-request counter packets 3920 bytes 243893 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 1074567 bytes 219486048 jump ufw-not-local
		...
		counter packets 1074567 bytes 219486048 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 1932 bytes 318825 accept
		ct state related,established counter packets 115342922 bytes 120806985672 accept
		counter packets 1543687 bytes 241810741 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		udp dport 137 counter packets 84 bytes 6768 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 7 bytes 1414 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 57 bytes 2648 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 811 bytes 36796 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 470115 bytes 154197328 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 7 bytes 196 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 82380 bytes 26322276 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 30480 bytes 1863726 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 30480 bytes 2812885 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 685628 bytes 41138680 accept
		ip protocol udp ct state new counter packets 855721 bytes 200442540 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 3432658 bytes 11441182483 jump f2b-ufw-portscan
		iifname "tun79" counter packets 0 bytes 0 accept
		counter packets 111119523 bytes 116743681192 jump ufw-before-logging-input
		counter packets 111119523 bytes 116743681192 jump ufw-before-input
		counter packets 771388 bytes 201070149 jump ufw-after-input
		counter packets 217927 bytes 20502723 jump ufw-after-logging-input
		counter packets 217927 bytes 20502723 jump ufw-reject-input
		counter packets 217927 bytes 20502723 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 0 bytes 0 accept
		counter packets 116888541 bytes 121049115238 jump ufw-before-logging-output
		counter packets 116888541 bytes 121049115238 jump ufw-before-output
		counter packets 1543687 bytes 241810741 jump ufw-after-output
		counter packets 1543687 bytes 241810741 jump ufw-after-logging-output
		counter packets 1543687 bytes 241810741 jump ufw-reject-output
		counter packets 1543687 bytes 241810741 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 149193825 bytes 147876284368 jump DOCKER-USER
		counter packets 149193826 bytes 147876284428 jump DOCKER-FORWARD
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 1755643 bytes 2112006959 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 2391120 bytes 1234116254 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 386 bytes 54213 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 386 bytes 54213 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 553461 bytes 180567426 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 522061 bytes 38966728 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 552506 bytes 180519320 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 183225 bytes 10771108 accept
		udp dport 46539 counter packets 61 bytes 79367 accept
		udp dport 39425 counter packets 158 bytes 14738 accept
		tcp dport 80 counter packets 97135 bytes 5719160 accept
		udp dport 1194 counter packets 60 bytes 3073 accept
		tcp dport 8443 counter packets 20287 bytes 1189174 accept
		udp dport 21194 counter packets 20 bytes 3120 accept
		iifname "tun79" counter packets 0 bytes 0 accept
		udp dport 33328 counter packets 1 bytes 389 accept
		tcp dport 52000 counter packets 160 bytes 8628 accept
		udp dport 51820 counter packets 1 bytes 214 accept
		tcp dport 25 counter packets 1608 bytes 91212 accept
	...
	chain DOCKER {
		ip daddr 172.29.172.2 iifname != "amn0" oifname "amn0" udp dport 39425 counter packets 8272 bytes 876011 accept
```
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 49 | 2026-09-21 04:30:03 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-09-20 13:54:48 |
...
total 468K
drwxr-xr-x  2 root root   4.0K Sep 21 03:10 .
