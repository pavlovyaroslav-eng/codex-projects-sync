...
Generated: 2026-09-22T04:30:10+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:10 up 8 days,  1:17,  0 user,  load average: 1.26, 0.43, 0.22
```
...
-A f2b-ufw-portscan -s 176.58.20.18/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 184.25.53.44/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 81.2.197.150/32 -j REJECT --reject-with icmp-port-unreachable
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 34801 bytes 7778820 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 34801 bytes 3211699 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		...
		ct state related,established counter packets 124316413 bytes 135364506188 accept
		ct state invalid counter packets 458 bytes 61000 jump ufw-logging-deny
		ct state invalid counter packets 458 bytes 61000 drop
		...
		ip protocol icmp icmp type echo-request counter packets 4541 bytes 280452 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 1189835 bytes 243189361 jump ufw-not-local
		...
		counter packets 1189835 bytes 243189361 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 1932 bytes 318825 accept
		ct state related,established counter packets 128392734 bytes 140283070326 accept
		counter packets 1751395 bytes 274543696 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		udp dport 137 counter packets 95 bytes 7626 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 8 bytes 1616 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 66 bytes 3068 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 873 bytes 39840 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 521645 bytes 171099112 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 7 bytes 196 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 92193 bytes 29739056 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 34801 bytes 2128738 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 34800 bytes 3204363 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 783689 bytes 47022340 accept
		ip protocol udp ct state new counter packets 965027 bytes 227256657 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 7624379 bytes 25443896479 jump f2b-ufw-portscan
		iifname "tun79" counter packets 0 bytes 0 accept
		counter packets 125513179 bytes 135608355826 jump ufw-before-logging-input
		counter packets 125513179 bytes 135608355826 jump ufw-before-input
		counter packets 853268 bytes 222806561 jump ufw-after-input
		counter packets 238381 bytes 21916047 jump ufw-after-logging-input
		counter packets 238381 bytes 21916047 jump ufw-reject-input
		counter packets 238381 bytes 21916047 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 0 bytes 0 accept
		counter packets 130146061 bytes 140557932847 jump ufw-before-logging-output
		counter packets 130146061 bytes 140557932847 jump ufw-before-output
		counter packets 1751395 bytes 274543696 jump ufw-after-output
		counter packets 1751395 bytes 274543696 jump ufw-after-logging-output
		counter packets 1751395 bytes 274543696 jump ufw-reject-output
		counter packets 1751395 bytes 274543696 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 172989944 bytes 170874162551 jump DOCKER-USER
		counter packets 172989945 bytes 170874162611 jump DOCKER-FORWARD
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 2083562 bytes 2582893781 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 2824715 bytes 1424464987 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 458 bytes 61000 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 458 bytes 61000 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 614887 bytes 200890514 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 575984 bytes 42351515 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 613851 bytes 200837846 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 188450 bytes 11078564 accept
		udp dport 46539 counter packets 61 bytes 79367 accept
		udp dport 39425 counter packets 158 bytes 14738 accept
		tcp dport 80 counter packets 122205 bytes 7197103 accept
		udp dport 1194 counter packets 65 bytes 3287 accept
		tcp dport 8443 counter packets 23290 bytes 1365738 accept
		udp dport 21194 counter packets 20 bytes 3120 accept
		iifname "tun79" counter packets 0 bytes 0 accept
		udp dport 33328 counter packets 1 bytes 389 accept
		tcp dport 52000 counter packets 179 bytes 9656 accept
		udp dport 51820 counter packets 1 bytes 214 accept
		tcp dport 25 counter packets 1673 bytes 94848 accept
		...
		ip saddr 93.183.106.203 tcp dport 2222 counter packets 25 bytes 1500 accept
	...
	chain DOCKER {
		ip daddr 172.29.172.2 iifname != "amn0" oifname "amn0" udp dport 39425 counter packets 11286 bytes 1214369 accept
```
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 49 | 2026-09-22 04:30:06 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-09-21 20:04:45 |
...
total 472K
drwxr-xr-x  2 root root   4.0K Sep 22 03:10 .
