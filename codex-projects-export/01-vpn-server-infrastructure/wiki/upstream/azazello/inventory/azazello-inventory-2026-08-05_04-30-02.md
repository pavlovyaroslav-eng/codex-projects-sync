...
Generated: 2026-08-05T04:30:03+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:03 up 3 days,  1:17,  0 user,  load average: 0.43, 0.42, 0.41
```
...
udp   UNCONN 0      0                  *:56984            *:*    users:(("xray-linux-amd6",pid=1574,fd=21))             
...
-A f2b-ufw-portscan -s 34.54.126.129/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 204.48.25.231/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 44.192.202.0/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 20.168.93.93/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 44.192.202.16/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 85.217.140.40/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 95.164.93.120/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 85.112.121.49/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 84.42.94.254/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 85.217.140.9/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 178.17.14.2/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 85.217.149.63/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 185.180.14.178/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 178.236.132.38/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 194.180.209.21/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 85.217.140.12/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 52.154.19.231/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 52.159.229.51/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 137.220.136.188/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 74.235.126.230/32 -j REJECT --reject-with icmp-port-unreachable
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 13201 bytes 5186238 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 13201 bytes 820680 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		iifname "lo" counter packets 3238599 bytes 1637355395 accept
		ct state related,established counter packets 5446670 bytes 3841652102 accept
		ct state invalid counter packets 3159 bytes 173938 jump ufw-logging-deny
		ct state invalid counter packets 3159 bytes 173938 drop
		...
		ip protocol icmp icmp type echo-request counter packets 1861 bytes 112222 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 596821 bytes 357960379 jump ufw-not-local
		...
		counter packets 596821 bytes 357960379 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 3238599 bytes 1637355395 accept
		ct state related,established counter packets 5520559 bytes 4037800707 accept
		counter packets 97392 bytes 11447813 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		udp dport 137 counter packets 163 bytes 13632 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 39 bytes 1756 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 177 bytes 8368 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 220822 bytes 72429616 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 40240 bytes 11520322 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 13201 bytes 821132 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 13200 bytes 917683 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 20321 bytes 1219260 accept
		ip protocol udp ct state new counter packets 74259 bytes 10009645 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 4565927 bytes 2589196534 jump f2b-ufw-portscan
		iifname "tun79" counter packets 0 bytes 0 accept
		counter packets 9287110 bytes 5837254036 jump ufw-before-logging-input
		counter packets 9287110 bytes 5837254036 jump ufw-before-input
		counter packets 335414 bytes 88189288 jump ufw-after-input
		counter packets 73973 bytes 4215594 jump ufw-after-logging-input
		counter packets 73973 bytes 4215594 jump ufw-reject-input
		counter packets 73973 bytes 4215594 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 0 bytes 0 accept
		counter packets 8856550 bytes 5686603915 jump ufw-before-logging-output
		counter packets 8856550 bytes 5686603915 jump ufw-before-output
		counter packets 97392 bytes 11447813 jump ufw-after-output
		counter packets 97392 bytes 11447813 jump ufw-after-logging-output
		counter packets 97392 bytes 11447813 jump ufw-reject-output
		counter packets 97392 bytes 11447813 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 138344936 bytes 156895038526 jump DOCKER-USER
		counter packets 138344943 bytes 156895039740 jump DOCKER-FORWARD
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 834663 bytes 1082107991 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 1064470 bytes 452162886 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 2679 bytes 150540 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 2679 bytes 150540 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 261441 bytes 83973694 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 335636 bytes 273999929 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 261185 bytes 83960450 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 29282 bytes 1728211 accept
		udp dport 46539 counter packets 195368 bytes 265880930 accept
		udp dport 39425 counter packets 95 bytes 9126 accept
		tcp dport 80 counter packets 12248 bytes 717250 accept
		udp dport 1194 counter packets 46 bytes 1662 accept
		tcp dport 8443 counter packets 23197 bytes 1365464 accept
		udp dport 21194 counter packets 21 bytes 3276 accept
		iifname "tun79" counter packets 0 bytes 0 accept
		udp dport 33328 counter packets 0 bytes 0 accept
		tcp dport 52000 counter packets 83 bytes 4176 accept
		udp dport 51820 counter packets 0 bytes 0 accept
		tcp dport 25 counter packets 1062 bytes 60696 accept
	...
	chain f2b-ufw-portscan {
		ip saddr 149.154.166.110 counter packets 44 bytes 2288 reject
		ip saddr 85.217.140.34 counter packets 5 bytes 260 reject
		ip saddr 34.54.126.129 counter packets 29 bytes 1664 reject
```
...
mtproto-telegram             telegrammessenger/proxy:latest   Up 3 days   0.0.0.0:9443->443/tcp, [::]:9443->443/tcp
amnezia-awg2                 amnezia-awg2                     Up 3 days   0.0.0.0:39425->39425/udp, [::]:39425->39425/udp
signal-tls-proxy-certbot-1   certbot/certbot                  Up 3 days   80/tcp, 443/tcp
```
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 47 | 2026-08-05 04:30:01 |
...
| `/etc/x-ui` | directory | root:root | 755 | 4096 | 2026-08-05 04:30:19 |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 192K
drwxr-xr-x  2 root root   4.0K Aug  5 03:10 .
