...
Generated: 2026-08-09T04:30:02+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:02 up 7 days,  1:17,  0 user,  load average: 1.34, 0.78, 0.55
```
...
tcp   LISTEN 0      4096         0.0.0.0:52000      0.0.0.0:*    users:(("sshd",pid=3956,fd=3),("systemd",pid=1,fd=148))    
...
-A f2b-ufw-portscan -s 47.93.43.47/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 121.41.167.149/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 91.201.18.50/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 121.40.46.70/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 176.65.148.107/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 142.251.38.206/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 121.43.116.220/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 179.43.175.250/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 47.95.211.174/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 34.98.110.65/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 112.124.67.154/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 121.199.173.43/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 47.94.82.6/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 121.41.167.161/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 39.100.84.225/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 137.220.136.177/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 85.217.140.45/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 34.107.243.93/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 172.237.130.194/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 47.95.206.102/32 -j REJECT --reject-with icmp-port-unreachable
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 30481 bytes 12121862 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 30481 bytes 1881564 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		iifname "lo" counter packets 7485265 bytes 3784847833 accept
		ct state related,established counter packets 43156680 bytes 38760689849 accept
		ct state invalid counter packets 6496 bytes 365844 jump ufw-logging-deny
		ct state invalid counter packets 6496 bytes 365844 drop
		...
		ip protocol icmp icmp type echo-request counter packets 4433 bytes 272913 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 1325131 bytes 824643822 jump ufw-not-local
		...
		counter packets 1325131 bytes 824643822 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 7485265 bytes 3784847833 accept
		ct state related,established counter packets 38414408 bytes 24201505676 accept
		counter packets 211739 bytes 22278142 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		udp dport 137 counter packets 200 bytes 16734 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 77 bytes 3508 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 437 bytes 20976 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 521669 bytes 171107132 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 94097 bytes 26764037 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 30481 bytes 1894182 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 30480 bytes 2143260 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 47159 bytes 2839620 accept
		ip protocol udp ct state new counter packets 158554 bytes 18937025 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 2535850 bytes 1607993371 jump f2b-ufw-portscan
		iifname "tun79" counter packets 0 bytes 0 accept
		counter packets 51978005 bytes 43370820261 jump ufw-before-logging-input
		counter packets 51978005 bytes 43370820261 jump ufw-before-input
		counter packets 783395 bytes 207330253 jump ufw-after-input
		counter packets 166915 bytes 9417866 jump ufw-after-logging-input
		counter packets 166915 bytes 9417866 jump ufw-reject-input
		counter packets 166915 bytes 9417866 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 0 bytes 0 accept
		counter packets 46111412 bytes 28008631651 jump ufw-before-logging-output
		counter packets 46111412 bytes 28008631651 jump ufw-before-output
		counter packets 211739 bytes 22278142 jump ufw-after-output
		counter packets 211739 bytes 22278142 jump ufw-after-logging-output
		counter packets 211739 bytes 22278142 jump ufw-reject-output
		counter packets 211739 bytes 22278142 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 324376642 bytes 383020158745 jump DOCKER-USER
		counter packets 324376649 bytes 383020159959 jump DOCKER-FORWARD
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 1694143 bytes 2128351613 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 2138057 bytes 935638140 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 5576 bytes 320520 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 5576 bytes 320520 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 616480 bytes 197912387 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 709231 bytes 626761017 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 615900 bytes 197882805 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 34730 bytes 2053935 accept
		udp dport 46539 counter packets 447766 bytes 611763462 accept
		udp dport 39425 counter packets 220 bytes 21968 accept
		tcp dport 80 counter packets 31295 bytes 1849680 accept
		udp dport 1194 counter packets 78 bytes 3085 accept
		tcp dport 8443 counter packets 25535 bytes 1500139 accept
		udp dport 21194 counter packets 26 bytes 4008 accept
		iifname "tun79" counter packets 0 bytes 0 accept
		udp dport 33328 counter packets 0 bytes 0 accept
		tcp dport 52000 counter packets 208 bytes 10224 accept
		udp dport 51820 counter packets 0 bytes 0 accept
		tcp dport 25 counter packets 1873 bytes 106768 accept
	...
	chain DOCKER {
		ip daddr 172.29.172.2 iifname != "amn0" oifname "amn0" udp dport 39425 counter packets 12061 bytes 2377347 accept
		ip daddr 172.17.0.2 iifname != "docker0" oifname "docker0" tcp dport 443 counter packets 57017 bytes 3508478 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 45 | 2026-08-09 04:30:11 |
...
| `/etc/x-ui` | directory | root:root | 755 | 4096 | 2026-08-09 04:30:10 |
| `/etc/3x-ui` | missing | - | - | - | - |
...
drwxrwxr-x 18 root syslog 4.0K Aug  9 00:00 ..
