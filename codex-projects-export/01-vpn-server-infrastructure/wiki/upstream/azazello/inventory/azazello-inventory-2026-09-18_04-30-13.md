...
Generated: 2026-09-18T04:30:13+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:14 up 4 days,  1:17,  0 user,  load average: 1.27, 0.71, 0.43
```
...
-A f2b-ufw-portscan -s 216.180.246.82/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 51.79.35.142/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 178.32.19.191/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 17.57.13.47/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 185.102.115.190/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 4.153.25.230/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 17.253.122.195/32 -j REJECT --reject-with icmp-port-unreachable
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 17521 bytes 3548699 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 17521 bytes 1640157 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		...
		ct state related,established counter packets 61493147 bytes 65517953821 accept
		ct state invalid counter packets 252 bytes 43099 jump ufw-logging-deny
		ct state invalid counter packets 252 bytes 43099 drop
		...
		ip protocol icmp icmp type echo-request counter packets 1931 bytes 123492 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 737433 bytes 139329269 jump ufw-not-local
		...
		counter packets 737433 bytes 139329269 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 1922 bytes 317445 accept
		ct state related,established counter packets 64456296 bytes 67935660775 accept
		counter packets 856583 bytes 133592110 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		udp dport 137 counter packets 39 bytes 3042 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 4 bytes 808 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 33 bytes 1560 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 456 bytes 20968 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 306374 bytes 100490504 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 4 bytes 112 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 51672 bytes 15884358 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 17520 bytes 1066139 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 17520 bytes 1638152 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 370571 bytes 22234228 accept
		ip protocol udp ct state new counter packets 484670 bytes 111224780 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 14103545 bytes 36158930275 jump f2b-ufw-portscan
		iifname "tun79" counter packets 0 bytes 0 accept
		counter packets 62234685 bytes 65657767126 jump ufw-before-logging-input
		counter packets 62234685 bytes 65657767126 jump ufw-before-input
		counter packets 479917 bytes 123594678 jump ufw-after-input
		counter packets 121335 bytes 7193326 jump ufw-after-logging-input
		counter packets 121335 bytes 7193326 jump ufw-reject-input
		counter packets 121335 bytes 7193326 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 0 bytes 0 accept
		counter packets 65314801 bytes 68069570330 jump ufw-before-logging-output
		counter packets 65314801 bytes 68069570330 jump ufw-before-output
		counter packets 856583 bytes 133592110 jump ufw-after-output
		counter packets 856583 bytes 133592110 jump ufw-after-logging-output
		counter packets 856583 bytes 133592110 jump ufw-reject-output
		counter packets 856583 bytes 133592110 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 83479049 bytes 79415380330 jump DOCKER-USER
		counter packets 83479050 bytes 79415380390 jump DOCKER-FORWARD
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 937458 bytes 1157970371 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 1286546 bytes 633350390 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 252 bytes 43099 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 252 bytes 43099 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 358582 bytes 116401352 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 379387 bytes 22954989 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 358046 bytes 116374280 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 149973 bytes 8813684 accept
		udp dport 46539 counter packets 60 bytes 78960 accept
		udp dport 39425 counter packets 146 bytes 13530 accept
		tcp dport 80 counter packets 94653 bytes 5577978 accept
		udp dport 1194 counter packets 39 bytes 2180 accept
		tcp dport 8443 counter packets 11047 bytes 647281 accept
		udp dport 21194 counter packets 20 bytes 3120 accept
		iifname "tun79" counter packets 0 bytes 0 accept
		udp dport 33328 counter packets 0 bytes 0 accept
		tcp dport 52000 counter packets 90 bytes 5104 accept
		udp dport 51820 counter packets 1 bytes 214 accept
		tcp dport 25 counter packets 1027 bytes 59500 accept
		...
		ip saddr 185.71.196.110 udp dport 24443 counter packets 405 bytes 529740 accept
	}
	...
	chain f2b-ufw-portscan {
		ip saddr 216.180.246.82 counter packets 46 bytes 2024 reject
```
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 49 | 2026-09-18 04:30:16 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-09-17 19:41:35 |
...
total 444K
drwxr-xr-x  2 root root   4.0K Sep 18 03:10 .
