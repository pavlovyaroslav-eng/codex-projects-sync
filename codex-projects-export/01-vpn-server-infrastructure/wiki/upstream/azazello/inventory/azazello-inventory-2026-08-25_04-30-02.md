...
Generated: 2026-08-25T04:30:02+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:03 up 23 days,  1:17,  0 user,  load average: 0.19, 0.14, 0.18
```
...
-A f2b-ufw-portscan -s 178.218.226.80/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 178.218.226.65/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 216.180.246.143/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 31.56.185.1/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 178.217.160.235/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 44.192.202.69/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 163.171.242.133/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 31.13.84.8/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 104.83.4.49/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 185.162.229.2/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 52.0.252.126/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 172.65.254.112/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 15.236.235.17/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 185.59.244.174/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 34.117.147.68/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 2.23.97.232/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 78.156.235.21/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 176.120.214.175/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 17.253.122.203/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 81.163.43.72/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 17.253.122.196/32 -j REJECT --reject-with icmp-port-unreachable
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 99601 bytes 36630442 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 99601 bytes 6149430 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		iifname "lo" counter packets 24673653 bytes 12910824415 accept
		ct state related,established counter packets 218803182 bytes 250739624246 accept
		ct state invalid counter packets 21290 bytes 1233043 jump ufw-logging-deny
		ct state invalid counter packets 21290 bytes 1233043 drop
		...
		ip protocol icmp icmp type echo-request counter packets 15912 bytes 984622 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 4923684 bytes 2964126443 jump ufw-not-local
		...
		counter packets 4923684 bytes 2964126443 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 24673653 bytes 12910824415 accept
		ct state related,established counter packets 179580767 bytes 151030973980 accept
		counter packets 719252 bytes 75011107 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		udp dport 137 counter packets 434 bytes 36718 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 731 bytes 167198 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 217 bytes 10312 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 1445 bytes 70892 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 1721435 bytes 564630081 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 293798 bytes 86659953 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 99601 bytes 6220524 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 99600 bytes 7136249 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 170467 bytes 11075919 accept
		ip protocol udp ct state new counter packets 525952 bytes 61866278 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 79656803 bytes 94871407898 jump f2b-ufw-portscan
		iifname "tun79" counter packets 0 bytes 0 accept
		counter packets 248437721 bytes 266616792769 jump ufw-before-logging-input
		counter packets 248437721 bytes 266616792769 jump ufw-before-input
		counter packets 2746448 bytes 711361008 jump ufw-after-input
		counter packets 728388 bytes 59785854 jump ufw-after-logging-input
		counter packets 728388 bytes 59785854 jump ufw-reject-input
		counter packets 728388 bytes 59785854 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 0 bytes 0 accept
		counter packets 204973672 bytes 164016809502 jump ufw-before-logging-output
		counter packets 204973672 bytes 164016809502 jump ufw-before-output
		counter packets 719252 bytes 75011107 jump ufw-after-output
		counter packets 719252 bytes 75011107 jump ufw-after-logging-output
		counter packets 719252 bytes 75011107 jump ufw-reject-output
		counter packets 719252 bytes 75011107 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 1132470432 bytes 1395836320052 jump DOCKER-USER
		counter packets 1132470439 bytes 1395836321266 jump DOCKER-FORWARD
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 5326846 bytes 6509737846 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 6622427 bytes 2932184856 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 15320 bytes 955909 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 15320 bytes 955909 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 2018060 bytes 651575154 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 2907465 bytes 2312646256 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 2016219 bytes 651480187 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 260668 bytes 13830770 accept
		udp dport 46539 counter packets 1602208 bytes 2222582977 accept
		udp dport 39425 counter packets 625 bytes 62591 accept
		tcp dport 80 counter packets 235747 bytes 11725650 accept
		udp dport 1194 counter packets 228 bytes 9544 accept
		tcp dport 8443 counter packets 73504 bytes 4315721 accept
		udp dport 21194 counter packets 44 bytes 6736 accept
		iifname "tun79" counter packets 0 bytes 0 accept
		udp dport 33328 counter packets 1 bytes 438 accept
		tcp dport 52000 counter packets 633 bytes 32380 accept
		udp dport 51820 counter packets 1 bytes 436 accept
		tcp dport 25 counter packets 3572 bytes 197892 accept
	...
	chain DOCKER {
		ip daddr 172.29.172.2 iifname != "amn0" oifname "amn0" udp dport 39425 counter packets 19895 bytes 3610395 accept
		ip daddr 172.17.0.2 iifname != "docker0" oifname "docker0" tcp dport 443 counter packets 255092 bytes 15781139 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 48 | 2026-08-25 04:30:01 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-08-24 13:45:26 |
| `/root/cert` | directory | root:root | 755 | 4096 | 2025-03-17 17:20:58 |
| `/etc/x-ui` | directory | root:root | 755 | 4096 | 2026-08-25 04:30:10 |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 304K
drwxr-xr-x  2 root root   4.0K Aug 25 03:10 .
