...
Generated: 2026-09-13T11:07:03+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 11:07:04 up 42 days,  7:54,  1 user,  load average: 1.46, 1.61, 1.20
```
...
udp   UNCONN 0      0                  *:57521            *:*    users:(("rw-core",pid=2921938,fd=18))                      
udp   UNCONN 0      0              [::1]:8388          [::]:*    users:(("ss-server",pid=818092,fd=7))                      
udp   UNCONN 0      0                  *:37297            *:*    users:(("rw-core",pid=2921938,fd=35))                      
udp   UNCONN 0      0               [::]:39425         [::]:*    users:(("docker-proxy",pid=3859403,fd=7))                  
udp   UNCONN 0      0                  *:49727            *:*    users:(("rw-core",pid=2921938,fd=30))                      
udp   UNCONN 0      0                  *:41560            *:*    users:(("rw-core",pid=2921938,fd=29))                      
udp   UNCONN 0      0               [::]:51820         [::]:*                                                               
udp   UNCONN 0      0                  *:60715            *:*    users:(("rw-core",pid=2921938,fd=17))                      
udp   UNCONN 0      0                  *:54679            *:*    users:(("rw-core",pid=2921938,fd=34))                      
udp   UNCONN 0      0                  *:48582            *:*    users:(("rw-core",pid=2921938,fd=31))                      
...
tcp   LISTEN 0      4096         0.0.0.0:52000      0.0.0.0:*    users:(("sshd",pid=818111,fd=3),("systemd",pid=1,fd=122))  
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 43763 bytes 10367426 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 43763 bytes 3455990 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		...
		ct state related,established counter packets 134407293 bytes 128640718849 accept
		ct state invalid counter packets 888 bytes 246267 jump ufw-logging-deny
		ct state invalid counter packets 888 bytes 246267 drop
		...
		ip protocol icmp icmp type echo-request counter packets 5317 bytes 313681 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 2179406 bytes 683704559 jump ufw-not-local
		...
		counter packets 2179406 bytes 683704559 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 203166 bytes 86146464 accept
		ct state related,established counter packets 148856082 bytes 133146895929 accept
		counter packets 2614886 bytes 314662151 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		...
		tcp dport 445 counter packets 782 bytes 36876 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 763086 bytes 250291532 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 7 bytes 196 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 127163 bytes 38729090 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 43762 bytes 2697497 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 43763 bytes 3477215 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 1159780 bytes 69589836 accept
		ip protocol udp ct state new counter packets 1451547 bytes 244694984 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 17377 bytes 29555014 jump f2b-ufw-portscan
		iifname "tun79" counter packets 0 bytes 0 accept
		counter packets 445091811 bytes 451091328456 jump ufw-before-logging-input
		counter packets 445091811 bytes 451091328456 jump ufw-before-input
		counter packets 4904339 bytes 1288240331 jump ufw-after-input
		counter packets 1189757 bytes 86929973 jump ufw-after-logging-input
		counter packets 1189757 bytes 86929973 jump ufw-reject-input
		counter packets 1189757 bytes 86929973 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 0 bytes 0 accept
		counter packets 406893491 bytes 331457337393 jump ufw-before-logging-output
		counter packets 406893491 bytes 331457337393 jump ufw-before-output
		counter packets 3564085 bytes 407785107 jump ufw-after-output
		counter packets 3564085 bytes 407785107 jump ufw-after-logging-output
		counter packets 3564085 bytes 407785107 jump ufw-reject-output
		counter packets 3564085 bytes 407785107 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 1745008566 bytes 2079987704072 jump DOCKER-USER
		counter packets 1745008573 bytes 2079987705286 jump DOCKER-FORWARD
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 9148969 bytes 11376683261 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 11656797 bytes 5240882964 accept
	...
	chain DOCKER {
		ip daddr 172.29.172.2 iifname != "amn0" oifname "amn0" udp dport 39425 counter packets 13067 bytes 3718730 accept
		ip daddr 172.17.0.2 iifname != "docker0" oifname "docker0" tcp dport 443 counter packets 491303 bytes 30240618 accept
	...
	chain DOCKER-FORWARD {
		counter packets 1745008573 bytes 2079987705286 jump DOCKER-CT
		counter packets 882274746 bytes 1053792143251 jump DOCKER-INTERNAL
		counter packets 882274746 bytes 1053792143251 jump DOCKER-BRIDGE
		iifname "br-566ebeb0f4f1" counter packets 0 bytes 0 accept
		iifname "amn0" counter packets 635363167 bytes 893259507013 accept
		iifname "docker0" counter packets 225571770 bytes 143876119222 accept
	}

	chain DOCKER-BRIDGE {
		oifname "br-566ebeb0f4f1" counter packets 22 bytes 1320 jump DOCKER
		oifname "amn0" counter packets 42738 bytes 8718130 jump DOCKER
		oifname "docker0" counter packets 491303 bytes 30240618 jump DOCKER
	}

	chain DOCKER-CT {
		oifname "br-566ebeb0f4f1" ct state related,established counter packets 0 bytes 0 accept
		oifname "amn0" ct state related,established counter packets 652865144 bytes 866312015368 accept
		oifname "docker0" ct state related,established counter packets 209868683 bytes 159883546667 accept
	}
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 849 bytes 244707 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 849 bytes 244707 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 891312 bytes 289077438 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 1289101 bytes 394679819 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 890305 bytes 289024740 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 442952 bytes 26124310 accept
		udp dport 46539 counter packets 246425 bytes 330877908 accept
		udp dport 39425 counter packets 236 bytes 56623 accept
		tcp dport 80 counter packets 313921 bytes 18502987 accept
		udp dport 1194 counter packets 105 bytes 4612 accept
		tcp dport 8443 counter packets 76565 bytes 4507811 accept
		udp dport 21194 counter packets 3 bytes 452 accept
		iifname "tun79" counter packets 0 bytes 0 accept
		udp dport 33328 counter packets 0 bytes 0 accept
		tcp dport 52000 counter packets 476 bytes 27337 accept
		udp dport 51820 counter packets 0 bytes 0 accept
		tcp dport 25 counter packets 1176 bytes 64588 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 49 | 2026-09-13 11:07:07 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-09-13 11:01:33 |
...
drwxr-xr-x 17 root syslog 4.0K Sep 13 11:01 ..
