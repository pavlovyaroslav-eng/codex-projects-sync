...
Generated: 2026-09-13T09:21:10+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 09:21:10 up 42 days,  6:08,  1 user,  load average: 0.68, 0.52, 0.39
```
...
udp   UNCONN 0      0                  *:59047            *:*    users:(("rw-core",pid=2921938,fd=13))                      
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 43445 bytes 10306537 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 43445 bytes 3426766 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		...
		ct state related,established counter packets 133132802 bytes 127427320735 accept
		...
		ip protocol icmp icmp type echo-request counter packets 5304 bytes 313033 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 2162335 bytes 680233654 jump ufw-not-local
		...
		counter packets 2162335 bytes 680233654 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 203166 bytes 86146464 accept
		ct state related,established counter packets 147525421 bytes 131884153634 accept
		counter packets 2605068 bytes 312641959 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		...
		tcp dport 445 counter packets 776 bytes 36596 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 757530 bytes 248469164 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 7 bytes 196 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 126231 bytes 38437506 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 43445 bytes 2678803 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 43445 bytes 3449963 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 1156065 bytes 69366936 accept
		ip protocol udp ct state new counter packets 1445468 bytes 242900660 accept
	}
	...
	chain INPUT {
		...
		counter packets 443800225 bytes 449874447953 jump ufw-before-logging-input
		counter packets 443800225 bytes 449874447953 jump ufw-before-input
		counter packets 4896457 bytes 1286039234 jump ufw-after-input
		counter packets 1188369 bytes 86843108 jump ufw-after-logging-input
		counter packets 1188369 bytes 86843108 jump ufw-reject-input
		counter packets 1188369 bytes 86843108 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 0 bytes 0 accept
		counter packets 405553012 bytes 330192574906 jump ufw-before-logging-output
		counter packets 405553012 bytes 330192574906 jump ufw-before-output
		counter packets 3554267 bytes 405764915 jump ufw-after-output
		counter packets 3554267 bytes 405764915 jump ufw-after-logging-output
		counter packets 3554267 bytes 405764915 jump ufw-reject-output
		counter packets 3554267 bytes 405764915 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 1743957437 bytes 2079015035725 jump DOCKER-USER
		counter packets 1743957444 bytes 2079015036939 jump DOCKER-FORWARD
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 9139856 bytes 11367373541 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 11643215 bytes 5235244583 accept
	...
	chain DOCKER {
		ip daddr 172.29.172.2 iifname != "amn0" oifname "amn0" udp dport 39425 counter packets 12928 bytes 3705869 accept
		ip daddr 172.17.0.2 iifname != "docker0" oifname "docker0" tcp dport 443 counter packets 490592 bytes 30198222 accept
	...
	chain DOCKER-FORWARD {
		counter packets 1743957444 bytes 2079015036939 jump DOCKER-CT
		counter packets 881719157 bytes 1053295813495 jump DOCKER-INTERNAL
		counter packets 881719157 bytes 1053295813495 jump DOCKER-BRIDGE
		iifname "br-566ebeb0f4f1" counter packets 0 bytes 0 accept
		iifname "amn0" counter packets 635220802 bytes 893080826985 accept
		iifname "docker0" counter packets 225182091 bytes 143573472852 accept
	}

	chain DOCKER-BRIDGE {
		oifname "br-566ebeb0f4f1" counter packets 22 bytes 1320 jump DOCKER
		oifname "amn0" counter packets 42599 bytes 8705269 jump DOCKER
		oifname "docker0" counter packets 490592 bytes 30198222 jump DOCKER
	}

	chain DOCKER-CT {
		oifname "br-566ebeb0f4f1" ct state related,established counter packets 0 bytes 0 accept
		oifname "amn0" ct state related,established counter packets 652723222 bytes 866138724332 accept
		oifname "docker0" ct state related,established counter packets 209515065 bytes 159580499112 accept
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 884818 bytes 286963206 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 1278518 bytes 393322866 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 883817 bytes 286910788 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 440291 bytes 25967282 accept
		udp dport 46539 counter packets 245868 bytes 330117977 accept
		udp dport 39425 counter packets 231 bytes 56034 accept
		tcp dport 80 counter packets 308101 bytes 18159351 accept
		udp dport 1194 counter packets 105 bytes 4612 accept
		tcp dport 8443 counter packets 76457 bytes 4501419 accept
		udp dport 21194 counter packets 3 bytes 452 accept
		iifname "tun79" counter packets 0 bytes 0 accept
		udp dport 33328 counter packets 0 bytes 0 accept
		tcp dport 52000 counter packets 443 bytes 25357 accept
...
## Fail2Ban summary
```text
2026-09-13 09:21:14,951 fail2ban                [872478]: ERROR   Failed to access socket path: /var/run/fail2ban/fail2ban.sock. Is fail2ban running?
```
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 49 | 2026-09-13 09:21:03 |
