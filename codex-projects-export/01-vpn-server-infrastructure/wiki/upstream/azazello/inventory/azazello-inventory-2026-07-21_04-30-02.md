...
Generated: 2026-07-21T04:30:02+03:00  
Host: azazello  
FQDN: azazello  
...
Linux azazello 6.8.0-107-generic #107-Ubuntu SMP PREEMPT_DYNAMIC Fri Mar 13 19:51:50 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:30:02 up 44 days, 14:09,  0 user,  load average: 0.87, 0.59, 0.50
```
...
-A f2b-ufw-portscan -s 123.58.193.169/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 172.217.19.1/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 141.98.83.48/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 95.163.58.193/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 2.23.97.162/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 57.144.248.141/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 44.192.202.67/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 157.240.253.13/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 149.154.175.211/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 46.228.223.174/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 13.37.137.209/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 146.75.30.73/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 91.208.42.67/32 -j REJECT --reject-with icmp-port-unreachable
-A f2b-ufw-portscan -s 104.83.5.163/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 52.81.8.223/32 -j REJECT --reject-with icmp-port-unreachable
...
-A f2b-ufw-portscan -s 45.205.1.169/32 -j REJECT --reject-with icmp-port-unreachable
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 12227 bytes 4607110 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 12227 bytes 773791 log prefix "[UFW AUDIT] "
	}
	...
	chain ufw-before-input {
		iifname "lo" counter packets 23653448 bytes 12629647350 accept
		ct state related,established counter packets 344327703 bytes 235083424157 accept
		ct state invalid counter packets 24514 bytes 1998720 jump ufw-logging-deny
		ct state invalid counter packets 24514 bytes 1998720 drop
		...
		ip protocol icmp icmp type echo-request counter packets 46301 bytes 2671121 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 3935506 bytes 1247437586 jump ufw-not-local
		...
		counter packets 3935506 bytes 1247437586 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 23653452 bytes 12629647566 accept
		ct state related,established counter packets 426188042 bytes 493470804530 accept
		counter packets 2232898 bytes 278647245 jump ufw-user-output
	}
	...
	chain ufw-after-input {
		udp dport 137 counter packets 314 bytes 28910 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 3 bytes 2383 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 258 bytes 13512 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 1542 bytes 78104 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 1684990 bytes 552678764 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 3 bytes 2384 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 283007 bytes 85711254 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 12227 bytes 1484005 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 12227 bytes 856778 log prefix "[UFW ALLOW] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 374291 bytes 23969808 accept
		ip protocol udp ct state new counter packets 1815619 bytes 250473850 accept
	}
	...
	chain INPUT {
		...
		ip protocol tcp counter packets 9359303 bytes 8962226589 jump f2b-ufw-portscan
		iifname "tun79" counter packets 1090750 bytes 1359553160 accept
		counter packets 746062947 bytes 615404160537 jump ufw-before-logging-input
		counter packets 746062947 bytes 615404160537 jump ufw-before-input
		counter packets 6914859 bytes 1435227480 jump ufw-after-input
		counter packets 3023474 bytes 174352849 jump ufw-after-logging-input
		counter packets 3023474 bytes 174352849 jump ufw-reject-input
		counter packets 3023474 bytes 174352849 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		oifname "tun79" counter packets 31413 bytes 6024246 accept
		counter packets 770231142 bytes 784887026125 jump ufw-before-logging-output
		counter packets 770231142 bytes 784887026125 jump ufw-before-output
		counter packets 5736597 bytes 782174323 jump ufw-after-output
		counter packets 5736597 bytes 782174323 jump ufw-after-logging-output
		counter packets 5736597 bytes 782174323 jump ufw-reject-output
		counter packets 5736597 bytes 782174323 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		ip daddr 10.79.0.0/24 iifname "eth0" oifname "tun79" ct state related,established counter packets 156194378 bytes 368245873098 accept
		ip saddr 10.79.0.0/24 iifname "tun79" oifname "eth0" counter packets 187188609 bytes 40878897989 accept
		...
		counter packets 602429448 bytes 635043968185 jump DOCKER-USER
		counter packets 602429448 bytes 635043968185 jump DOCKER-FORWARD
	...
	chain DOCKER {
		ip daddr 172.29.172.2 iifname != "amn0" oifname "amn0" udp dport 39425 counter packets 8991 bytes 1631765 accept
		ip daddr 172.17.0.3 iifname != "docker0" oifname "docker0" tcp dport 443 counter packets 24749 bytes 1496414 accept
	...
	chain DOCKER-FORWARD {
		counter packets 602429448 bytes 635043968185 jump DOCKER-CT
		counter packets 302823449 bytes 314536050384 jump DOCKER-INTERNAL
		counter packets 302823449 bytes 314536050384 jump DOCKER-BRIDGE
		iifname "br-566ebeb0f4f1" counter packets 0 bytes 0 accept
		iifname "amn0" counter packets 222747294 bytes 281190724361 accept
		iifname "docker0" counter packets 79907219 bytes 33330026629 accept
	}

	chain DOCKER-BRIDGE {
		oifname "br-566ebeb0f4f1" counter packets 0 bytes 0 jump DOCKER
		oifname "amn0" counter packets 47077 bytes 7881658 jump DOCKER
		oifname "docker0" counter packets 121307 bytes 7386126 jump DOCKER
	}

	chain DOCKER-CT {
		oifname "br-566ebeb0f4f1" ct state related,established counter packets 0 bytes 0 accept
		oifname "amn0" ct state related,established counter packets 229657648 bytes 273160808716 accept
		oifname "docker0" ct state related,established counter packets 69948351 bytes 47347109085 accept
	}
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 2481 bytes 175093 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 2481 bytes 175093 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 1970117 bytes 638515311 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 1967372 bytes 609036600 return
		fib daddr type multicast counter packets 0 bytes 0 return
		fib daddr type broadcast counter packets 1968134 bytes 638400986 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 6582 bytes 389493 accept
		udp dport 46539 counter packets 139433 bytes 192965209 accept
		udp dport 39425 counter packets 103 bytes 9794 accept
		tcp dport 80 counter packets 25282 bytes 1510217 accept
		udp dport 1194 counter packets 38 bytes 4082 accept
		tcp dport 8443 counter packets 1047 bytes 59595 accept
		...
		tcp dport 52000 counter packets 123 bytes 6523 accept
		udp dport 51820 counter packets 0 bytes 0 accept
		tcp dport 25 counter packets 246 bytes 13120 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 43 | 2026-07-21 04:30:00 |
...
| `/etc/x-ui` | directory | root:root | 755 | 4096 | 2026-07-21 04:29:43 |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 112K
drwxr-xr-x  2 root root   4.0K Jul 21 03:10 .
