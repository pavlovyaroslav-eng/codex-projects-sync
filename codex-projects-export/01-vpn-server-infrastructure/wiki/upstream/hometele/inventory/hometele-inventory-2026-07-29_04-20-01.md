...
Generated: 2026-07-29T04:20:01+03:00  
Host: hometele  
...
Linux hometele 6.8.0-136-generic #136-Ubuntu SMP PREEMPT_DYNAMIC Wed Jul  1 21:53:05 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:02 up 11 days, 59 min,  1 user,  load average: 0.01, 0.05, 0.01
```
...
udp   UNCONN 0      0           10.8.1.2:60165      0.0.0.0:*    users:(("xray",pid=172876,fd=11))                                                                                  
udp   UNCONN 0      0           10.8.1.2:50082      0.0.0.0:*    users:(("xray",pid=172876,fd=101))                                                                                 
udp   UNCONN 0      0           10.8.1.2:33823      0.0.0.0:*    users:(("xray",pid=172876,fd=365))                                                                                 
udp   UNCONN 0      0           10.8.1.2:38954      0.0.0.0:*    users:(("xray",pid=172876,fd=55))                                                                                  
...
udp   UNCONN 0      0           10.8.1.2:55512      0.0.0.0:*    users:(("xray",pid=172876,fd=24))                                                                                  
udp   UNCONN 0      0           10.8.1.2:58807      0.0.0.0:*    users:(("xray",pid=172876,fd=209))                                                                                 
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 47709 bytes 5778972 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 47709 bytes 7937573 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 47677 bytes 2862657 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 17581973 bytes 8050071656 accept
		ct state related,established counter packets 287696566 bytes 353559248212 accept
		ct state invalid counter packets 115463 bytes 5866176 jump ufw-logging-deny
		ct state invalid counter packets 115463 bytes 5866176 drop
		...
		ip protocol icmp icmp type echo-request counter packets 3215 bytes 161791 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 2534867 bytes 271599252 jump ufw-not-local
		...
		counter packets 2534867 bytes 271599252 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 17581973 bytes 8050071656 accept
		ct state related,established counter packets 225549241 bytes 212792780999 accept
		counter packets 2303928 bytes 772041751 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 7741416 bytes 5169194368 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 4 bytes 112 accept
		counter packets 390236 bytes 23447381 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 54126 bytes 4221703 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 7059 bytes 1616424 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 103 bytes 4648 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 887 bytes 43136 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 62130 bytes 23812208 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 371547 bytes 124239989 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 47709 bytes 2298222 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 47709 bytes 11873873 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 47677 bytes 2862641 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 1577595 bytes 95481106 accept
		ip protocol udp ct state new counter packets 711252 bytes 674826538 accept
	}
	...
	chain INPUT {
		...
		counter packets 307932084 bytes 361886947087 jump ufw-before-logging-input
		counter packets 307932084 bytes 361886947087 jump ufw-before-input
		counter packets 1099812 bytes 186818783 jump ufw-after-input
		counter packets 603960 bytes 32880675 jump ufw-after-logging-input
		counter packets 603960 bytes 32880675 jump ufw-reject-input
		counter packets 603960 bytes 32880675 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 245435142 bytes 221614894406 jump ufw-before-logging-output
		counter packets 245435142 bytes 221614894406 jump ufw-before-output
		counter packets 2303928 bytes 772041751 jump ufw-after-output
		counter packets 2303928 bytes 772041751 jump ufw-after-logging-output
		counter packets 2303928 bytes 772041751 jump ufw-reject-output
		counter packets 2303928 bytes 772041751 jump ufw-track-output
	}
	...
	chain FORWARD {
		...
		counter packets 8131656 bytes 5192641861 jump ufw-before-logging-forward
		counter packets 8131656 bytes 5192641861 jump ufw-before-forward
		counter packets 390236 bytes 23447381 jump ufw-after-forward
		counter packets 390236 bytes 23447381 jump ufw-after-logging-forward
		counter packets 390236 bytes 23447381 jump ufw-reject-forward
		counter packets 390236 bytes 23447381 jump ufw-track-forward
		iifname "wg-home" counter packets 385883 bytes 23186040 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 22298 bytes 1279668 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 22298 bytes 1279668 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 495852 bytes 153938108 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 2032469 bytes 117471929 return
		fib daddr type multicast counter packets 7629 bytes 244128 return
		fib daddr type broadcast counter packets 494769 bytes 153883195 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 1422122 bytes 84016851 accept
		tcp dport 80 counter packets 12505 bytes 721568 accept
		tcp dport 52000 counter packets 181 bytes 10546 accept
		udp dport 21195 counter packets 0 bytes 0 accept
		udp dport 21196 counter packets 0 bytes 0 accept
		tcp dport 21197 counter packets 49 bytes 2720 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 43 | 2026-07-29 04:19:46 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-07-28 19:52:41 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 140K
drwxr-xr-x  2 root root   4.0K Jul 29 03:20 .
