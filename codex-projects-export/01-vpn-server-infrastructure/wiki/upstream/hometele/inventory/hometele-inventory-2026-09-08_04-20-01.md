...
Generated: 2026-09-08T04:20:01+03:00  
Host: hometele  
...
Linux hometele 6.8.0-139-generic #139-Ubuntu SMP PREEMPT_DYNAMIC Sat Aug  1 03:52:05 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:02 up 2 days, 59 min,  0 user,  load average: 0.08, 0.05, 0.01
```
...
udp   UNCONN 0      0                  *:52881            *:*    users:(("rw-core",pid=252273,fd=13))                                                                            
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 8829 bytes 1199403 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 8829 bytes 859082 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 8829 bytes 530076 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 340521 bytes 95910883 accept
		ct state related,established counter packets 37736441 bytes 27495426642 accept
		ct state invalid counter packets 6077 bytes 317167 jump ufw-logging-deny
		ct state invalid counter packets 6077 bytes 317167 drop
		...
		ip protocol icmp icmp type echo-request counter packets 743 bytes 35978 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 309745 bytes 52947629 jump ufw-not-local
		...
		counter packets 309745 bytes 52947629 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 340521 bytes 95910883 accept
		ct state related,established counter packets 32688201 bytes 28395456413 accept
		counter packets 590445 bytes 62667901 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 1300167 bytes 931998753 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 0 bytes 0 accept
		counter packets 65403 bytes 3926596 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 9186 bytes 716508 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 841 bytes 192562 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 40 bytes 1872 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 158 bytes 7788 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 8260 bytes 2544044 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 1 bytes 28 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 57189 bytes 20705747 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 8829 bytes 404016 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 8829 bytes 898194 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 8829 bytes 530076 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 368996 bytes 22140770 accept
		ip protocol udp ct state new counter packets 217882 bytes 39157018 accept
	}
	...
	chain INPUT {
		...
		counter packets 38393527 bytes 27644638299 jump ufw-before-logging-input
		counter packets 38393527 bytes 27644638299 jump ufw-before-input
		counter packets 205378 bytes 30308448 jump ufw-after-input
		counter packets 129703 bytes 6139899 jump ufw-after-logging-input
		counter packets 129703 bytes 6139899 jump ufw-reject-input
		counter packets 129703 bytes 6139899 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 33619167 bytes 28554035197 jump ufw-before-logging-output
		counter packets 33619167 bytes 28554035197 jump ufw-before-output
		counter packets 590445 bytes 62667901 jump ufw-after-output
		counter packets 590445 bytes 62667901 jump ufw-after-logging-output
		counter packets 590445 bytes 62667901 jump ufw-reject-output
		counter packets 590445 bytes 62667901 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 1639659 bytes 1029818496 jump DOCKER-USER
		counter packets 1639659 bytes 1029818496 jump DOCKER-FORWARD
		...
		counter packets 1365570 bytes 935925349 jump ufw-before-logging-forward
		counter packets 1365570 bytes 935925349 jump ufw-before-forward
		counter packets 65403 bytes 3926596 jump ufw-after-forward
		counter packets 65403 bytes 3926596 jump ufw-after-logging-forward
		counter packets 65403 bytes 3926596 jump ufw-reject-forward
		counter packets 65403 bytes 3926596 jump ufw-track-forward
		iifname "wg-home" counter packets 65403 bytes 3926596 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 2804 bytes 153510 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 2804 bytes 153510 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 75675 bytes 24168549 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 232878 bytes 28745494 return
		fib daddr type multicast counter packets 1411 bytes 45152 return
		fib daddr type broadcast counter packets 75456 bytes 24156983 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 25217 bytes 1503904 accept
		tcp dport 80 counter packets 50978 bytes 2719937 accept
		tcp dport 52000 counter packets 137 bytes 7956 accept
		udp dport 21195 counter packets 0 bytes 0 accept
		udp dport 21196 counter packets 0 bytes 0 accept
		tcp dport 21197 counter packets 6 bytes 324 accept
		...
		ip saddr 93.183.106.203 tcp dport 2222 counter packets 347 bytes 20820 accept
		udp dport 443 counter packets 27672 bytes 18383962 accept
	}
...
headscale   headscale/headscale:0.29.3   Up 2 days (healthy)   127.0.0.1:8081->8080/tcp, 127.0.0.1:9091->9090/tcp
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 48 | 2026-09-08 04:19:46 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-09-08 01:19:46 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 336K
drwxr-xr-x  2 root root   4.0K Sep  8 03:20 .
