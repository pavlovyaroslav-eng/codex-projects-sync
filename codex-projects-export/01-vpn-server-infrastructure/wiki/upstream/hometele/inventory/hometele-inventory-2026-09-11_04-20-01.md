...
Generated: 2026-09-11T04:20:01+03:00  
Host: hometele  
...
udp   UNCONN 0      0                  *:49597            *:*    users:(("rw-core",pid=252273,fd=16))                                                                            
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 21789 bytes 2702966 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 21789 bytes 1952983 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 21789 bytes 1308204 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 46938615 bytes 32285008818 accept
		ct state related,established counter packets 139877933 bytes 99819828809 accept
		ct state invalid counter packets 13101 bytes 784606 jump ufw-logging-deny
		ct state invalid counter packets 13101 bytes 784606 drop
		...
		ip protocol icmp icmp type echo-request counter packets 1618 bytes 79664 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 1231423 bytes 152292695 jump ufw-not-local
		...
		counter packets 1231423 bytes 152292695 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 46938615 bytes 32285008818 accept
		ct state related,established counter packets 112448084 bytes 102278191426 accept
		counter packets 1320470 bytes 164687174 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 3221747 bytes 2285960190 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 0 bytes 0 accept
		counter packets 161786 bytes 9713000 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 33131 bytes 2584218 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 1884 bytes 430842 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 86 bytes 4124 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 533 bytes 25244 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 20401 bytes 6283400 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 4 bytes 112 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 139371 bytes 50781419 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 21789 bytes 1010294 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 21789 bytes 2514631 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 21789 bytes 1308204 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 764732 bytes 45903177 accept
		ip protocol udp ct state new counter packets 540605 bytes 112774755 accept
	}
	...
	chain INPUT {
		...
		counter packets 188062690 bytes 132257994592 jump ufw-before-logging-input
		counter packets 188062690 bytes 132257994592 jump ufw-before-input
		counter packets 465374 bytes 73050722 jump ufw-after-input
		counter packets 269964 bytes 12941363 jump ufw-after-logging-input
		counter packets 269964 bytes 12941363 jump ufw-reject-input
		counter packets 269964 bytes 12941363 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 160707169 bytes 134727887418 jump ufw-before-logging-output
		counter packets 160707169 bytes 134727887418 jump ufw-before-output
		counter packets 1320470 bytes 164687174 jump ufw-after-output
		counter packets 1320470 bytes 164687174 jump ufw-after-logging-output
		counter packets 1320470 bytes 164687174 jump ufw-reject-output
		counter packets 1320470 bytes 164687174 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 4062898 bytes 2527678679 jump DOCKER-USER
		counter packets 4062898 bytes 2527678679 jump DOCKER-FORWARD
		...
		counter packets 3383533 bytes 2295673190 jump ufw-before-logging-forward
		counter packets 3383533 bytes 2295673190 jump ufw-before-forward
		counter packets 161786 bytes 9713000 jump ufw-after-forward
		counter packets 161786 bytes 9713000 jump ufw-after-logging-forward
		counter packets 161786 bytes 9713000 jump ufw-reject-forward
		counter packets 161786 bytes 9713000 jump ufw-track-forward
		iifname "wg-home" counter packets 161786 bytes 9713000 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 6581 bytes 381976 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 6581 bytes 381976 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 195410 bytes 60109359 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 1033206 bytes 92106664 return
		fib daddr type multicast counter packets 3485 bytes 111520 return
		fib daddr type broadcast counter packets 194732 bytes 60074511 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 442245 bytes 24903783 accept
		tcp dport 80 counter packets 262566 bytes 14644764 accept
		tcp dport 52000 counter packets 224 bytes 12956 accept
		udp dport 21195 counter packets 0 bytes 0 accept
		udp dport 21196 counter packets 0 bytes 0 accept
		tcp dport 21197 counter packets 34 bytes 1936 accept
		...
		ip saddr 93.183.106.203 tcp dport 2222 counter packets 872 bytes 52320 accept
		udp dport 443 counter packets 60097 bytes 39623760 accept
	}
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 48 | 2026-09-11 04:19:56 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-09-10 15:47:23 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
