...
Generated: 2026-09-15T18:15:32+03:00  
Host: hometele  
...
Linux hometele 6.8.0-139-generic #139-Ubuntu SMP PREEMPT_DYNAMIC Sat Aug  1 03:52:05 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 18:15:32 up 9 days, 14:55,  1 user,  load average: 0.07, 0.07, 0.08
```
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 41575 bytes 5786090 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 41575 bytes 3541556 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 41575 bytes 2496004 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 94115556 bytes 76961284805 accept
		ct state related,established counter packets 254563692 bytes 204877554915 accept
		ct state invalid counter packets 22794 bytes 1398192 jump ufw-logging-deny
		ct state invalid counter packets 22794 bytes 1398192 drop
		...
		ip protocol icmp icmp type echo-request counter packets 2932 bytes 158087 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 2003448 bytes 315318532 jump ufw-not-local
		...
		counter packets 2003448 bytes 315318532 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 94115556 bytes 76961284805 accept
		ct state related,established counter packets 202717919 bytes 209282667201 accept
		counter packets 2850377 bytes 333690558 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 6519107 bytes 4679028888 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 0 bytes 0 accept
		counter packets 308956 bytes 18548400 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 91730 bytes 7154940 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 3376 bytes 772402 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 140 bytes 6680 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 982 bytes 45724 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 39201 bytes 12142842 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 9 bytes 252 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 266518 bytes 96921219 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 41575 bytes 1969946 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 41575 bytes 4577733 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 41575 bytes 2496004 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 1747135 bytes 104912199 accept
		ip protocol udp ct state new counter packets 1071922 bytes 216159573 accept
	}
	...
	chain INPUT {
		...
		counter packets 350708422 bytes 282155714531 jump ufw-before-logging-input
		counter packets 350708422 bytes 282155714531 jump ufw-before-input
		counter packets 901380 bytes 163933071 jump ufw-after-input
		counter packets 499424 bytes 46889012 jump ufw-after-logging-input
		counter packets 499424 bytes 46889012 jump ufw-reject-input
		counter packets 499424 bytes 46889012 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 299683852 bytes 286577642564 jump ufw-before-logging-output
		counter packets 299683852 bytes 286577642564 jump ufw-before-output
		counter packets 2850377 bytes 333690558 jump ufw-after-output
		counter packets 2850377 bytes 333690558 jump ufw-after-logging-output
		counter packets 2850377 bytes 333690558 jump ufw-reject-output
		counter packets 2850377 bytes 333690558 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 8126074 bytes 5140901565 jump DOCKER-USER
		counter packets 8126074 bytes 5140901565 jump DOCKER-FORWARD
		...
		counter packets 6828063 bytes 4697577288 jump ufw-before-logging-forward
		counter packets 6828063 bytes 4697577288 jump ufw-before-forward
		counter packets 308956 bytes 18548400 jump ufw-after-forward
		counter packets 308956 bytes 18548400 jump ufw-after-logging-forward
		counter packets 308956 bytes 18548400 jump ufw-reject-forward
		counter packets 308956 bytes 18548400 jump ufw-track-forward
		iifname "wg-home" counter packets 308956 bytes 18548400 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 12595 bytes 748985 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 12595 bytes 748985 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 401956 bytes 117044059 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 1596077 bytes 198124985 return
		fib daddr type multicast counter packets 6650 bytes 212800 return
		fib daddr type broadcast counter packets 400721 bytes 116980747 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 676376 bytes 38982464 accept
		tcp dport 80 counter packets 294576 bytes 15964155 accept
		tcp dport 52000 counter packets 572 bytes 33628 accept
		...
		ip saddr 93.183.106.203 tcp dport 2222 counter packets 1739 bytes 104340 accept
		udp dport 443 counter packets 128719 bytes 96292398 accept
	}
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 49 | 2026-09-15 18:15:18 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-09-15 17:04:17 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
