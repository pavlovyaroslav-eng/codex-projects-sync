...
Generated: 2026-08-03T04:20:01+03:00  
Host: hometele  
...
Linux hometele 6.8.0-136-generic #136-Ubuntu SMP PREEMPT_DYNAMIC Wed Jul  1 21:53:05 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:01 up 16 days, 59 min,  1 user,  load average: 0.01, 0.03, 0.00
```
...
udp   UNCONN 0      0           10.8.1.2:41606      0.0.0.0:*    users:(("xray",pid=261657,fd=217))                                                                                 
udp   UNCONN 0      0           10.8.1.2:37548      0.0.0.0:*    users:(("xray",pid=261657,fd=301))                                                                                 
...
udp   UNCONN 0      0           10.8.1.2:55222      0.0.0.0:*    users:(("xray",pid=261657,fd=135))                                                                                 
...
udp   UNCONN 0      0           10.8.1.2:56394      0.0.0.0:*    users:(("xray",pid=261657,fd=303))                                                                                 
udp   UNCONN 0      0           10.8.1.2:52324      0.0.0.0:*    users:(("xray",pid=261657,fd=266))                                                                                 
udp   UNCONN 0      0           10.8.1.2:59517      0.0.0.0:*    users:(("xray",pid=261657,fd=279))                                                                                 
udp   UNCONN 0      0           10.8.1.2:32908      0.0.0.0:*    users:(("xray",pid=261657,fd=200))                                                                                 
udp   UNCONN 0      0           10.8.1.2:39158      0.0.0.0:*    users:(("xray",pid=261657,fd=275))                                                                                 
udp   UNCONN 0      0           10.8.1.2:38158      0.0.0.0:*    users:(("xray",pid=261657,fd=300))                                                                                 
udp   UNCONN 0      0           10.8.1.2:56665      0.0.0.0:*    users:(("xray",pid=261657,fd=245))                                                                                 
...
udp   UNCONN 0      0           10.8.1.2:50701      0.0.0.0:*    users:(("xray",pid=261657,fd=282))                                                                                 
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 69309 bytes 8378803 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 69309 bytes 11508505 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 69277 bytes 4159601 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 29980119 bytes 13762334694 accept
		ct state related,established counter packets 448494210 bytes 548275252699 accept
		ct state invalid counter packets 148445 bytes 7531510 jump ufw-logging-deny
		ct state invalid counter packets 148445 bytes 7531510 drop
		...
		ip protocol icmp icmp type echo-request counter packets 4528 bytes 226776 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 3938441 bytes 402872576 jump ufw-not-local
		...
		counter packets 3938441 bytes 402872576 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 29980119 bytes 13762334694 accept
		ct state related,established counter packets 356144395 bytes 329913300502 accept
		counter packets 3544334 bytes 1161726518 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 10732997 bytes 7114739208 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 4 bytes 112 accept
		counter packets 560809 bytes 33687505 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 60188 bytes 4694509 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 10903 bytes 2496922 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 158 bytes 7184 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 1301 bytes 63016 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 82355 bytes 30041508 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 511296 bytes 174822415 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 69309 bytes 3363909 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 69309 bytes 17277326 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 69277 bytes 4159585 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 2453742 bytes 148856353 accept
		ip protocol udp ct state new counter packets 1066925 bytes 1010134045 accept
	}
	...
	chain INPUT {
		...
		counter packets 482565743 bytes 562448218255 jump ufw-before-logging-input
		counter packets 482565743 bytes 562448218255 jump ufw-before-input
		counter packets 1500564 bytes 260421494 jump ufw-after-input
		counter packets 834363 bytes 48295940 jump ufw-after-logging-input
		counter packets 834363 bytes 48295940 jump ufw-reject-input
		counter packets 834363 bytes 48295940 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 389668848 bytes 344837361714 jump ufw-before-logging-output
		counter packets 389668848 bytes 344837361714 jump ufw-before-output
		counter packets 3544334 bytes 1161726518 jump ufw-after-output
		counter packets 3544334 bytes 1161726518 jump ufw-after-logging-output
		counter packets 3544334 bytes 1161726518 jump ufw-reject-output
		counter packets 3544334 bytes 1161726518 jump ufw-track-output
	}
	...
	chain FORWARD {
		...
		counter packets 11293810 bytes 7148426825 jump ufw-before-logging-forward
		counter packets 11293810 bytes 7148426825 jump ufw-before-forward
		counter packets 560809 bytes 33687505 jump ufw-after-forward
		counter packets 560809 bytes 33687505 jump ufw-after-logging-forward
		counter packets 560809 bytes 33687505 jump ufw-reject-forward
		counter packets 560809 bytes 33687505 jump ufw-track-forward
		iifname "wg-home" counter packets 556456 bytes 33426164 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 32238 bytes 1836358 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 32238 bytes 1836358 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 666201 bytes 212125554 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 3262757 bytes 190473363 return
		fib daddr type multicast counter packets 11082 bytes 354624 return
		fib daddr type broadcast counter packets 664602 bytes 212044589 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 2419999 bytes 141425114 accept
		tcp dport 80 counter packets 17357 bytes 978580 accept
		tcp dport 52000 counter packets 261 bytes 15156 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 45 | 2026-08-03 04:19:50 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-08-02 22:03:11 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 164K
drwxr-xr-x  2 root root   4.0K Aug  3 03:20 .
