...
Generated: 2026-08-04T04:20:01+03:00  
Host: hometele  
...
Linux hometele 6.8.0-136-generic #136-Ubuntu SMP PREEMPT_DYNAMIC Wed Jul  1 21:53:05 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:01 up 17 days, 59 min,  1 user,  load average: 0.00, 0.00, 0.00
```
...
udp   UNCONN 0      0           10.8.1.2:56006      0.0.0.0:*    users:(("xray",pid=261657,fd=60))                                                                                  
...
udp   UNCONN 0      0           10.8.1.2:40654      0.0.0.0:*    users:(("xray",pid=261657,fd=302))                                                                                 
...
udp   UNCONN 0      0           10.8.1.2:48875      0.0.0.0:*    users:(("xray",pid=261657,fd=611))                                                                                 
udp   UNCONN 0      0           10.8.1.2:41827      0.0.0.0:*    users:(("xray",pid=261657,fd=325))                                                                                 
udp   UNCONN 0      0           10.8.1.2:36728      0.0.0.0:*    users:(("xray",pid=261657,fd=283))                                                                                 
udp   UNCONN 0      0            0.0.0.0:58240      0.0.0.0:*    users:(("openvpn",pid=261409,fd=3))                                                                                
udp   UNCONN 0      0           10.8.1.2:52119      0.0.0.0:*    users:(("xray",pid=261657,fd=414))                                                                                 
udp   UNCONN 0      0           10.8.1.2:41979      0.0.0.0:*    users:(("xray",pid=261657,fd=328))                                                                                 
...
udp   UNCONN 0      0           10.8.1.2:46189      0.0.0.0:*    users:(("xray",pid=261657,fd=389))                                                                                 
udp   UNCONN 0      0           10.8.1.2:50359      0.0.0.0:*    users:(("xray",pid=261657,fd=394))                                                                                 
udp   UNCONN 0      0           10.8.1.2:59655      0.0.0.0:*    users:(("xray",pid=261657,fd=301))                                                                                 
udp   UNCONN 0      0           10.8.1.2:38330      0.0.0.0:*    users:(("xray",pid=261657,fd=566))                                                                                 
udp   UNCONN 0      0           10.8.1.2:45506      0.0.0.0:*    users:(("xray",pid=261657,fd=592))                                                                                 
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 73629 bytes 8877516 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 73629 bytes 12335070 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 70528 bytes 4235701 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 31358251 bytes 14382774144 accept
		ct state related,established counter packets 473397597 bytes 578328252279 accept
		ct state invalid counter packets 152102 bytes 7755159 jump ufw-logging-deny
		ct state invalid counter packets 152102 bytes 7755159 drop
		...
		ip protocol icmp icmp type echo-request counter packets 4851 bytes 244124 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 4135808 bytes 423864275 jump ufw-not-local
		...
		counter packets 4135808 bytes 423864275 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 31358251 bytes 14382774144 accept
		ct state related,established counter packets 376457677 bytes 347938093648 accept
		counter packets 3827479 bytes 1252838264 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 11440055 bytes 7606829198 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 4 bytes 112 accept
		counter packets 567540 bytes 34092501 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 64437 bytes 5025931 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 11566 bytes 2648749 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 161 bytes 7308 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 1369 bytes 66344 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 86401 bytes 31287676 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 537932 bytes 184723828 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 73629 bytes 3581440 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 73629 bytes 18550654 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 70528 bytes 4235685 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 2655150 bytes 161031117 accept
		ip protocol udp ct state new counter packets 1146334 bytes 1088842944 accept
	}
	...
	chain INPUT {
		...
		counter packets 509048609 bytes 593142889981 jump ufw-before-logging-input
		counter packets 509048609 bytes 593142889981 jump ufw-before-input
		counter packets 1586762 bytes 274646664 jump ufw-after-input
		counter packets 884896 bytes 50886828 jump ufw-after-logging-input
		counter packets 884896 bytes 50886828 jump ufw-reject-input
		counter packets 884896 bytes 50886828 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 411643407 bytes 363573706056 jump ufw-before-logging-output
		counter packets 411643407 bytes 363573706056 jump ufw-before-output
		counter packets 3827479 bytes 1252838264 jump ufw-after-output
		counter packets 3827479 bytes 1252838264 jump ufw-after-logging-output
		counter packets 3827479 bytes 1252838264 jump ufw-reject-output
		counter packets 3827479 bytes 1252838264 jump ufw-track-output
	}
	...
	chain FORWARD {
		...
		counter packets 12007599 bytes 7640921811 jump ufw-before-logging-forward
		counter packets 12007599 bytes 7640921811 jump ufw-before-forward
		counter packets 567540 bytes 34092501 jump ufw-after-forward
		counter packets 567540 bytes 34092501 jump ufw-after-logging-forward
		counter packets 567540 bytes 34092501 jump ufw-reject-forward
		counter packets 567540 bytes 34092501 jump ufw-track-forward
		iifname "wg-home" counter packets 563187 bytes 33831160 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 34320 bytes 1937472 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 34320 bytes 1937472 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 701866 bytes 223759836 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 3423844 bytes 199812510 return
		fib daddr type multicast counter packets 11773 bytes 376736 return
		fib daddr type broadcast counter packets 700191 bytes 223675029 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 2530733 bytes 148167943 accept
		tcp dport 80 counter packets 17786 bytes 1001960 accept
		tcp dport 52000 counter packets 265 bytes 15396 accept
		udp dport 21195 counter packets 0 bytes 0 accept
		udp dport 21196 counter packets 0 bytes 0 accept
		tcp dport 21197 counter packets 64 bytes 3528 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 45 | 2026-08-04 04:19:37 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-08-04 01:59:49 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 168K
drwxr-xr-x  2 root root   4.0K Aug  4 03:20 .
