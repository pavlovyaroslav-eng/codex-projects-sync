...
Generated: 2026-09-06T04:20:01+03:00  
Host: hometele  
...
Linux hometele 6.8.0-139-generic #139-Ubuntu SMP PREEMPT_DYNAMIC Sat Aug  1 03:52:05 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:02 up 59 min,  0 user,  load average: 0.07, 0.02, 0.00
```
...
udp   UNCONN 0      0                  *:59368            *:*    users:(("rw-core",pid=2222,fd=9))                                                                  
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 189 bytes 25607 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 189 bytes 13481 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 189 bytes 11340 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 4226 bytes 412892 accept
		ct state related,established counter packets 211759 bytes 182011048 accept
		ct state invalid counter packets 56 bytes 2916 jump ufw-logging-deny
		ct state invalid counter packets 56 bytes 2916 drop
		...
		ip protocol icmp icmp type echo-request counter packets 5 bytes 217 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 4541 bytes 967952 jump ufw-not-local
		...
		counter packets 4541 bytes 967952 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 4226 bytes 412892 accept
		ct state related,established counter packets 183434 bytes 188047989 accept
		counter packets 5214 bytes 417408 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 60720 bytes 47031651 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 0 bytes 0 accept
		counter packets 1366 bytes 82056 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 1 bytes 78 jump ufw-skip-to-policy-input
		...
		tcp dport 445 counter packets 2 bytes 96 jump ufw-skip-to-policy-input
		...
		fib daddr type broadcast counter packets 1161 bytes 415588 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 189 bytes 8424 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 189 bytes 14952 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 189 bytes 11340 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 2056 bytes 123360 accept
		ip protocol udp ct state new counter packets 3114 bytes 279266 accept
	}
	...
	chain INPUT {
		...
		counter packets 220587 bytes 183395025 jump ufw-before-logging-input
		counter packets 220587 bytes 183395025 jump ufw-before-input
		counter packets 3723 bytes 579963 jump ufw-after-input
		counter packets 2371 bytes 107877 jump ufw-after-logging-input
		counter packets 2371 bytes 107877 jump ufw-reject-input
		counter packets 2371 bytes 107877 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 192874 bytes 188878289 jump ufw-before-logging-output
		counter packets 192874 bytes 188878289 jump ufw-before-output
		counter packets 5214 bytes 417408 jump ufw-after-output
		counter packets 5214 bytes 417408 jump ufw-after-logging-output
		counter packets 5214 bytes 417408 jump ufw-reject-output
		counter packets 5214 bytes 417408 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 67742 bytes 49067503 jump DOCKER-USER
		counter packets 67742 bytes 49067503 jump DOCKER-FORWARD
		...
		counter packets 62086 bytes 47113707 jump ufw-before-logging-forward
		counter packets 62086 bytes 47113707 jump ufw-before-forward
		counter packets 1366 bytes 82056 jump ufw-after-forward
		counter packets 1366 bytes 82056 jump ufw-after-logging-forward
		counter packets 1366 bytes 82056 jump ufw-reject-forward
		counter packets 1366 bytes 82056 jump ufw-track-forward
		iifname "wg-home" counter packets 1366 bytes 82056 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 56 bytes 2916 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 56 bytes 2916 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 1352 bytes 472086 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 3163 bytes 495112 return
		fib daddr type multicast counter packets 29 bytes 928 return
		fib daddr type broadcast counter packets 1349 bytes 471912 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 94 bytes 5536 accept
		tcp dport 80 counter packets 300 bytes 12360 accept
		tcp dport 52000 counter packets 3 bytes 168 accept
		...
		ip saddr 93.183.106.203 tcp dport 2222 counter packets 16 bytes 960 accept
		udp dport 443 counter packets 396 bytes 367797 accept
	}
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 46 | 2026-09-06 04:19:44 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-09-06 01:06:49 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
...
drwxrwxr-x 15 root syslog 4.0K Sep  6 03:20 ..
