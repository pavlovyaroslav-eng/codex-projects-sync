...
Generated: 2026-09-13T09:20:51+03:00  
Host: hometele  
...
Linux hometele 6.8.0-139-generic #139-Ubuntu SMP PREEMPT_DYNAMIC Sat Aug  1 03:52:05 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 09:20:52 up 7 days,  6:00,  1 user,  load average: 0.09, 0.03, 0.01
```
...
udp   UNCONN 0      0                  *:40780            *:*    users:(("rw-core",pid=252273,fd=47))                                                                               
udp   UNCONN 0      0                  *:44903            *:*    users:(("rw-core",pid=252273,fd=26))                                                                               
udp   UNCONN 0      0                  *:48285            *:*    users:(("rw-core",pid=252273,fd=32))                                                                               
udp   UNCONN 0      0                  *:38064            *:*    users:(("rw-core",pid=252273,fd=9))                                                                                
udp   UNCONN 0      0                  *:51385            *:*    users:(("rw-core",pid=252273,fd=16))                                                                               
udp   UNCONN 0      0               [::]:59810         [::]:*    users:(("amneziawg-go",pid=905,fd=15))                                                                             
udp   UNCONN 0      0                  *:443              *:*    users:(("rw-core",pid=252273,fd=7))                                                                                
udp   UNCONN 0      0                  *:53745            *:*    users:(("rw-core",pid=252273,fd=43))                                                                               
udp   UNCONN 0      0                  *:41483            *:*    users:(("rw-core",pid=252273,fd=40))                                                                               
udp   UNCONN 0      0                  *:58896            *:*    users:(("rw-core",pid=252273,fd=48))                                                                               
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 31331 bytes 4023132 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 31331 bytes 2763895 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 31331 bytes 1881028 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 66501141 bytes 45764133529 accept
		ct state related,established counter packets 187260480 bytes 134522831280 accept
		ct state invalid counter packets 17569 bytes 1061586 jump ufw-logging-deny
		ct state invalid counter packets 17569 bytes 1061586 drop
		...
		ip protocol icmp icmp type echo-request counter packets 2283 bytes 116732 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 1531327 bytes 205078890 jump ufw-not-local
		...
		counter packets 1531327 bytes 205078890 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 66501141 bytes 45764133529 accept
		ct state related,established counter packets 150813458 bytes 136938092507 accept
		counter packets 1740554 bytes 210872670 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 4847347 bytes 3476287132 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 0 bytes 0 accept
		counter packets 232629 bytes 13966108 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 59998 bytes 4679844 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 2612 bytes 597500 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 116 bytes 5564 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 741 bytes 34824 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 29340 bytes 9036504 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 6 bytes 168 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 200722 bytes 73076375 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 31331 bytes 1475907 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 31331 bytes 3545391 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 31331 bytes 1881028 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 999033 bytes 59983771 accept
		ip protocol udp ct state new counter packets 721186 bytes 142847801 accept
	}
	...
	chain INPUT {
		...
		counter packets 255312800 bytes 180493222017 jump ufw-before-logging-input
		counter packets 255312800 bytes 180493222017 jump ufw-before-input
		counter packets 638834 bytes 104155767 jump ufw-after-input
		counter packets 345299 bytes 16724988 jump ufw-after-logging-input
		counter packets 345299 bytes 16724988 jump ufw-reject-input
		counter packets 345299 bytes 16724988 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 219055153 bytes 182913098706 jump ufw-before-logging-output
		counter packets 219055153 bytes 182913098706 jump ufw-before-output
		counter packets 1740554 bytes 210872670 jump ufw-after-output
		counter packets 1740554 bytes 210872670 jump ufw-after-logging-output
		counter packets 1740554 bytes 210872670 jump ufw-reject-output
		counter packets 1740554 bytes 210872670 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 6058484 bytes 3823854567 jump DOCKER-USER
		counter packets 6058484 bytes 3823854567 jump DOCKER-FORWARD
		...
		counter packets 5079976 bytes 3490253240 jump ufw-before-logging-forward
		counter packets 5079976 bytes 3490253240 jump ufw-before-forward
		counter packets 232629 bytes 13966108 jump ufw-after-forward
		counter packets 232629 bytes 13966108 jump ufw-after-logging-forward
		counter packets 232629 bytes 13966108 jump ufw-reject-forward
		counter packets 232629 bytes 13966108 jump ufw-track-forward
		iifname "wg-home" counter packets 232629 bytes 13966108 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 9259 bytes 540300 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 9259 bytes 540300 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 293535 bytes 87430779 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 1233730 bytes 117536931 return
		fib daddr type multicast counter packets 5011 bytes 160352 return
		fib daddr type broadcast counter packets 292586 bytes 87381607 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 530338 bytes 30209357 accept
		tcp dport 80 counter packets 278288 bytes 15292120 accept
		tcp dport 52000 counter packets 446 bytes 26104 accept
		...
		ip saddr 93.183.106.203 tcp dport 2222 counter packets 1306 bytes 78360 accept
		udp dport 443 counter packets 82055 bytes 55311944 accept
	}
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 49 | 2026-09-13 09:20:33 |
