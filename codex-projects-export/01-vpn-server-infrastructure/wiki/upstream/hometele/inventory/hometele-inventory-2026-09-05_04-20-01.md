...
Generated: 2026-09-05T04:20:01+03:00  
Host: hometele  
...
Linux hometele 6.8.0-138-generic #138-Ubuntu SMP PREEMPT_DYNAMIC Fri Jul 31 22:41:49 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:01 up 15 days, 59 min,  0 user,  load average: 0.05, 0.04, 0.00
```
...
veth338f68c@if2  UP             fe80::b4de:33ff:fedd:990b/64 
```
...
fe80::/64 dev veth338f68c proto kernel metric 256 pref medium
```
...
udp   UNCONN 0      0                  *:443              *:*    users:(("rw-core",pid=443209,fd=7))                                                                                
udp   UNCONN 0      0               [::]:51820         [::]:*                                                                                                                       
udp   UNCONN 0      0               [::]:37619         [::]:*    users:(("amneziawg-go",pid=837,fd=15))                                                                             
udp   UNCONN 0      0                  *:59217            *:*    users:(("rw-core",pid=443209,fd=9))                                                                                
...
tcp   LISTEN 0      4096       127.0.0.1:8081       0.0.0.0:*    users:(("docker-proxy",pid=442954,fd=8))                                                                           
tcp   LISTEN 0      4096       127.0.0.1:9091       0.0.0.0:*    users:(("docker-proxy",pid=442982,fd=8))                                                                           
...
tcp   LISTEN 0      4096         0.0.0.0:52000      0.0.0.0:*    users:(("sshd",pid=393650,fd=3),("systemd",pid=1,fd=156))                                                          
...
tcp   LISTEN 0      511                *:2222             *:*    users:(("rw-node",pid=443092,fd=21))                                                                               
tcp   LISTEN 0      511             [::]:80            [::]:*    users:(("nginx",pid=245830,fd=8),("nginx",pid=245829,fd=8),("nginx",pid=245828,fd=8),("nginx",pid=216070,fd=8))    
tcp   LISTEN 0      4096            [::]:52000         [::]:*    users:(("sshd",pid=393650,fd=4),("systemd",pid=1,fd=160))                                                          
```
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 7985 bytes 989440 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 7985 bytes 652726 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 7985 bytes 479164 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 266324 bytes 43809117 accept
		ct state related,established counter packets 38448468 bytes 34631646993 accept
		ct state invalid counter packets 5836 bytes 281993 jump ufw-logging-deny
		ct state invalid counter packets 5836 bytes 281993 drop
		...
		ip protocol icmp icmp type echo-request counter packets 457 bytes 21514 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 764801 bytes 76519631 jump ufw-not-local
		...
		counter packets 764801 bytes 76519631 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 266324 bytes 43809117 accept
		ct state related,established counter packets 32518379 bytes 37109431433 accept
		counter packets 402779 bytes 45590924 jump ufw-user-output
	}
	...
	chain ufw-before-forward {
		...
		counter packets 226363 bytes 13585084 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 7992 bytes 623325 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 887 bytes 203123 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 24 bytes 1156 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 165 bytes 8008 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 7468 bytes 2299864 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 1 bytes 28 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 50067 bytes 18426159 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 7985 bytes 352935 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 7985 bytes 756000 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 7985 bytes 479164 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 206963 bytes 12422652 accept
		ip protocol udp ct state new counter packets 164951 bytes 28986130 accept
	}
	...
	chain INPUT {
		...
		counter packets 489250049 bytes 574699210325 jump ufw-before-logging-input
		counter packets 489250049 bytes 574699210325 jump ufw-before-input
		counter packets 1515681 bytes 243238740 jump ufw-after-input
		counter packets 921829 bytes 66432379 jump ufw-after-logging-input
		counter packets 921829 bytes 66432379 jump ufw-reject-input
		counter packets 921829 bytes 66432379 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 385235038 bytes 372323328312 jump ufw-before-logging-output
		counter packets 385235038 bytes 372323328312 jump ufw-before-output
		counter packets 3475517 bytes 1342487199 jump ufw-after-output
		counter packets 3475517 bytes 1342487199 jump ufw-after-logging-output
		counter packets 3475517 bytes 1342487199 jump ufw-reject-output
		counter packets 3475517 bytes 1342487199 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 148039 bytes 30602969 jump DOCKER-USER
		counter packets 148039 bytes 30602969 jump DOCKER-FORWARD
		...
		counter packets 7938735 bytes 5156214053 jump ufw-before-logging-forward
		counter packets 7938735 bytes 5156214053 jump ufw-before-forward
		counter packets 404449 bytes 24285540 jump ufw-after-forward
		counter packets 404449 bytes 24285540 jump ufw-after-logging-forward
		counter packets 404449 bytes 24285540 jump ufw-reject-forward
		counter packets 404449 bytes 24285540 jump ufw-track-forward
		iifname "wg-home" counter packets 404449 bytes 24285540 accept
	...
	chain DOCKER-FORWARD {
		counter packets 148039 bytes 30602969 jump DOCKER-CT
		counter packets 114454 bytes 12383182 jump DOCKER-INTERNAL
		counter packets 114454 bytes 12383182 jump DOCKER-BRIDGE
		iifname "br-be53f5c116d2" counter packets 43454 bytes 8122158 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 1423 bytes 86798 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 1423 bytes 86798 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 66604 bytes 21561663 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 697135 bytes 54928099 return
		fib daddr type multicast counter packets 1276 bytes 40832 return
		fib daddr type broadcast counter packets 66390 bytes 21550700 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 529324 bytes 33278397 accept
		tcp dport 80 counter packets 24518 bytes 1199436 accept
		tcp dport 52000 counter packets 166 bytes 9832 accept
		udp dport 21195 counter packets 0 bytes 0 accept
		udp dport 21196 counter packets 0 bytes 0 accept
		tcp dport 21197 counter packets 10 bytes 552 accept
		...
		ip saddr 93.183.106.203 tcp dport 2222 counter packets 233 bytes 13980 accept
		udp dport 443 counter packets 27385 bytes 15120035 accept
	}
...
headscale   headscale/headscale:0.29.3   Up 14 hours (healthy)   127.0.0.1:8081->8080/tcp, 127.0.0.1:9091->9090/tcp
remnanode   remnawave/node:3.4.1         Up 14 hours             
```
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 47 | 2026-09-05 04:19:36 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-09-05 01:27:08 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 320K
drwxr-xr-x  2 root root   4.0K Sep  5 03:20 .
