...
Generated: 2026-08-10T04:20:01+03:00  
Host: hometele  
...
Linux hometele 6.8.0-137-generic #137-Ubuntu SMP PREEMPT_DYNAMIC Fri Jul 17 20:28:23 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:01 up 2 days, 59 min,  0 user,  load average: 0.00, 0.01, 0.00
```
...
udp   UNCONN 0      0           10.8.1.2:45565      0.0.0.0:*    users:(("xray",pid=956,fd=602))                                                                            
...
udp   UNCONN 0      0           10.8.1.2:37757      0.0.0.0:*    users:(("xray",pid=956,fd=514))                                                                            
udp   UNCONN 0      0            0.0.0.0:42910      0.0.0.0:*    users:(("openvpn",pid=827,fd=4))                                                                           
udp   UNCONN 0      0           10.8.1.2:56255      0.0.0.0:*    users:(("xray",pid=956,fd=229))                                                                            
...
udp   UNCONN 0      0           10.8.1.2:35123      0.0.0.0:*    users:(("xray",pid=956,fd=236))                                                                            
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 8829 bytes 972055 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 8829 bytes 1734336 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 1704 bytes 105796 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 4133305 bytes 1892272743 accept
		ct state related,established counter packets 67256323 bytes 80968386968 accept
		ct state invalid counter packets 9358 bytes 465135 jump ufw-logging-deny
		ct state invalid counter packets 9358 bytes 465135 drop
		...
		ip protocol icmp icmp type echo-request counter packets 607 bytes 29614 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 503205 bytes 47605038 jump ufw-not-local
		...
		counter packets 503205 bytes 47605038 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 4133305 bytes 1892272743 accept
		ct state related,established counter packets 55526920 bytes 48837651470 accept
		counter packets 421643 bytes 176757481 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 1156996 bytes 768366127 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 0 bytes 0 accept
		counter packets 7420 bytes 448756 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 395 bytes 30810 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 1468 bytes 336172 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 22 bytes 1012 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 125 bytes 5984 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 8262 bytes 2544696 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 53003 bytes 19796806 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 8829 bytes 429676 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 8829 bytes 2463473 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 1706 bytes 105900 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 253032 bytes 15373772 accept
		ip protocol udp ct state new counter packets 164763 bytes 161003016 accept
	}
	...
	chain INPUT {
		...
		counter packets 71902798 bytes 82908759498 jump ufw-before-logging-input
		counter packets 71902798 bytes 82908759498 jump ufw-before-input
		counter packets 167408 bytes 27875903 jump ufw-after-input
		counter packets 104133 bytes 5160423 jump ufw-after-logging-input
		counter packets 104133 bytes 5160423 jump ufw-reject-input
		counter packets 104133 bytes 5160423 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 60081868 bytes 50906681694 jump ufw-before-logging-output
		counter packets 60081868 bytes 50906681694 jump ufw-before-output
		counter packets 421643 bytes 176757481 jump ufw-after-output
		counter packets 421643 bytes 176757481 jump ufw-after-logging-output
		counter packets 421643 bytes 176757481 jump ufw-reject-output
		counter packets 421643 bytes 176757481 jump ufw-track-output
	}
	...
	chain FORWARD {
		...
		counter packets 1164416 bytes 768814883 jump ufw-before-logging-forward
		counter packets 1164416 bytes 768814883 jump ufw-before-forward
		counter packets 7420 bytes 448756 jump ufw-after-forward
		counter packets 7420 bytes 448756 jump ufw-after-logging-forward
		counter packets 7420 bytes 448756 jump ufw-reject-forward
		counter packets 7420 bytes 448756 jump ufw-track-forward
		iifname "wg-home" counter packets 7420 bytes 448756 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 3702 bytes 195575 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 3702 bytes 195575 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 63275 bytes 22715480 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 438677 bytes 24852260 return
		fib daddr type multicast counter packets 1411 bytes 45152 return
		fib daddr type broadcast counter packets 63117 bytes 22707626 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 334957 bytes 19682687 accept
		tcp dport 80 counter packets 801 bytes 43336 accept
		tcp dport 52000 counter packets 17 bytes 924 accept
		udp dport 21195 counter packets 0 bytes 0 accept
		udp dport 21196 counter packets 0 bytes 0 accept
		tcp dport 21197 counter packets 9 bytes 524 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 46 | 2026-08-10 04:19:54 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-08-09 19:30:39 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 196K
drwxr-xr-x  2 root root   4.0K Aug 10 03:20 .
