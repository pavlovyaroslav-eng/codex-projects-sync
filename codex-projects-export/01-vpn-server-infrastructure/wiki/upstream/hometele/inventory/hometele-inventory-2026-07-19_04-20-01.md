...
Generated: 2026-07-19T04:20:01+03:00  
Host: hometele  
...
Linux hometele 6.8.0-136-generic #136-Ubuntu SMP PREEMPT_DYNAMIC Wed Jul  1 21:53:05 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:01 up 1 day, 59 min,  1 user,  load average: 0.00, 0.00, 0.00
```
...
udp   UNCONN 0      0           10.8.1.2:48690      0.0.0.0:*    users:(("xray",pid=11123,fd=59))                                                                       
udp   UNCONN 0      0          127.0.0.1:10808      0.0.0.0:*    users:(("xray",pid=11123,fd=7))                                                                        
udp   UNCONN 0      0           10.8.1.2:48746      0.0.0.0:*    users:(("xray",pid=11123,fd=93))                                                                       
udp   UNCONN 0      0            0.0.0.0:51820      0.0.0.0:*                                                                                                           
udp   UNCONN 0      0           10.8.1.2:41587      0.0.0.0:*    users:(("xray",pid=11123,fd=42))                                                                       
udp   UNCONN 0      0           10.8.1.2:48773      0.0.0.0:*    users:(("xray",pid=11123,fd=29))                                                                       
...
udp   UNCONN 0      0           10.8.1.2:37667      0.0.0.0:*    users:(("xray",pid=11123,fd=78))                                                                       
udp   UNCONN 0      0           10.8.1.2:47105      0.0.0.0:*    users:(("xray",pid=11123,fd=50))                                                                       
...
udp   UNCONN 0      0           10.8.1.2:55452      0.0.0.0:*    users:(("xray",pid=11123,fd=46))                                                                       
udp   UNCONN 0      0           10.8.1.2:45356      0.0.0.0:*    users:(("xray",pid=11123,fd=54))                                                                       
udp   UNCONN 0      0           10.8.1.2:41398      0.0.0.0:*    users:(("xray",pid=11123,fd=76))                                                                       
udp   UNCONN 0      0           10.8.1.2:45503      0.0.0.0:*    users:(("xray",pid=11123,fd=15))                                                                       
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 4509 bytes 620377 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 4509 bytes 635011 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 4477 bytes 268849 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 1072111 bytes 484027542 accept
		ct state related,established counter packets 22574050 bytes 27969758356 accept
		ct state invalid counter packets 11637 bytes 651234 jump ufw-logging-deny
		ct state invalid counter packets 11637 bytes 651234 drop
		...
		ip protocol icmp icmp type echo-request counter packets 275 bytes 14748 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 197804 bytes 25079581 jump ufw-not-local
		...
		counter packets 197804 bytes 25079581 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 1072111 bytes 484027542 accept
		ct state related,established counter packets 17282493 bytes 16238917284 accept
		counter packets 180865 bytes 57667979 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 821240 bytes 566580566 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 4 bytes 112 accept
		counter packets 39928 bytes 2396985 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 280 bytes 21785 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 500 bytes 114500 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 17 bytes 780 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 59 bytes 2764 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 9955 bytes 4604728 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 35456 bytes 11657533 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 4509 bytes 228931 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 4509 bytes 885755 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 4477 bytes 268833 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 122769 bytes 7380549 accept
		ip protocol udp ct state new counter packets 56692 bytes 50122393 accept
	}
	...
	chain INPUT {
		...
		counter packets 23855877 bytes 28479531461 jump ufw-before-logging-input
		counter packets 23855877 bytes 28479531461 jump ufw-before-input
		counter packets 104002 bytes 19317287 jump ufw-after-input
		counter packets 57735 bytes 2915197 jump ufw-after-logging-input
		counter packets 57735 bytes 2915197 jump ufw-reject-input
		counter packets 57735 bytes 2915197 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 18535469 bytes 16780612805 jump ufw-before-logging-output
		counter packets 18535469 bytes 16780612805 jump ufw-before-output
		counter packets 180865 bytes 57667979 jump ufw-after-output
		counter packets 180865 bytes 57667979 jump ufw-after-logging-output
		counter packets 180865 bytes 57667979 jump ufw-reject-output
		counter packets 180865 bytes 57667979 jump ufw-track-output
	}
	...
	chain FORWARD {
		...
		counter packets 861172 bytes 568977663 jump ufw-before-logging-forward
		counter packets 861172 bytes 568977663 jump ufw-before-forward
		counter packets 39928 bytes 2396985 jump ufw-after-forward
		counter packets 39928 bytes 2396985 jump ufw-after-logging-forward
		counter packets 39928 bytes 2396985 jump ufw-reject-forward
		counter packets 39928 bytes 2396985 jump ufw-track-forward
		iifname "wg-home" counter packets 35575 bytes 2135644 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 1735 bytes 101530 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 1735 bytes 101530 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 46267 bytes 16402090 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 150921 bytes 8660124 return
		fib daddr type multicast counter packets 720 bytes 23040 return
		fib daddr type broadcast counter packets 46163 bytes 16396417 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 92990 bytes 5700378 accept
		tcp dport 80 counter packets 525 bytes 27800 accept
		tcp dport 52000 counter packets 88 bytes 5280 accept
		udp dport 21195 counter packets 0 bytes 0 accept
		udp dport 21196 counter packets 0 bytes 0 accept
		tcp dport 21197 counter packets 1 bytes 52 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 41 | 2026-07-19 04:19:40 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-07-18 12:04:11 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
