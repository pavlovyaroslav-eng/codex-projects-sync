...
Generated: 2026-07-30T04:20:01+03:00  
Host: hometele  
...
Linux hometele 6.8.0-136-generic #136-Ubuntu SMP PREEMPT_DYNAMIC Wed Jul  1 21:53:05 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:01 up 12 days, 59 min,  1 user,  load average: 0.08, 0.06, 0.02
```
...
udp   UNCONN 0      0           10.8.1.2:41013      0.0.0.0:*    users:(("xray",pid=172876,fd=442))                                                                                 
...
udp   UNCONN 0      0           10.8.1.2:44306      0.0.0.0:*    users:(("xray",pid=172876,fd=701))                                                                                 
udp   UNCONN 0      0           10.8.1.2:51672      0.0.0.0:*    users:(("xray",pid=172876,fd=880))                                                                                 
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 52029 bytes 6319707 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 52029 bytes 8852904 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 51997 bytes 3122049 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 19627195 bytes 8991990394 accept
		ct state related,established counter packets 316646350 bytes 389564663975 accept
		ct state invalid counter packets 125963 bytes 6355661 jump ufw-logging-deny
		ct state invalid counter packets 125963 bytes 6355661 drop
		...
		ip protocol icmp icmp type echo-request counter packets 3475 bytes 174610 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 2783610 bytes 298955889 jump ufw-not-local
		...
		counter packets 2783610 bytes 298955889 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 19627195 bytes 8991990394 accept
		ct state related,established counter packets 249380971 bytes 234191244593 accept
		counter packets 2522259 bytes 862931583 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 8418160 bytes 5617100471 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 4 bytes 112 accept
		counter packets 424287 bytes 25491641 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 55522 bytes 4330591 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 7776 bytes 1780617 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 115 bytes 5220 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 982 bytes 47704 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 66176 bytes 25058376 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 399165 bytes 134306509 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 52029 bytes 2514924 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 52029 bytes 13189316 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 51997 bytes 3122033 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 1712208 bytes 103848432 accept
		ip protocol udp ct state new counter packets 793716 bytes 757203175 accept
	}
	...
	chain INPUT {
		...
		counter packets 339186593 bytes 398862140529 jump ufw-before-logging-input
		counter packets 339186593 bytes 398862140529 jump ufw-before-input
		counter packets 1181856 bytes 204463638 jump ufw-after-input
		counter packets 652120 bytes 38934621 jump ufw-after-logging-input
		counter packets 652120 bytes 38934621 jump ufw-reject-input
		counter packets 652120 bytes 38934621 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 271530425 bytes 244046166570 jump ufw-before-logging-output
		counter packets 271530425 bytes 244046166570 jump ufw-before-output
		counter packets 2522259 bytes 862931583 jump ufw-after-output
		counter packets 2522259 bytes 862931583 jump ufw-after-logging-output
		counter packets 2522259 bytes 862931583 jump ufw-reject-output
		counter packets 2522259 bytes 862931583 jump ufw-track-output
	}
	...
	chain FORWARD {
		...
		counter packets 8842451 bytes 5642592224 jump ufw-before-logging-forward
		counter packets 8842451 bytes 5642592224 jump ufw-before-forward
		counter packets 424287 bytes 25491641 jump ufw-after-forward
		counter packets 424287 bytes 25491641 jump ufw-after-logging-forward
		counter packets 424287 bytes 25491641 jump ufw-reject-forward
		counter packets 424287 bytes 25491641 jump ufw-track-forward
		iifname "wg-home" counter packets 419934 bytes 25230300 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 25327 bytes 1427404 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 25327 bytes 1427404 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 529736 bytes 165529017 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 2246750 bytes 133221107 return
		fib daddr type multicast counter packets 8319 bytes 266208 return
		fib daddr type broadcast counter packets 528541 bytes 165468574 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 1588346 bytes 93702905 accept
		tcp dport 80 counter packets 12973 bytes 746940 accept
		tcp dport 52000 counter packets 186 bytes 10810 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 44 | 2026-07-30 04:19:45 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-07-29 17:39:01 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 144K
drwxr-xr-x  2 root root   4.0K Jul 30 03:20 .
