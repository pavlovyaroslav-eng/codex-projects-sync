...
Generated: 2026-08-17T04:20:01+03:00  
Host: hometele  
...
Linux hometele 6.8.0-137-generic #137-Ubuntu SMP PREEMPT_DYNAMIC Fri Jul 17 20:28:23 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:02 up 9 days, 59 min,  0 user,  load average: 0.00, 0.00, 0.00
```
...
udp   UNCONN 0      0           10.8.1.2:47559      0.0.0.0:*    users:(("xray",pid=116591,fd=1076))                                                                            
udp   UNCONN 0      0           10.8.1.2:35305      0.0.0.0:*    users:(("xray",pid=116591,fd=1056))                                                                            
udp   UNCONN 0      0           10.8.1.2:54821      0.0.0.0:*    users:(("xray",pid=116591,fd=1745))                                                                            
...
udp   UNCONN 0      0           10.8.1.2:48729      0.0.0.0:*    users:(("xray",pid=116591,fd=1762))                                                                            
...
udp   UNCONN 0      0           10.8.1.2:39755      0.0.0.0:*    users:(("xray",pid=116591,fd=1613))                                                                            
udp   UNCONN 0      0            0.0.0.0:42910      0.0.0.0:*    users:(("openvpn",pid=827,fd=4))                                                                               
udp   UNCONN 0      0           10.8.1.2:60397      0.0.0.0:*    users:(("xray",pid=116591,fd=1352))                                                                            
udp   UNCONN 0      0           10.8.1.2:50166      0.0.0.0:*    users:(("xray",pid=116591,fd=1824))                                                                            
...
udp   UNCONN 0      0           10.8.1.2:52292      0.0.0.0:*    users:(("xray",pid=116591,fd=1800))                                                                            
udp   UNCONN 0      0           10.8.1.2:53335      0.0.0.0:*    users:(("xray",pid=116591,fd=1832))                                                                            
udp   UNCONN 0      0           10.8.1.2:32935      0.0.0.0:*    users:(("xray",pid=116591,fd=1509))                                                                            
udp   UNCONN 0      0           10.8.1.2:54532      0.0.0.0:*    users:(("xray",pid=116591,fd=1731))                                                                            
udp   UNCONN 0      0           10.8.1.2:34102      0.0.0.0:*    users:(("xray",pid=116591,fd=1787))                                                                            
udp   UNCONN 0      0           10.8.1.2:45453      0.0.0.0:*    users:(("xray",pid=116591,fd=1201))                                                                            
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 39069 bytes 3992038 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 39069 bytes 6670490 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 7548 bytes 463908 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 18795259 bytes 8593972622 accept
		ct state related,established counter packets 320958454 bytes 398372913894 accept
		ct state invalid counter packets 56310 bytes 2666572 jump ufw-logging-deny
		ct state invalid counter packets 56310 bytes 2666572 drop
		...
		ip protocol icmp icmp type echo-request counter packets 2579 bytes 130318 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 2361129 bytes 220253557 jump ufw-not-local
		...
		counter packets 2361129 bytes 220253557 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 18795259 bytes 8593972622 accept
		ct state related,established counter packets 255769787 bytes 238480635380 accept
		counter packets 2424779 bytes 836064158 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 5613633 bytes 3786354785 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 0 bytes 0 accept
		counter packets 33368 bytes 2013444 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 30926 bytes 2412228 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 5859 bytes 1341711 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 97 bytes 4448 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 655 bytes 31512 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 36688 bytes 11325152 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 234948 bytes 88006405 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 39069 bytes 1862059 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 39069 bytes 9683284 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 7550 bytes 464012 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 1612916 bytes 97764991 accept
		ip protocol udp ct state new counter packets 793346 bytes 736461038 accept
	}
	...
	chain INPUT {
		...
		counter packets 342173731 bytes 407189936963 jump ufw-before-logging-input
		counter packets 342173731 bytes 407189936963 jump ufw-before-input
		counter packets 833095 bytes 129206893 jump ufw-after-input
		counter packets 523922 bytes 26085437 jump ufw-after-logging-input
		counter packets 523922 bytes 26085437 jump ufw-reject-input
		counter packets 523922 bytes 26085437 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 276989825 bytes 247910672160 jump ufw-before-logging-output
		counter packets 276989825 bytes 247910672160 jump ufw-before-output
		counter packets 2424779 bytes 836064158 jump ufw-after-output
		counter packets 2424779 bytes 836064158 jump ufw-after-logging-output
		counter packets 2424779 bytes 836064158 jump ufw-reject-output
		counter packets 2424779 bytes 836064158 jump ufw-track-output
	}
	...
	chain FORWARD {
		...
		counter packets 5647001 bytes 3788368229 jump ufw-before-logging-forward
		counter packets 5647001 bytes 3788368229 jump ufw-before-forward
		counter packets 33368 bytes 2013444 jump ufw-after-forward
		counter packets 33368 bytes 2013444 jump ufw-after-logging-forward
		counter packets 33368 bytes 2013444 jump ufw-reject-forward
		counter packets 33368 bytes 2013444 jump ufw-track-forward
		iifname "wg-home" counter packets 33368 bytes 2013444 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 21648 bytes 1056425 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 21648 bytes 1056425 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 309173 bytes 103121456 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 2046518 bytes 116972597 return
		fib daddr type multicast counter packets 6249 bytes 199968 return
		fib daddr type broadcast counter packets 308362 bytes 103080992 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 1523740 bytes 90809955 accept
		tcp dport 80 counter packets 4008 bytes 218483 accept
		tcp dport 52000 counter packets 240 bytes 13968 accept
		udp dport 21195 counter packets 1 bytes 54 accept
		udp dport 21196 counter packets 0 bytes 0 accept
		tcp dport 21197 counter packets 24 bytes 1340 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 45 | 2026-08-17 04:20:01 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-08-16 18:36:13 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 228K
drwxr-xr-x  2 root root   4.0K Aug 17 03:20 .
