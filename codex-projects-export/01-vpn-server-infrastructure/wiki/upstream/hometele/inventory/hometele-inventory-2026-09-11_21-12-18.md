...
Generated: 2026-09-11T21:12:18+03:00  
Host: hometele  
...
Linux hometele 6.8.0-139-generic #139-Ubuntu SMP PREEMPT_DYNAMIC Sat Aug  1 03:52:05 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 21:12:18 up 5 days, 17:52,  1 user,  load average: 0.34, 0.15, 0.13
```
...
udp   UNCONN 960    0      127.0.0.53%lo:53         0.0.0.0:*    users:(("systemd-resolve",pid=788215,fd=14))                                                                       
...
udp   UNCONN 0      0                  *:59151            *:*    users:(("rw-core",pid=252273,fd=34))                                                                               
udp   UNCONN 0      0                  *:48973            *:*    users:(("rw-core",pid=252273,fd=20))                                                                               
udp   UNCONN 0      0                  *:49091            *:*    users:(("rw-core",pid=252273,fd=48))                                                                               
udp   UNCONN 0      0                  *:57309            *:*    users:(("rw-core",pid=252273,fd=24))                                                                               
udp   UNCONN 0      0                  *:41026            *:*    users:(("rw-core",pid=252273,fd=60))                                                                               
udp   UNCONN 0      0                  *:38003            *:*    users:(("rw-core",pid=252273,fd=38))                                                                               
udp   UNCONN 0      0                  *:55536            *:*    users:(("rw-core",pid=252273,fd=35))                                                                               
udp   UNCONN 4608   0                  *:56625            *:*    users:(("rw-core",pid=252273,fd=9))                                                                                
udp   UNCONN 0      0                  *:59718            *:*    users:(("rw-core",pid=252273,fd=39))                                                                               
udp   UNCONN 0      0                  *:53607            *:*    users:(("rw-core",pid=252273,fd=25))                                                                               
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 24826 bytes 3157438 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 24826 bytes 2197391 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 24825 bytes 1490492 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 50901423 bytes 35439742466 accept
		ct state related,established counter packets 151662568 bytes 109281355592 accept
		ct state invalid counter packets 14154 bytes 866751 jump ufw-logging-deny
		ct state invalid counter packets 14154 bytes 866751 drop
		...
		ip protocol icmp icmp type echo-request counter packets 1804 bytes 90222 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 1329713 bytes 171509510 jump ufw-not-local
		...
		counter packets 1329713 bytes 171509510 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 50901423 bytes 35439742466 accept
		ct state related,established counter packets 122137257 bytes 111725703301 accept
		counter packets 1528685 bytes 186789698 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 3700544 bytes 2626424539 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 0 bytes 0 accept
		counter packets 184222 bytes 11060008 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 43624 bytes 3402672 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 2136 bytes 488550 jump ufw-skip-to-policy-input
		...
		udp dport 67 counter packets 23246 bytes 7159588 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 5 bytes 140 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 159163 bytes 57992157 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 24825 bytes 1160778 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 24826 bytes 2816045 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 24825 bytes 1490492 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 896113 bytes 53792935 accept
		ip protocol udp ct state new counter packets 614702 bytes 125809228 accept
	}
	...
	chain INPUT {
		...
		counter packets 203909663 bytes 144893566173 jump ufw-before-logging-input
		counter packets 203909663 bytes 144893566173 jump ufw-before-input
		counter packets 522582 bytes 83213251 jump ufw-after-input
		counter packets 293698 bytes 14136504 jump ufw-after-logging-input
		counter packets 293698 bytes 14136504 jump ufw-reject-input
		counter packets 293698 bytes 14136504 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 174567366 bytes 147352237097 jump ufw-before-logging-output
		counter packets 174567366 bytes 147352237097 jump ufw-before-output
		counter packets 1528685 bytes 186789698 jump ufw-after-output
		counter packets 1528685 bytes 186789698 jump ufw-after-logging-output
		counter packets 1528685 bytes 186789698 jump ufw-reject-output
		counter packets 1528685 bytes 186789698 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 4660824 bytes 2901837736 jump DOCKER-USER
		counter packets 4660824 bytes 2901837736 jump DOCKER-FORWARD
		...
		counter packets 3884766 bytes 2637484547 jump ufw-before-logging-forward
		counter packets 3884766 bytes 2637484547 jump ufw-before-forward
		counter packets 184222 bytes 11060008 jump ufw-after-forward
		counter packets 184222 bytes 11060008 jump ufw-after-logging-forward
		counter packets 184222 bytes 11060008 jump ufw-reject-forward
		counter packets 184222 bytes 11060008 jump ufw-track-forward
		iifname "wg-home" counter packets 184222 bytes 11060008 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 7442 bytes 439685 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 7442 bytes 439685 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 228884 bytes 69076747 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 1097641 bytes 102346195 return
		fib daddr type multicast counter packets 3970 bytes 127040 return
		fib daddr type broadcast counter packets 228102 bytes 69036275 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 469683 bytes 26580331 accept
		tcp dport 80 counter packets 267507 bytes 14846820 accept
		tcp dport 52000 counter packets 319 bytes 18628 accept
		...
		ip saddr 93.183.106.203 tcp dport 2222 counter packets 1027 bytes 61620 accept
		udp dport 443 counter packets 68538 bytes 46783774 accept
	}
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 48 | 2026-09-11 21:12:03 |
