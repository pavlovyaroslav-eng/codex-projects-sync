...
Generated: 2026-09-11T21:17:00+03:00  
Host: hometele  
...
Linux hometele 6.8.0-139-generic #139-Ubuntu SMP PREEMPT_DYNAMIC Sat Aug  1 03:52:05 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 21:17:01 up 5 days, 17:56,  1 user,  load average: 0.41, 0.25, 0.18
```
...
udp   UNCONN 0      0                  *:33748            *:*    users:(("rw-core",pid=252273,fd=58))                                                                               
udp   UNCONN 0      0                  *:58396            *:*    users:(("rw-core",pid=252273,fd=81))                                                                               
udp   UNCONN 0      0                  *:55353            *:*    users:(("rw-core",pid=252273,fd=40))                                                                               
udp   UNCONN 0      0                  *:41026            *:*    users:(("rw-core",pid=252273,fd=60))                                                                               
udp   UNCONN 0      0                  *:56425            *:*    users:(("rw-core",pid=252273,fd=80))                                                                               
udp   UNCONN 0      0                  *:44205            *:*    users:(("rw-core",pid=252273,fd=68))                                                                               
udp   UNCONN 0      0                  *:54557            *:*    users:(("rw-core",pid=252273,fd=27))                                                                               
...
udp   UNCONN 0      0                  *:51654            *:*    users:(("rw-core",pid=252273,fd=57))                                                                               
udp   UNCONN 0      0                  *:48635            *:*    users:(("rw-core",pid=252273,fd=21))                                                                               
udp   UNCONN 0      0                  *:54812            *:*    users:(("rw-core",pid=252273,fd=61))                                                                               
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 24840 bytes 3159558 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 24840 bytes 2199743 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 24840 bytes 1491392 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 51110502 bytes 35559449809 accept
		ct state related,established counter packets 152051037 bytes 109560801852 accept
		...
		ip protocol icmp icmp type echo-request counter packets 1807 bytes 90426 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 1330166 bytes 171572741 jump ufw-not-local
		...
		counter packets 1330166 bytes 171572741 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 51110502 bytes 35559449809 accept
		ct state related,established counter packets 122438803 bytes 112000926989 accept
		counter packets 1529432 bytes 186878802 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 3700697 bytes 2626457450 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 0 bytes 0 accept
		counter packets 184325 bytes 11066188 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 43746 bytes 3412188 jump ufw-skip-to-policy-input
		...
		udp dport 67 counter packets 23260 bytes 7163900 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 5 bytes 140 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 159256 bytes 58022656 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 24840 bytes 1161474 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 24840 bytes 2818783 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 24840 bytes 1491392 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 896509 bytes 53816695 accept
		ip protocol udp ct state new counter packets 615042 bytes 125868384 accept
	}
	...
	chain INPUT {
		...
		counter packets 204507666 bytes 145292781579 jump ufw-before-logging-input
		counter packets 204507666 bytes 145292781579 jump ufw-before-input
		counter packets 522916 bytes 83262308 jump ufw-after-input
		counter packets 293803 bytes 14141234 jump ufw-after-logging-input
		counter packets 293803 bytes 14141234 jump ufw-reject-input
		counter packets 293803 bytes 14141234 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 175078737 bytes 147747255600 jump ufw-before-logging-output
		counter packets 175078737 bytes 147747255600 jump ufw-before-output
		counter packets 1529432 bytes 186878802 jump ufw-after-output
		counter packets 1529432 bytes 186878802 jump ufw-after-logging-output
		counter packets 1529432 bytes 186878802 jump ufw-reject-output
		counter packets 1529432 bytes 186878802 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 4661522 bytes 2902020814 jump DOCKER-USER
		counter packets 4661522 bytes 2902020814 jump DOCKER-FORWARD
		...
		counter packets 3885022 bytes 2637523638 jump ufw-before-logging-forward
		counter packets 3885022 bytes 2637523638 jump ufw-before-forward
		counter packets 184325 bytes 11066188 jump ufw-after-forward
		counter packets 184325 bytes 11066188 jump ufw-after-logging-forward
		counter packets 184325 bytes 11066188 jump ufw-reject-forward
		counter packets 184325 bytes 11066188 jump ufw-track-forward
		iifname "wg-home" counter packets 184325 bytes 11066188 accept
	...
	chain ufw-skip-to-policy-input {
		counter packets 229113 bytes 69121074 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 1097862 bytes 102365003 return
		fib daddr type multicast counter packets 3973 bytes 127136 return
		fib daddr type broadcast counter packets 228331 bytes 69080602 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 469762 bytes 26585043 accept
		tcp dport 80 counter packets 267528 bytes 14847660 accept
		tcp dport 52000 counter packets 329 bytes 19228 accept
		...
		ip saddr 93.183.106.203 tcp dport 2222 counter packets 1029 bytes 61740 accept
		udp dport 443 counter packets 68545 bytes 46791676 accept
	}
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 48 | 2026-09-11 21:16:34 |
