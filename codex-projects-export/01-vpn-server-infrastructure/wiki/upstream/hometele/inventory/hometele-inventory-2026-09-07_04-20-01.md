...
Generated: 2026-09-07T04:20:01+03:00  
Host: hometele  
...
Linux hometele 6.8.0-139-generic #139-Ubuntu SMP PREEMPT_DYNAMIC Sat Aug  1 03:52:05 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:01 up 1 day, 59 min,  0 user,  load average: 0.00, 0.00, 0.00
```
...
udp   UNCONN 0      0                  *:57954            *:*    users:(("rw-core",pid=2222,fd=16))                                                                 
udp   UNCONN 0      0               [::]:51820         [::]:*                                                                                                       
udp   UNCONN 0      0                  *:60665            *:*    users:(("rw-core",pid=2222,fd=8))                                                                  
udp   UNCONN 0      0               [::]:59810         [::]:*    users:(("amneziawg-go",pid=905,fd=15))                                                             
udp   UNCONN 0      0                  *:443              *:*    users:(("rw-core",pid=2222,fd=7))                                                                  
...
tcp   LISTEN 0      511          0.0.0.0:80         0.0.0.0:*    users:(("nginx",pid=970,fd=7),("nginx",pid=969,fd=7),("nginx",pid=968,fd=7),("nginx",pid=967,fd=7))
...
tcp   LISTEN 0      511          0.0.0.0:443        0.0.0.0:*    users:(("nginx",pid=970,fd=9),("nginx",pid=969,fd=9),("nginx",pid=968,fd=9),("nginx",pid=967,fd=9))
tcp   LISTEN 0      4096            [::]:52000         [::]:*    users:(("sshd",pid=2684,fd=4),("systemd",pid=1,fd=90))                                             
tcp   LISTEN 0      511                *:2222             *:*    users:(("rw-node",pid=2128,fd=21))                                                                 
tcp   LISTEN 0      511             [::]:80            [::]:*    users:(("nginx",pid=970,fd=8),("nginx",pid=969,fd=8),("nginx",pid=968,fd=8),("nginx",pid=967,fd=8))
tcp   LISTEN 0      511             [::]:443           [::]:*    users:(("nginx",pid=970,fd=10),("nginx",pid=969,fd=10),("nginx",pid=968,fd=10),("nginx",pid=967,fd=10))
```
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 4509 bytes 603892 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 4509 bytes 481787 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 4509 bytes 270732 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 165788 bytes 18868610 accept
		ct state related,established counter packets 14600033 bytes 8867607706 accept
		ct state invalid counter packets 2209 bytes 116233 jump ufw-logging-deny
		ct state invalid counter packets 2209 bytes 116233 drop
		...
		ip protocol icmp icmp type echo-request counter packets 328 bytes 14636 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 130953 bytes 22680657 jump ufw-not-local
		...
		counter packets 130953 bytes 22680657 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 165788 bytes 18868610 accept
		ct state related,established counter packets 12232269 bytes 8724588833 accept
		counter packets 414238 bytes 42189306 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 779468 bytes 569515812 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 0 bytes 0 accept
		counter packets 33533 bytes 2013276 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 3822 bytes 298116 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 480 bytes 109920 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 10 bytes 484 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 65 bytes 3188 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 4214 bytes 1297912 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 29180 bytes 10515759 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 4509 bytes 206264 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 4509 bytes 503615 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 4509 bytes 270732 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 276866 bytes 16612970 accept
		ip protocol udp ct state new counter packets 135780 bytes 25045340 accept
	}
	...
	chain INPUT {
		...
		counter packets 14899311 bytes 8909287842 jump ufw-before-logging-input
		counter packets 14899311 bytes 8909287842 jump ufw-before-input
		counter packets 101287 bytes 15213430 jump ufw-after-input
		counter packets 63516 bytes 2988051 jump ufw-after-logging-input
		counter packets 63516 bytes 2988051 jump ufw-reject-input
		counter packets 63516 bytes 2988051 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 12812295 bytes 8785646749 jump ufw-before-logging-output
		counter packets 12812295 bytes 8785646749 jump ufw-before-output
		counter packets 414238 bytes 42189306 jump ufw-after-output
		counter packets 414238 bytes 42189306 jump ufw-after-logging-output
		counter packets 414238 bytes 42189306 jump ufw-reject-output
		counter packets 414238 bytes 42189306 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 952853 bytes 619472518 jump DOCKER-USER
		counter packets 952853 bytes 619472518 jump DOCKER-FORWARD
		...
		counter packets 813001 bytes 571529088 jump ufw-before-logging-forward
		counter packets 813001 bytes 571529088 jump ufw-before-forward
		counter packets 33533 bytes 2013276 jump ufw-after-forward
		counter packets 33533 bytes 2013276 jump ufw-after-logging-forward
		counter packets 33533 bytes 2013276 jump ufw-reject-forward
		counter packets 33533 bytes 2013276 jump ufw-track-forward
		iifname "wg-home" counter packets 33533 bytes 2013276 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 1937 bytes 99913 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 1937 bytes 99913 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 37771 bytes 12225379 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 92545 bytes 10436534 return
		fib daddr type multicast counter packets 720 bytes 23040 return
		fib daddr type broadcast counter packets 37688 bytes 12221083 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 8805 bytes 524232 accept
		tcp dport 80 counter packets 11589 bytes 555044 accept
		tcp dport 52000 counter packets 19 bytes 1032 accept
		udp dport 21195 counter packets 0 bytes 0 accept
		udp dport 21196 counter packets 0 bytes 0 accept
		tcp dport 21197 counter packets 3 bytes 152 accept
		...
		ip saddr 93.183.106.203 tcp dport 2222 counter packets 178 bytes 10680 accept
		udp dport 443 counter packets 9063 bytes 6374919 accept
	}
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 47 | 2026-09-07 04:19:37 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-09-06 19:02:17 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 332K
drwxr-xr-x  2 root root   4.0K Sep  7 03:20 .
