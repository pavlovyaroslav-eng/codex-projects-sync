...
Generated: 2026-09-14T04:20:03+03:00  
Host: hometele  
...
Linux hometele 6.8.0-139-generic #139-Ubuntu SMP PREEMPT_DYNAMIC Sat Aug  1 03:52:05 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:04 up 8 days, 59 min,  0 user,  load average: 0.26, 0.06, 0.02
```
...
udp   UNCONN 0      0                  *:51829            *:*    users:(("rw-core",pid=252273,fd=75))                                                                               
udp   UNCONN 0      0                  *:59004            *:*    users:(("rw-core",pid=252273,fd=122))                                                                              
udp   UNCONN 0      0                  *:43946            *:*    users:(("rw-core",pid=252273,fd=64))                                                                               
udp   UNCONN 0      0                  *:46119            *:*    users:(("rw-core",pid=252273,fd=37))                                                                               
udp   UNCONN 0      0                  *:38980            *:*    users:(("rw-core",pid=252273,fd=138))                                                                              
udp   UNCONN 0      0                  *:60510            *:*    users:(("rw-core",pid=252273,fd=86))                                                                               
udp   UNCONN 0      0                  *:42402            *:*    users:(("rw-core",pid=252273,fd=70))                                                                               
udp   UNCONN 0      0               [::]:59810         [::]:*    users:(("amneziawg-go",pid=905,fd=15))                                                                             
udp   UNCONN 0      0                  *:33203            *:*    users:(("rw-core",pid=252273,fd=57))                                                                               
udp   UNCONN 0      0                  *:443              *:*    users:(("rw-core",pid=252273,fd=7))                                                                                
udp   UNCONN 0      0                  *:38346            *:*    users:(("rw-core",pid=252273,fd=18))                                                                               
udp   UNCONN 0      0                  *:46583            *:*    users:(("rw-core",pid=252273,fd=59))                                                                               
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 34749 bytes 4487116 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 34749 bytes 3040435 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 34749 bytes 2086204 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 76472433 bytes 55368973176 accept
		ct state related,established counter packets 212536253 bytes 158386170761 accept
		ct state invalid counter packets 19523 bytes 1175251 jump ufw-logging-deny
		ct state invalid counter packets 19523 bytes 1175251 drop
		...
		ip protocol icmp icmp type echo-request counter packets 2473 bytes 125580 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 1647774 bytes 226272304 jump ufw-not-local
		...
		counter packets 1647774 bytes 226272304 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 76472433 bytes 55368973176 accept
		ct state related,established counter packets 170800050 bytes 161675684417 accept
		counter packets 2164008 bytes 261472582 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 5365693 bytes 3845193798 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 0 bytes 0 accept
		counter packets 258146 bytes 15498088 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 68413 bytes 5336214 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 2882 bytes 659330 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 124 bytes 5932 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 807 bytes 37780 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 32542 bytes 10022684 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 7 bytes 196 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 223830 bytes 81194116 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 34749 bytes 1636495 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 34749 bytes 3926088 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 34749 bytes 2086204 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 1277436 bytes 76695870 accept
		ip protocol udp ct state new counter packets 861901 bytes 174952534 accept
	}
	...
	chain INPUT {
		...
		counter packets 290678456 bytes 213982717072 jump ufw-before-logging-input
		counter packets 290678456 bytes 213982717072 jump ufw-before-input
		counter packets 702744 bytes 115375980 jump ufw-after-input
		counter packets 374139 bytes 18119728 jump ufw-after-logging-input
		counter packets 374139 bytes 18119728 jump ufw-reject-input
		counter packets 374139 bytes 18119728 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 249436491 bytes 217306130175 jump ufw-before-logging-output
		counter packets 249436491 bytes 217306130175 jump ufw-before-output
		counter packets 2164008 bytes 261472582 jump ufw-after-output
		counter packets 2164008 bytes 261472582 jump ufw-after-logging-output
		counter packets 2164008 bytes 261472582 jump ufw-reject-output
		counter packets 2164008 bytes 261472582 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 6708729 bytes 4230792461 jump DOCKER-USER
		counter packets 6708729 bytes 4230792461 jump DOCKER-FORWARD
		...
		counter packets 5623839 bytes 3860691886 jump ufw-before-logging-forward
		counter packets 5623839 bytes 3860691886 jump ufw-before-forward
		counter packets 258146 bytes 15498088 jump ufw-after-forward
		counter packets 258146 bytes 15498088 jump ufw-after-logging-forward
		counter packets 258146 bytes 15498088 jump ufw-reject-forward
		counter packets 258146 bytes 15498088 jump ufw-track-forward
		iifname "wg-home" counter packets 258146 bytes 15498088 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 10347 bytes 599587 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 10347 bytes 599587 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 328605 bytes 97256252 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 1314641 bytes 128891382 return
		fib daddr type multicast counter packets 5558 bytes 177856 return
		fib daddr type broadcast counter packets 327575 bytes 97203066 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 567681 bytes 32466110 accept
		tcp dport 80 counter packets 283889 bytes 15522256 accept
		tcp dport 52000 counter packets 491 bytes 28788 accept
		udp dport 21195 counter packets 0 bytes 0 accept
		udp dport 21196 counter packets 0 bytes 0 accept
		tcp dport 21197 counter packets 50 bytes 2844 accept
		...
		ip saddr 93.183.106.203 tcp dport 2222 counter packets 1447 bytes 86820 accept
		udp dport 443 counter packets 91461 bytes 62787052 accept
	}
...
headscale   headscale/headscale:0.29.3   Up 8 days (healthy)   127.0.0.1:8081->8080/tcp, 127.0.0.1:9091->9090/tcp
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 49 | 2026-09-14 04:19:49 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-09-14 02:46:34 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 368K
drwxr-xr-x  2 root root   4.0K Sep 14 03:20 .
