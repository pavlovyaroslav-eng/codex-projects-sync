...
Generated: 2026-09-01T04:20:01+03:00  
Host: hometele  
...
Linux hometele 6.8.0-138-generic #138-Ubuntu SMP PREEMPT_DYNAMIC Fri Jul 31 22:41:49 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:01 up 11 days, 59 min,  0 user,  load average: 0.06, 0.02, 0.00
```
...
udp   UNCONN 0      0           10.8.1.2:49540      0.0.0.0:*    users:(("xray",pid=971,fd=5271))                                                                               
udp   UNCONN 0      0           10.8.1.2:36408      0.0.0.0:*    users:(("xray",pid=971,fd=5489))                                                                               
...
udp   UNCONN 0      0           10.8.1.2:37678      0.0.0.0:*    users:(("xray",pid=971,fd=5032))                                                                               
udp   UNCONN 0      0           10.8.1.2:52017      0.0.0.0:*    users:(("xray",pid=971,fd=5483))                                                                               
udp   UNCONN 0      0            0.0.0.0:53062      0.0.0.0:*    users:(("openvpn",pid=833,fd=4))                                                                               
udp   UNCONN 0      0           10.8.1.2:55172      0.0.0.0:*    users:(("xray",pid=971,fd=5302))                                                                               
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 47709 bytes 4962797 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 47709 bytes 9813998 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 17684 bytes 1071488 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 19750309 bytes 8958547779 accept
		ct state related,established counter packets 361538094 bytes 452139937764 accept
		ct state invalid counter packets 37259 bytes 2107524 jump ufw-logging-deny
		ct state invalid counter packets 37259 bytes 2107524 drop
		...
		ip protocol icmp icmp type echo-request counter packets 3456 bytes 188953 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 2901131 bytes 291776160 jump ufw-not-local
		...
		counter packets 2901131 bytes 291776160 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 19750309 bytes 8958547779 accept
		ct state related,established counter packets 277509630 bytes 277284838245 accept
		counter packets 2585484 bytes 1074557326 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 6492661 bytes 4394356660 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 0 bytes 0 accept
		counter packets 108811 bytes 6541572 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 125187 bytes 9764579 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 5147 bytes 1178663 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 162 bytes 7500 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 825 bytes 39828 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 45057 bytes 13976094 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 1 bytes 28 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 278587 bytes 105911365 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 47709 bytes 2183898 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 47709 bytes 14349150 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 17686 bytes 1071592 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 1590106 bytes 96640116 accept
		ip protocol udp ct state new counter packets 976030 bytes 975781173 accept
	}
	...
	chain INPUT {
		...
		counter packets 384230249 bytes 461392558180 jump ufw-before-logging-input
		counter packets 384230249 bytes 461392558180 jump ufw-before-input
		counter packets 1131478 bytes 186159045 jump ufw-after-input
		counter packets 676512 bytes 55280988 jump ufw-after-logging-input
		counter packets 676512 bytes 55280988 jump ufw-reject-input
		counter packets 676512 bytes 55280988 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 299845423 bytes 287317943350 jump ufw-before-logging-output
		counter packets 299845423 bytes 287317943350 jump ufw-before-output
		counter packets 2585484 bytes 1074557326 jump ufw-after-output
		counter packets 2585484 bytes 1074557326 jump ufw-after-logging-output
		counter packets 2585484 bytes 1074557326 jump ufw-reject-output
		counter packets 2585484 bytes 1074557326 jump ufw-track-output
	}
	...
	chain FORWARD {
		...
		counter packets 6601472 bytes 4400898232 jump ufw-before-logging-forward
		counter packets 6601472 bytes 4400898232 jump ufw-before-forward
		counter packets 108811 bytes 6541572 jump ufw-after-forward
		counter packets 108811 bytes 6541572 jump ufw-after-logging-forward
		counter packets 108811 bytes 6541572 jump ufw-reject-forward
		counter packets 108811 bytes 6541572 jump ufw-track-forward
		iifname "wg-home" counter packets 108811 bytes 6541572 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 17990 bytes 1063380 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 17990 bytes 1063380 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 454966 bytes 130878057 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 2439614 bytes 160708390 return
		fib daddr type multicast counter packets 7630 bytes 244160 return
		fib daddr type broadcast counter packets 453887 bytes 130823610 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 1615282 bytes 96691740 accept
		tcp dport 80 counter packets 154154 bytes 8909460 accept
		tcp dport 52000 counter packets 133 bytes 7300 accept
		udp dport 21195 counter packets 1 bytes 439 accept
		udp dport 21196 counter packets 1 bytes 439 accept
		tcp dport 21197 counter packets 55 bytes 3080 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 48 | 2026-09-01 04:19:42 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-08-31 19:51:31 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 304K
drwxr-xr-x  2 root root   4.0K Sep  1 03:20 .
