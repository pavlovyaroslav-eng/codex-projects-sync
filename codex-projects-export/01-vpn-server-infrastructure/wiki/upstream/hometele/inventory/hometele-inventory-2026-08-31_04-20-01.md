...
Generated: 2026-08-31T04:20:01+03:00  
Host: hometele  
...
Linux hometele 6.8.0-138-generic #138-Ubuntu SMP PREEMPT_DYNAMIC Fri Jul 31 22:41:49 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:02 up 10 days, 59 min,  0 user,  load average: 0.00, 0.00, 0.00
```
...
udp   UNCONN 0      0           10.8.1.2:48341      0.0.0.0:*    users:(("xray",pid=971,fd=5011))                                                                               
udp   UNCONN 0      0           10.8.1.2:48412      0.0.0.0:*    users:(("xray",pid=971,fd=5040))                                                                               
udp   UNCONN 0      0           10.8.1.2:52514      0.0.0.0:*    users:(("xray",pid=971,fd=5065))                                                                               
udp   UNCONN 0      0           10.8.1.2:36304      0.0.0.0:*    users:(("xray",pid=971,fd=5021))                                                                               
udp   UNCONN 0      0           10.8.1.2:38376      0.0.0.0:*    users:(("xray",pid=971,fd=5054))                                                                               
udp   UNCONN 0      0           10.8.1.2:59931      0.0.0.0:*    users:(("xray",pid=971,fd=5051))                                                                               
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 43389 bytes 4462710 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 43389 bytes 8741255 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 13501 bytes 820316 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 18219473 bytes 8281916209 accept
		ct state related,established counter packets 329192776 bytes 411089323837 accept
		ct state invalid counter packets 33666 bytes 1905252 jump ufw-logging-deny
		ct state invalid counter packets 33666 bytes 1905252 drop
		...
		ip protocol icmp icmp type echo-request counter packets 3223 bytes 177052 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 2670905 bytes 269017249 jump ufw-not-local
		...
		counter packets 2670905 bytes 269017249 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 18219473 bytes 8281916209 accept
		ct state related,established counter packets 253695202 bytes 253807142002 accept
		counter packets 2380285 bytes 970873147 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 5911353 bytes 3977293151 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 0 bytes 0 accept
		counter packets 77630 bytes 4669560 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 118660 bytes 9255473 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 4655 bytes 1065995 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 141 bytes 6532 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 728 bytes 35140 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 41010 bytes 12729618 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 1 bytes 28 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 254036 bytes 96409603 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 43389 bytes 1973525 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 43389 bytes 12522512 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 13503 bytes 820420 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 1486358 bytes 90284542 accept
		ip protocol udp ct state new counter packets 878151 bytes 878772728 accept
	}
	...
	chain INPUT {
		...
		counter packets 350120043 bytes 419642339599 jump ufw-before-logging-input
		counter packets 350120043 bytes 419642339599 jump ufw-before-input
		counter packets 1051976 bytes 172596880 jump ufw-after-input
		counter packets 632745 bytes 53094491 jump ufw-after-logging-input
		counter packets 632745 bytes 53094491 jump ufw-reject-input
		counter packets 632745 bytes 53094491 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 274294960 bytes 263059931358 jump ufw-before-logging-output
		counter packets 274294960 bytes 263059931358 jump ufw-before-output
		counter packets 2380285 bytes 970873147 jump ufw-after-output
		counter packets 2380285 bytes 970873147 jump ufw-after-logging-output
		counter packets 2380285 bytes 970873147 jump ufw-reject-output
		counter packets 2380285 bytes 970873147 jump ufw-track-output
	}
	...
	chain FORWARD {
		...
		counter packets 5988983 bytes 3981962711 jump ufw-before-logging-forward
		counter packets 5988983 bytes 3981962711 jump ufw-before-forward
		counter packets 77630 bytes 4669560 jump ufw-after-forward
		counter packets 77630 bytes 4669560 jump ufw-after-logging-forward
		counter packets 77630 bytes 4669560 jump ufw-reject-forward
		counter packets 77630 bytes 4669560 jump ufw-track-forward
		iifname "wg-home" counter packets 77630 bytes 4669560 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 16623 bytes 980681 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 16623 bytes 980681 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 419231 bytes 119502389 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 2245684 bytes 149340667 return
		fib daddr type multicast counter packets 6939 bytes 222048 return
		fib daddr type broadcast counter packets 418282 bytes 119454534 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 1487933 bytes 88848470 accept
		tcp dport 80 counter packets 130811 bytes 7557940 accept
		tcp dport 52000 counter packets 113 bytes 6156 accept
		udp dport 21195 counter packets 1 bytes 439 accept
		udp dport 21196 counter packets 1 bytes 439 accept
		tcp dport 21197 counter packets 44 bytes 2444 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 47 | 2026-08-31 04:19:37 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-08-30 17:47:56 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 300K
drwxr-xr-x  2 root root   4.0K Aug 31 03:20 .
