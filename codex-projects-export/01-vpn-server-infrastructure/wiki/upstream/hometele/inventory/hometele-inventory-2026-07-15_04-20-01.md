...
Generated: 2026-07-15T04:20:01+03:00  
Host: hometele  
...
Linux hometele 6.8.0-134-generic #134-Ubuntu SMP PREEMPT_DYNAMIC Fri Jun 26 18:43:11 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:01 up 20:24,  0 user,  load average: 0.00, 0.01, 0.00
```
...
udp   UNCONN 0      0          127.0.0.1:10808      0.0.0.0:*    users:(("xray",pid=12094,fd=7))                                                                            
udp   UNCONN 0      0          10.79.0.2:54851      0.0.0.0:*    users:(("xray",pid=12094,fd=77))                                                                           
udp   UNCONN 0      0            0.0.0.0:51820      0.0.0.0:*                                                                                                               
udp   UNCONN 0      0          10.79.0.2:59013      0.0.0.0:*    users:(("xray",pid=12094,fd=55))                                                                           
...
tcp   LISTEN 0      511        127.0.0.1:8443       0.0.0.0:*    users:(("nginx",pid=12114,fd=9),("nginx",pid=12113,fd=9),("nginx",pid=12112,fd=9),("nginx",pid=12111,fd=9))
tcp   LISTEN 0      4096         0.0.0.0:52000      0.0.0.0:*    users:(("sshd",pid=1598,fd=3),("systemd",pid=1,fd=81))                                                     
tcp   LISTEN 0      100          0.0.0.0:25         0.0.0.0:*    users:(("master",pid=12406,fd=13))                                                                         
tcp   LISTEN 0      4096       127.0.0.1:10808      0.0.0.0:*    users:(("xray",pid=12094,fd=6))                                                                            
tcp   LISTEN 0      511          0.0.0.0:80         0.0.0.0:*    users:(("nginx",pid=12114,fd=7),("nginx",pid=12113,fd=7),("nginx",pid=12112,fd=7),("nginx",pid=12111,fd=7))
tcp   LISTEN 0      4096      127.0.0.54:53         0.0.0.0:*    users:(("systemd-resolve",pid=793,fd=17))                                                                  
tcp   LISTEN 0      4096            [::]:52000         [::]:*    users:(("sshd",pid=1598,fd=4),("systemd",pid=1,fd=82))                                                     
tcp   LISTEN 0      511             [::]:80            [::]:*    users:(("nginx",pid=12114,fd=8),("nginx",pid=12113,fd=8),("nginx",pid=12112,fd=8),("nginx",pid=12111,fd=8))
tcp   LISTEN 0      4096               *:443              *:*    users:(("xray",pid=12094,fd=3))                                                                            
tcp   LISTEN 0      511            [::1]:8443          [::]:*    users:(("nginx",pid=12114,fd=10),("nginx",pid=12113,fd=10),("nginx",pid=12112,fd=10),("nginx",pid=12111,fd=10))
```
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 3682 bytes 396799 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 3682 bytes 564097 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 3682 bytes 220984 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 1754774 bytes 807090439 accept
		ct state related,established counter packets 32029336 bytes 31583938437 accept
		ct state invalid counter packets 1843 bytes 116443 jump ufw-logging-deny
		ct state invalid counter packets 1843 bytes 116443 drop
		...
		ip protocol icmp icmp type echo-request counter packets 164 bytes 6988 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 330147 bytes 27885509 jump ufw-not-local
		...
		counter packets 330147 bytes 27885509 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 1754774 bytes 807090439 accept
		ct state related,established counter packets 25981129 bytes 20233826365 accept
		counter packets 226582 bytes 62345547 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 705604 bytes 491766005 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 12 bytes 336 accept
		counter packets 32343 bytes 1952296 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 13665 bytes 1066086 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 411 bytes 94119 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 10 bytes 480 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 55 bytes 2656 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 3453 bytes 1065601 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 30373 bytes 9860417 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 3682 bytes 176782 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 3682 bytes 885822 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 3682 bytes 221112 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 170703 bytes 10270895 accept
		ip protocol udp ct state new counter packets 54753 bytes 51929089 accept
	}
	...
	chain INPUT {
		...
		counter packets 34116264 bytes 32419037816 jump ufw-before-logging-input
		counter packets 34116264 bytes 32419037816 jump ufw-before-input
		counter packets 186490 bytes 19510178 jump ufw-after-input
		counter packets 138523 bytes 7420819 jump ufw-after-logging-input
		counter packets 138523 bytes 7420819 jump ufw-reject-input
		counter packets 138523 bytes 7420819 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 27962485 bytes 21103262351 jump ufw-before-logging-output
		counter packets 27962485 bytes 21103262351 jump ufw-before-output
		counter packets 226582 bytes 62345547 jump ufw-after-output
		counter packets 226582 bytes 62345547 jump ufw-after-logging-output
		counter packets 226582 bytes 62345547 jump ufw-reject-output
		counter packets 226582 bytes 62345547 jump ufw-track-output
	}
	...
	chain FORWARD {
		...
		ip daddr 10.91.0.0/24 iifname "tun79" oifname "tun91" ct state related,established counter packets 2375434 bytes 2795291760 accept
		ip saddr 10.91.0.0/24 iifname "tun91" oifname "tun79" counter packets 1428056 bytes 177121360 accept
		...
		counter packets 737959 bytes 493718637 jump ufw-before-logging-forward
		counter packets 737959 bytes 493718637 jump ufw-before-forward
		counter packets 32343 bytes 1952296 jump ufw-after-forward
		counter packets 32343 bytes 1952296 jump ufw-after-logging-forward
		counter packets 32343 bytes 1952296 jump ufw-reject-forward
		counter packets 32343 bytes 1952296 jump ufw-track-forward
		ip saddr 10.77.77.0/24 iifname "wg-home" oifname "tun79" counter packets 1932 bytes 115952 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 1036 bytes 69337 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 1036 bytes 69337 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 47967 bytes 12089359 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 281663 bytes 15780846 return
		fib daddr type multicast counter packets 586 bytes 18752 return
		fib daddr type broadcast counter packets 47898 bytes 12085911 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 143225 bytes 8351357 accept
		tcp dport 80 counter packets 406 bytes 22356 accept
		tcp dport 52000 counter packets 5 bytes 240 accept
		...
		udp dport 21198 counter packets 3 bytes 126 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 42 | 2026-07-15 04:19:46 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-07-14 22:52:52 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
