...
Generated: 2026-09-11T21:29:11+03:00  
Host: hometele  
...
Linux hometele 6.8.0-139-generic #139-Ubuntu SMP PREEMPT_DYNAMIC Sat Aug  1 03:52:05 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 21:29:11 up 5 days, 18:08,  1 user,  load average: 0.11, 0.21, 0.19
```
...
udp   UNCONN 0      0                  *:45678            *:*    users:(("rw-core",pid=252273,fd=29))                                                                               
udp   UNCONN 0      0                  *:48872            *:*    users:(("rw-core",pid=252273,fd=69))                                                                               
udp   UNCONN 0      0                  *:55055            *:*    users:(("rw-core",pid=252273,fd=154))                                                                              
udp   UNCONN 0      0                  *:45963            *:*    users:(("rw-core",pid=252273,fd=35))                                                                               
udp   UNCONN 0      0                  *:49053            *:*    users:(("rw-core",pid=252273,fd=82))                                                                               
udp   UNCONN 0      0                  *:33705            *:*    users:(("rw-core",pid=252273,fd=39))                                                                               
udp   UNCONN 0      0                  *:59396            *:*    users:(("rw-core",pid=252273,fd=38))                                                                               
udp   UNCONN 0      0                  *:41026            *:*    users:(("rw-core",pid=252273,fd=60))                                                                               
udp   UNCONN 0      0                  *:49233            *:*    users:(("rw-core",pid=252273,fd=46))                                                                               
udp   UNCONN 0      0                  *:39084            *:*    users:(("rw-core",pid=252273,fd=71))                                                                               
udp   UNCONN 0      0                  *:57641            *:*    users:(("rw-core",pid=252273,fd=26))                                                                               
udp   UNCONN 0      0                  *:56625            *:*    users:(("rw-core",pid=252273,fd=9))                                                                                
udp   UNCONN 0      0                  *:50516            *:*    users:(("rw-core",pid=252273,fd=158))                                                                              
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 24876 bytes 3164792 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 24876 bytes 2204697 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 24876 bytes 1493552 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 51340610 bytes 35695867826 accept
		ct state related,established counter packets 152916337 bytes 110285469286 accept
		ct state invalid counter packets 14204 bytes 869759 jump ufw-logging-deny
		ct state invalid counter packets 14204 bytes 869759 drop
		...
		ip protocol icmp icmp type echo-request counter packets 1821 bytes 91308 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 1331272 bytes 171757540 jump ufw-not-local
		...
		counter packets 1331272 bytes 171757540 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 51340610 bytes 35695867826 accept
		ct state related,established counter packets 123093541 bytes 112718418922 accept
		counter packets 1531645 bytes 187206460 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 3701090 bytes 2626545577 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 0 bytes 0 accept
		counter packets 184585 bytes 11081788 jump ufw-user-forward
	}
	...
	chain ufw-after-input {
		...
		udp dport 67 counter packets 23294 bytes 7174372 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 5 bytes 140 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 159504 bytes 58108975 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 24876 bytes 1163094 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 24876 bytes 2822933 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 24876 bytes 1493552 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 897696 bytes 53887915 accept
		ip protocol udp ct state new counter packets 616028 bytes 126106072 accept
	}
	...
	chain INPUT {
		...
		counter packets 205604244 bytes 146154055719 jump ufw-before-logging-input
		counter packets 205604244 bytes 146154055719 jump ufw-before-input
		counter packets 523475 bytes 83373460 jump ufw-after-input
		counter packets 294076 bytes 14154830 jump ufw-after-logging-input
		counter packets 294076 bytes 14154830 jump ufw-reject-input
		counter packets 294076 bytes 14154830 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 175965796 bytes 148601493208 jump ufw-before-logging-output
		counter packets 175965796 bytes 148601493208 jump ufw-before-output
		counter packets 1531645 bytes 187206460 jump ufw-after-output
		counter packets 1531645 bytes 187206460 jump ufw-after-logging-output
		counter packets 1531645 bytes 187206460 jump ufw-reject-output
		counter packets 1531645 bytes 187206460 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 4663320 bytes 2902506133 jump DOCKER-USER
		counter packets 4663320 bytes 2902506133 jump DOCKER-FORWARD
		...
		counter packets 3885675 bytes 2637627365 jump ufw-before-logging-forward
		counter packets 3885675 bytes 2637627365 jump ufw-before-forward
		counter packets 184585 bytes 11081788 jump ufw-after-forward
		counter packets 184585 bytes 11081788 jump ufw-after-logging-forward
		counter packets 184585 bytes 11081788 jump ufw-reject-forward
		counter packets 184585 bytes 11081788 jump ufw-track-forward
		iifname "wg-home" counter packets 184585 bytes 11081788 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 7467 bytes 441105 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 7467 bytes 441105 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 229399 bytes 69218630 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 1098677 bytes 102452132 return
		fib daddr type multicast counter packets 3979 bytes 127328 return
		fib daddr type broadcast counter packets 228616 bytes 69178080 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 470174 bytes 26610224 accept
		tcp dport 80 counter packets 267584 bytes 14849920 accept
		tcp dport 52000 counter packets 339 bytes 19828 accept
		...
		udp dport 443 counter packets 68613 bytes 46837222 accept
	}
