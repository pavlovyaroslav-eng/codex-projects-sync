...
Generated: 2026-08-28T04:20:01+03:00  
Host: hometele  
...
Linux hometele 6.8.0-138-generic #138-Ubuntu SMP PREEMPT_DYNAMIC Fri Jul 31 22:41:49 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:02 up 7 days, 59 min,  0 user,  load average: 0.02, 0.02, 0.00
```
...
udp   UNCONN 0      0           10.8.1.2:47395      0.0.0.0:*    users:(("xray",pid=971,fd=1144))                                                                               
udp   UNCONN 0      0           10.8.1.2:58781      0.0.0.0:*    users:(("xray",pid=971,fd=939))                                                                                
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 30429 bytes 3047976 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 30429 bytes 6545616 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 6798 bytes 416072 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 14468337 bytes 6626057086 accept
		ct state related,established counter packets 253314145 bytes 319008418528 accept
		ct state invalid counter packets 26710 bytes 1498165 jump ufw-logging-deny
		ct state invalid counter packets 26710 bytes 1498165 drop
		...
		ip protocol icmp icmp type echo-request counter packets 2212 bytes 117950 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 2024602 bytes 203239556 jump ufw-not-local
		...
		counter packets 2024602 bytes 203239556 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 14468337 bytes 6626057086 accept
		ct state related,established counter packets 196663090 bytes 198509802403 accept
		counter packets 1896952 bytes 791522411 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 3790293 bytes 2500016058 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 0 bytes 0 accept
		counter packets 30645 bytes 1847052 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 73137 bytes 5704685 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 3281 bytes 751349 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 93 bytes 4268 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 511 bytes 24496 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 28584 bytes 8826998 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 1 bytes 28 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 178664 bytes 67784554 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 30429 bytes 1372318 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 30429 bytes 9382577 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 6800 bytes 416176 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 1199521 bytes 72558212 accept
		ip protocol udp ct state new counter packets 687971 bytes 717878027 accept
	}
	...
	chain INPUT {
		...
		counter packets 269836006 bytes 325839331285 jump ufw-before-logging-input
		counter packets 269836006 bytes 325839331285 jump ufw-before-input
		counter packets 788834 bytes 129830087 jump ufw-after-input
		counter packets 504563 bytes 46733709 jump ufw-after-logging-input
		counter packets 504563 bytes 46733709 jump ufw-reject-input
		counter packets 504563 bytes 46733709 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 213028379 bytes 205927381900 jump ufw-before-logging-output
		counter packets 213028379 bytes 205927381900 jump ufw-before-output
		counter packets 1896952 bytes 791522411 jump ufw-after-output
		counter packets 1896952 bytes 791522411 jump ufw-after-logging-output
		counter packets 1896952 bytes 791522411 jump ufw-reject-output
		counter packets 1896952 bytes 791522411 jump ufw-track-output
	}
	...
	chain FORWARD {
		...
		counter packets 3820938 bytes 2501863110 jump ufw-before-logging-forward
		counter packets 3820938 bytes 2501863110 jump ufw-before-forward
		counter packets 30645 bytes 1847052 jump ufw-after-forward
		counter packets 30645 bytes 1847052 jump ufw-after-logging-forward
		counter packets 30645 bytes 1847052 jump ufw-reject-forward
		counter packets 30645 bytes 1847052 jump ufw-track-forward
		iifname "wg-home" counter packets 30645 bytes 1847052 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 12585 bytes 747224 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 12585 bytes 747224 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 284271 bytes 83096378 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 1736122 bytes 120020313 return
		fib daddr type multicast counter packets 4866 bytes 155712 return
		fib daddr type broadcast counter packets 283614 bytes 83063531 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 1164568 bytes 69285854 accept
		tcp dport 80 counter packets 71068 bytes 4112868 accept
		tcp dport 52000 counter packets 67 bytes 3564 accept
		udp dport 21195 counter packets 1 bytes 439 accept
		udp dport 21196 counter packets 1 bytes 439 accept
		tcp dport 21197 counter packets 39 bytes 2176 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 47 | 2026-08-28 04:19:44 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-08-27 18:22:16 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 280K
drwxr-xr-x  2 root root   4.0K Aug 28 03:20 .
