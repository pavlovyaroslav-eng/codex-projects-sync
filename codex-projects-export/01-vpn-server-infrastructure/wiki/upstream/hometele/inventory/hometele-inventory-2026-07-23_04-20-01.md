...
Generated: 2026-07-23T04:20:01+03:00  
Host: hometele  
...
Linux hometele 6.8.0-136-generic #136-Ubuntu SMP PREEMPT_DYNAMIC Wed Jul  1 21:53:05 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:01 up 5 days, 59 min,  1 user,  load average: 0.00, 0.00, 0.00
```
...
udp   UNCONN 0      0           10.8.1.2:34394      0.0.0.0:*    users:(("xray",pid=52736,fd=276))                                                                              
...
udp   UNCONN 0      0           10.8.1.2:49052      0.0.0.0:*    users:(("xray",pid=52736,fd=781))                                                                              
udp   UNCONN 0      0           10.8.1.2:60316      0.0.0.0:*    users:(("xray",pid=52736,fd=742))                                                                              
udp   UNCONN 0      0           10.8.1.2:44984      0.0.0.0:*    users:(("xray",pid=52736,fd=612))                                                                              
udp   UNCONN 0      0           10.8.1.2:59387      0.0.0.0:*    users:(("xray",pid=52736,fd=330))                                                                              
udp   UNCONN 0      0           10.8.1.2:47153      0.0.0.0:*    users:(("xray",pid=52736,fd=166))                                                                              
...
udp   UNCONN 0      0           10.8.1.2:60567      0.0.0.0:*    users:(("xray",pid=52736,fd=513))                                                                              
udp   UNCONN 0      0           10.8.1.2:54450      0.0.0.0:*    users:(("xray",pid=52736,fd=798))                                                                              
udp   UNCONN 0      0           10.8.1.2:58647      0.0.0.0:*    users:(("xray",pid=52736,fd=688))                                                                              
udp   UNCONN 0      0           10.8.1.2:46457      0.0.0.0:*    users:(("xray",pid=52736,fd=646))                                                                              
...
udp   UNCONN 0      0           10.8.1.2:41454      0.0.0.0:*    users:(("xray",pid=52736,fd=571))                                                                              
udp   UNCONN 0      0           10.8.1.2:33277      0.0.0.0:*    users:(("xray",pid=52736,fd=310))                                                                              
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 21789 bytes 2761490 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 21789 bytes 3499357 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 21757 bytes 1306401 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 6635878 bytes 3020190534 accept
		ct state related,established counter packets 120402667 bytes 141655569005 accept
		ct state invalid counter packets 76480 bytes 3873539 jump ufw-logging-deny
		ct state invalid counter packets 76480 bytes 3873539 drop
		...
		ip protocol icmp icmp type echo-request counter packets 1537 bytes 79899 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 1085888 bytes 126712297 jump ufw-not-local
		...
		counter packets 1085888 bytes 126712297 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 6635878 bytes 3020190534 accept
		ct state related,established counter packets 95981860 bytes 85178859617 accept
		counter packets 923977 bytes 315533250 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 3644242 bytes 2507442786 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 4 bytes 112 accept
		counter packets 179722 bytes 10789281 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 15505 bytes 1209300 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 2446 bytes 560134 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 49 bytes 2236 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 382 bytes 18436 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 37842 bytes 16329818 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 173791 bytes 57879289 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 21789 bytes 1049658 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 21789 bytes 5230933 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 21757 bytes 1306385 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 619045 bytes 37509114 accept
		ip protocol udp ct state new counter packets 298468 bytes 277195824 accept
	}
	...
	chain INPUT {
		...
		counter packets 128202450 bytes 144806425274 jump ufw-before-logging-input
		counter packets 128202450 bytes 144806425274 jump ufw-before-input
		counter packets 539570 bytes 94137774 jump ufw-after-input
		counter packets 309555 bytes 18138561 jump ufw-after-logging-input
		counter packets 309555 bytes 18138561 jump ufw-reject-input
		counter packets 309555 bytes 18138561 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 103541715 bytes 88514583401 jump ufw-before-logging-output
		counter packets 103541715 bytes 88514583401 jump ufw-before-output
		counter packets 923977 bytes 315533250 jump ufw-after-output
		counter packets 923977 bytes 315533250 jump ufw-after-logging-output
		counter packets 923977 bytes 315533250 jump ufw-reject-output
		counter packets 923977 bytes 315533250 jump ufw-track-output
	}
	...
	chain FORWARD {
		...
		counter packets 3823968 bytes 2518232179 jump ufw-before-logging-forward
		counter packets 3823968 bytes 2518232179 jump ufw-before-forward
		counter packets 179722 bytes 10789281 jump ufw-after-forward
		counter packets 179722 bytes 10789281 jump ufw-after-logging-forward
		counter packets 179722 bytes 10789281 jump ufw-reject-forward
		counter packets 179722 bytes 10789281 jump ufw-track-forward
		iifname "wg-home" counter packets 175369 bytes 10527940 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 10825 bytes 614105 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 10825 bytes 614105 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 230015 bytes 75999213 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 852877 bytes 50626578 return
		fib daddr type multicast counter packets 3483 bytes 111456 return
		fib daddr type broadcast counter packets 229528 bytes 75974263 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 543450 bytes 32401112 accept
		tcp dport 80 counter packets 2529 bytes 136483 accept
		tcp dport 52000 counter packets 113 bytes 6624 accept
		udp dport 21195 counter packets 0 bytes 0 accept
		udp dport 21196 counter packets 0 bytes 0 accept
		tcp dport 21197 counter packets 28 bytes 1520 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 42 | 2026-07-23 04:19:41 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-07-22 12:59:49 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 112K
drwxr-xr-x  2 root root   4.0K Jul 23 03:20 .
