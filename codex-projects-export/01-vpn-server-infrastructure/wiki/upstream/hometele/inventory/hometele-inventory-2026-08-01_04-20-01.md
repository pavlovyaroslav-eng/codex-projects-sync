...
Generated: 2026-08-01T04:20:01+03:00  
Host: hometele  
...
Linux hometele 6.8.0-136-generic #136-Ubuntu SMP PREEMPT_DYNAMIC Wed Jul  1 21:53:05 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:01 up 14 days, 59 min,  1 user,  load average: 0.06, 0.07, 0.01
```
...
udp   UNCONN 0      0           10.8.1.2:33432      0.0.0.0:*    users:(("xray",pid=172876,fd=1355))                                                                                
udp   UNCONN 0      0           10.8.1.2:34481      0.0.0.0:*    users:(("xray",pid=172876,fd=1169))                                                                                
...
udp   UNCONN 0      0           10.8.1.2:38610      0.0.0.0:*    users:(("xray",pid=172876,fd=1751))                                                                                
udp   UNCONN 0      0           10.8.1.2:34631      0.0.0.0:*    users:(("xray",pid=172876,fd=1304))                                                                                
udp   UNCONN 0      0           10.8.1.2:39816      0.0.0.0:*    users:(("xray",pid=172876,fd=1629))                                                                                
udp   UNCONN 0      0           10.8.1.2:47041      0.0.0.0:*    users:(("xray",pid=172876,fd=1574))                                                                                
...
udp   UNCONN 0      0           10.8.1.2:41076      0.0.0.0:*    users:(("xray",pid=172876,fd=1075))                                                                                
udp   UNCONN 0      0           10.8.1.2:47342      0.0.0.0:*    users:(("xray",pid=172876,fd=1202))                                                                                
udp   UNCONN 0      0           10.8.1.2:44368      0.0.0.0:*    users:(("xray",pid=172876,fd=1321))                                                                                
udp   UNCONN 0      0           10.8.1.2:60797      0.0.0.0:*    users:(("xray",pid=172876,fd=1918))                                                                                
udp   UNCONN 0      0           10.8.1.2:51629      0.0.0.0:*    users:(("xray",pid=172876,fd=1633))                                                                                
...
tcp   LISTEN 0      4096         0.0.0.0:52000      0.0.0.0:*    users:(("sshd",pid=236258,fd=3),("systemd",pid=1,fd=186))                                                          
...
tcp   LISTEN 0      4096            [::]:52000         [::]:*    users:(("sshd",pid=236258,fd=4),("systemd",pid=1,fd=187))                                                          
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 60669 bytes 7387968 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 60669 bytes 10166876 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 60636 bytes 3640773 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 26783293 bytes 12314516919 accept
		ct state related,established counter packets 381226426 bytes 465512089216 accept
		ct state invalid counter packets 137329 bytes 6846607 jump ufw-logging-deny
		ct state invalid counter packets 137329 bytes 6846607 drop
		...
		ip protocol icmp icmp type echo-request counter packets 3883 bytes 193103 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 3516116 bytes 358715046 jump ufw-not-local
		...
		counter packets 3516116 bytes 358715046 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 26783293 bytes 12314516919 accept
		ct state related,established counter packets 301840988 bytes 280884233932 accept
		counter packets 2975869 bytes 1008043302 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 9481682 bytes 6285910305 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 4 bytes 112 accept
		counter packets 492627 bytes 29594281 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 57562 bytes 4489681 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 9258 bytes 2120103 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 136 bytes 6172 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 1164 bytes 56432 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 74264 bytes 27549480 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 453973 bytes 154441103 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 60669 bytes 2946058 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 60669 bytes 15342371 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 60636 bytes 3640757 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 2028597 bytes 123002804 accept
		ip protocol udp ct state new counter packets 928525 bytes 882842673 accept
	}
	...
	chain INPUT {
		...
		counter packets 411667047 bytes 478192360891 jump ufw-before-logging-input
		counter packets 411667047 bytes 478192360891 jump ufw-before-input
		counter packets 1336037 bytes 232035804 jump ufw-after-input
		counter packets 739680 bytes 43372833 jump ufw-after-logging-input
		counter packets 739680 bytes 43372833 jump ufw-reject-input
		counter packets 739680 bytes 43372833 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 331600150 bytes 294206794153 jump ufw-before-logging-output
		counter packets 331600150 bytes 294206794153 jump ufw-before-output
		counter packets 2975869 bytes 1008043302 jump ufw-after-output
		counter packets 2975869 bytes 1008043302 jump ufw-after-logging-output
		counter packets 2975869 bytes 1008043302 jump ufw-reject-output
		counter packets 2975869 bytes 1008043302 jump ufw-track-output
	}
	...
	chain FORWARD {
		...
		counter packets 9974313 bytes 6315504698 jump ufw-before-logging-forward
		counter packets 9974313 bytes 6315504698 jump ufw-before-forward
		counter packets 492627 bytes 29594281 jump ufw-after-forward
		counter packets 492627 bytes 29594281 jump ufw-after-logging-forward
		counter packets 492627 bytes 29594281 jump ufw-reject-forward
		counter packets 492627 bytes 29594281 jump ufw-track-forward
		iifname "wg-home" counter packets 488274 bytes 29332940 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 27108 bytes 1527250 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 27108 bytes 1527250 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 596357 bytes 188662971 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 2911486 bytes 169814030 return
		fib daddr type multicast counter packets 9700 bytes 310400 return
		fib daddr type broadcast counter packets 594930 bytes 188590616 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 2165694 bytes 125837764 accept
		tcp dport 80 counter packets 13926 bytes 797712 accept
		tcp dport 52000 counter packets 199 bytes 11534 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 44 | 2026-08-01 04:19:55 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-07-31 22:59:49 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 152K
drwxr-xr-x  2 root root   4.0K Aug  1 03:20 .
drwxrwxr-x 16 root syslog 4.0K Aug  1 00:00 ..
