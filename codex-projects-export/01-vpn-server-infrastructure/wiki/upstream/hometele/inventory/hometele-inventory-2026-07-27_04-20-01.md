...
Generated: 2026-07-27T04:20:01+03:00  
Host: hometele  
...
Linux hometele 6.8.0-136-generic #136-Ubuntu SMP PREEMPT_DYNAMIC Wed Jul  1 21:53:05 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:01 up 9 days, 59 min,  1 user,  load average: 0.00, 0.00, 0.00
```
...
udp   UNCONN 0      0           10.8.1.2:33366      0.0.0.0:*    users:(("xray",pid=115044,fd=1336))                                                                            
...
udp   UNCONN 0      0           10.8.1.2:36630      0.0.0.0:*    users:(("xray",pid=115044,fd=1481))                                                                            
udp   UNCONN 0      0           10.8.1.2:54044      0.0.0.0:*    users:(("xray",pid=115044,fd=954))                                                                             
udp   UNCONN 0      0           10.8.1.2:40799      0.0.0.0:*    users:(("xray",pid=115044,fd=1777))                                                                            
udp   UNCONN 0      0           10.8.1.2:54200      0.0.0.0:*    users:(("xray",pid=115044,fd=1217))                                                                            
udp   UNCONN 0      0           10.8.1.2:35789      0.0.0.0:*    users:(("xray",pid=115044,fd=1724))                                                                            
udp   UNCONN 0      0           10.8.1.2:35816      0.0.0.0:*    users:(("xray",pid=115044,fd=1170))                                                                            
...
udp   UNCONN 0      0           10.8.1.2:47177      0.0.0.0:*    users:(("xray",pid=115044,fd=1712))                                                                            
udp   UNCONN 0      0           10.8.1.2:46172      0.0.0.0:*    users:(("xray",pid=115044,fd=1677))                                                                            
udp   UNCONN 0      0           10.8.1.2:42334      0.0.0.0:*    users:(("xray",pid=115044,fd=1798))                                                                            
udp   UNCONN 0      0           10.8.1.2:53633      0.0.0.0:*    users:(("xray",pid=115044,fd=1804))                                                                            
...
udp   UNCONN 0      0           10.8.1.2:42523      0.0.0.0:*    users:(("xray",pid=115044,fd=1751))                                                                            
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 39069 bytes 4774958 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 39069 bytes 6673827 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 39037 bytes 2343905 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 11295338 bytes 5132404402 accept
		ct state related,established counter packets 227307703 bytes 280371176982 accept
		ct state invalid counter packets 90232 bytes 4684307 jump ufw-logging-deny
		ct state invalid counter packets 90232 bytes 4684307 drop
		...
		ip protocol icmp icmp type echo-request counter packets 2758 bytes 141421 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 1849817 bytes 213610187 jump ufw-not-local
		...
		counter packets 1849817 bytes 213610187 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 11295338 bytes 5132404402 accept
		ct state related,established counter packets 178249903 bytes 166782431070 accept
		counter packets 1766638 bytes 618122699 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 6165923 bytes 4146601197 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 4 bytes 112 accept
		counter packets 317161 bytes 19044197 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 50859 bytes 3966912 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 5482 bytes 1255291 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 85 bytes 3856 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 697 bytes 33868 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 54026 bytes 21314490 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 314243 bytes 103591218 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 39069 bytes 1883937 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 39069 bytes 9895267 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 39037 bytes 2343889 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 1179055 bytes 71340721 accept
		ip protocol udp ct state new counter packets 576475 bytes 545449102 accept
	}
	...
	chain INPUT {
		...
		counter packets 240545848 bytes 285722017299 jump ufw-before-logging-input
		counter packets 240545848 bytes 285722017299 jump ufw-before-input
		counter packets 924942 bytes 157897061 jump ufw-after-input
		counter packets 499550 bytes 27731426 jump ufw-after-logging-input
		counter packets 499550 bytes 27731426 jump ufw-reject-input
		counter packets 499550 bytes 27731426 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 191311879 bytes 172532958171 jump ufw-before-logging-output
		counter packets 191311879 bytes 172532958171 jump ufw-before-output
		counter packets 1766638 bytes 618122699 jump ufw-after-output
		counter packets 1766638 bytes 618122699 jump ufw-after-logging-output
		counter packets 1766638 bytes 618122699 jump ufw-reject-output
		counter packets 1766638 bytes 618122699 jump ufw-track-output
	}
	...
	chain FORWARD {
		...
		counter packets 6483088 bytes 4165645506 jump ufw-before-logging-forward
		counter packets 6483088 bytes 4165645506 jump ufw-before-forward
		counter packets 317161 bytes 19044197 jump ufw-after-forward
		counter packets 317161 bytes 19044197 jump ufw-after-logging-forward
		counter packets 317161 bytes 19044197 jump ufw-reject-forward
		counter packets 317161 bytes 19044197 jump ufw-track-forward
		iifname "wg-home" counter packets 312808 bytes 18782856 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 17469 bytes 1026387 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 17469 bytes 1026387 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 425392 bytes 130165635 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 1419031 bytes 83287866 return
		fib daddr type multicast counter packets 6248 bytes 199936 return
		fib daddr type broadcast counter packets 424538 bytes 130122385 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 912856 bytes 54998740 accept
		tcp dport 80 counter packets 11643 bytes 675360 accept
		tcp dport 52000 counter packets 141 bytes 8210 accept
		udp dport 21195 counter packets 0 bytes 0 accept
		udp dport 21196 counter packets 0 bytes 0 accept
		tcp dport 21197 counter packets 37 bytes 2032 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 43 | 2026-07-27 04:19:59 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-07-26 15:56:49 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 132K
drwxr-xr-x  2 root root   4.0K Jul 27 03:20 .
