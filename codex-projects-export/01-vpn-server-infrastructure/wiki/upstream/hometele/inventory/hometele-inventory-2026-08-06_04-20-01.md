...
Generated: 2026-08-06T04:20:01+03:00  
Host: hometele  
...
Linux hometele 6.8.0-136-generic #136-Ubuntu SMP PREEMPT_DYNAMIC Wed Jul  1 21:53:05 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:01 up 19 days, 59 min,  1 user,  load average: 0.03, 0.04, 0.01
```
...
udp   UNCONN 0      0          127.0.0.1:10808      0.0.0.0:*    users:(("xray",pid=326671,fd=7))                                                                                   
udp   UNCONN 0      0           10.8.1.2:47690      0.0.0.0:*    users:(("xray",pid=326671,fd=165))                                                                                 
udp   UNCONN 0      0            0.0.0.0:51820      0.0.0.0:*                                                                                                                       
udp   UNCONN 0      0           10.8.1.2:57027      0.0.0.0:*    users:(("xray",pid=326671,fd=53))                                                                                  
...
udp   UNCONN 0      0           10.8.1.2:59203      0.0.0.0:*    users:(("xray",pid=326671,fd=66))                                                                                  
udp   UNCONN 0      0            0.0.0.0:58240      0.0.0.0:*    users:(("openvpn",pid=261409,fd=3))                                                                                
udp   UNCONN 0      0           10.8.1.2:59331      0.0.0.0:*    users:(("xray",pid=326671,fd=10))                                                                                  
udp   UNCONN 0      0           10.8.1.2:39974      0.0.0.0:*    users:(("xray",pid=326671,fd=27))                                                                                  
...
udp   UNCONN 0      0           10.8.1.2:55370      0.0.0.0:*    users:(("xray",pid=326671,fd=115))                                                                                 
udp   UNCONN 0      0           10.8.1.2:45133      0.0.0.0:*    users:(("xray",pid=326671,fd=131))                                                                                 
udp   UNCONN 0      0           10.8.1.2:32930      0.0.0.0:*    users:(("xray",pid=326671,fd=36))                                                                                  
udp   UNCONN 0      0           10.8.1.2:37318      0.0.0.0:*    users:(("xray",pid=326671,fd=150))                                                                                 
...
tcp   LISTEN 0      4096       127.0.0.1:10808      0.0.0.0:*    users:(("xray",pid=326671,fd=6))                                                                                   
...
tcp   LISTEN 0      4096               *:443              *:*    users:(("xray",pid=326671,fd=3))                                                                                   
```
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 82269 bytes 9815241 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 82269 bytes 13763728 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 72373 bytes 4348657 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 34185971 bytes 15663083415 accept
		ct state related,established counter packets 526521236 bytes 645127224064 accept
		ct state invalid counter packets 157440 bytes 8010263 jump ufw-logging-deny
		ct state invalid counter packets 157440 bytes 8010263 drop
		...
		ip protocol icmp icmp type echo-request counter packets 5385 bytes 270751 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 4588516 bytes 468327813 jump ufw-not-local
		...
		counter packets 4588516 bytes 468327813 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 34185971 bytes 15663083415 accept
		ct state related,established counter packets 417587896 bytes 388189631592 accept
		counter packets 4251651 bytes 1404864137 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 12860211 bytes 8559095468 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 4 bytes 112 accept
		counter packets 575298 bytes 34560237 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 78308 bytes 6107869 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 12986 bytes 2973929 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 181 bytes 8204 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 1555 bytes 75508 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 94494 bytes 33780040 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 586836 bytes 203897660 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 82269 bytes 3980555 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 82269 bytes 20742880 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 72373 bytes 4348641 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 2933812 bytes 178516229 accept
		ip protocol udp ct state new counter packets 1287386 bytes 1222892966 accept
	}
	...
	chain INPUT {
		...
		counter packets 565458548 bytes 661266916306 jump ufw-before-logging-input
		counter packets 565458548 bytes 661266916306 jump ufw-before-input
		counter packets 1805224 bytes 304844655 jump ufw-after-input
		counter packets 1030864 bytes 58001445 jump ufw-after-logging-input
		counter packets 1030864 bytes 58001445 jump ufw-reject-input
		counter packets 1030864 bytes 58001445 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 456025518 bytes 405257579144 jump ufw-before-logging-output
		counter packets 456025518 bytes 405257579144 jump ufw-before-output
		counter packets 4251651 bytes 1404864137 jump ufw-after-output
		counter packets 4251651 bytes 1404864137 jump ufw-after-logging-output
		counter packets 4251651 bytes 1404864137 jump ufw-reject-output
		counter packets 4251651 bytes 1404864137 jump ufw-track-output
	}
	...
	chain FORWARD {
		...
		counter packets 13435513 bytes 8593655817 jump ufw-before-logging-forward
		counter packets 13435513 bytes 8593655817 jump ufw-before-forward
		counter packets 575298 bytes 34560237 jump ufw-after-forward
		counter packets 575298 bytes 34560237 jump ufw-after-logging-forward
		counter packets 575298 bytes 34560237 jump ufw-reject-forward
		counter packets 575298 bytes 34560237 jump ufw-track-forward
		iifname "wg-home" counter packets 570945 bytes 34298896 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 38227 bytes 2125960 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 38227 bytes 2125960 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 774360 bytes 246843210 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 3802892 bytes 221159240 return
		fib daddr type multicast counter packets 13155 bytes 420960 return
		fib daddr type broadcast counter packets 772469 bytes 246747613 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 2760134 bytes 162148033 accept
		tcp dport 80 counter packets 22610 bytes 1286285 accept
		tcp dport 52000 counter packets 282 bytes 16356 accept
		udp dport 21195 counter packets 0 bytes 0 accept
		udp dport 21196 counter packets 0 bytes 0 accept
		tcp dport 21197 counter packets 68 bytes 3700 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 47 | 2026-08-06 04:19:52 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-08-05 19:27:41 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 176K
drwxr-xr-x  2 root root   4.0K Aug  6 03:20 .
