...
Generated: 2026-09-13T04:20:03+03:00  
Host: hometele  
...
Linux hometele 6.8.0-139-generic #139-Ubuntu SMP PREEMPT_DYNAMIC Sat Aug  1 03:52:05 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:03 up 7 days, 59 min,  0 user,  load average: 0.16, 0.06, 0.01
```
...
udp   UNCONN 0      0                  *:48372            *:*    users:(("rw-core",pid=252273,fd=9))                                                                                
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 30429 bytes 3895145 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 30429 bytes 2676257 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 30429 bytes 1826908 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 65979347 bytes 45406194145 accept
		ct state related,established counter packets 186146083 bytes 133689320732 accept
		ct state invalid counter packets 17342 bytes 1048945 jump ufw-logging-deny
		ct state invalid counter packets 17342 bytes 1048945 drop
		...
		ip protocol icmp icmp type echo-request counter packets 2188 bytes 112682 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 1506699 bytes 200670967 jump ufw-not-local
		...
		counter packets 1506699 bytes 200670967 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 65979347 bytes 45406194145 accept
		ct state related,established counter packets 149862980 bytes 136089983977 accept
		counter packets 1718008 bytes 208161372 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 4638844 bytes 3319807172 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 0 bytes 0 accept
		counter packets 225977 bytes 13566748 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 58435 bytes 4557930 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 2536 bytes 580123 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 114 bytes 5472 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 728 bytes 34284 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 28493 bytes 8775664 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 6 bytes 168 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 194789 bytes 70966285 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 30429 bytes 1431247 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 30429 bytes 3413470 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 30429 bytes 1826908 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 989490 bytes 59411156 accept
		ip protocol udp ct state new counter packets 708657 bytes 140909026 accept
	}
	...
	chain INPUT {
		...
		counter packets 253651659 bytes 179297347471 jump ufw-before-logging-input
		counter packets 253651659 bytes 179297347471 jump ufw-before-input
		counter packets 622271 bytes 101246893 jump ufw-after-input
		counter packets 337170 bytes 16326967 jump ufw-after-logging-input
		counter packets 337170 bytes 16326967 jump ufw-reject-input
		counter packets 337170 bytes 16326967 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 217560335 bytes 181704339494 jump ufw-before-logging-output
		counter packets 217560335 bytes 181704339494 jump ufw-before-output
		counter packets 1718008 bytes 208161372 jump ufw-after-output
		counter packets 1718008 bytes 208161372 jump ufw-after-logging-output
		counter packets 1718008 bytes 208161372 jump ufw-reject-output
		counter packets 1718008 bytes 208161372 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 5815175 bytes 3657356686 jump DOCKER-USER
		counter packets 5815175 bytes 3657356686 jump DOCKER-FORWARD
		...
		counter packets 4864821 bytes 3333373920 jump ufw-before-logging-forward
		counter packets 4864821 bytes 3333373920 jump ufw-before-forward
		counter packets 225977 bytes 13566748 jump ufw-after-forward
		counter packets 225977 bytes 13566748 jump ufw-after-logging-forward
		counter packets 225977 bytes 13566748 jump ufw-reject-forward
		counter packets 225977 bytes 13566748 jump ufw-track-forward
		iifname "wg-home" counter packets 225977 bytes 13566748 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 9060 bytes 529331 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 9060 bytes 529331 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 285101 bytes 84919926 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 1217660 bytes 115643129 return
		fib daddr type multicast counter packets 4867 bytes 155744 return
		fib daddr type broadcast counter packets 284172 bytes 84872094 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 525477 bytes 29920911 accept
		tcp dport 80 counter packets 276851 bytes 15233828 accept
		tcp dport 52000 counter packets 436 bytes 25504 accept
		...
		ip saddr 93.183.106.203 tcp dport 2222 counter packets 1260 bytes 75600 accept
		udp dport 443 counter packets 80344 bytes 54162993 accept
	}
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 49 | 2026-09-13 04:19:38 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-09-12 14:29:22 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
