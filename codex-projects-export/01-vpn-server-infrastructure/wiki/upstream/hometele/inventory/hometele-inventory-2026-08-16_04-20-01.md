...
Generated: 2026-08-16T04:20:01+03:00  
Host: hometele  
...
Linux hometele 6.8.0-137-generic #137-Ubuntu SMP PREEMPT_DYNAMIC Fri Jul 17 20:28:23 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:01 up 8 days, 59 min,  0 user,  load average: 0.00, 0.01, 0.00
```
...
udp   UNCONN 0      0           10.8.1.2:40380      0.0.0.0:*    users:(("xray",pid=116591,fd=973))                                                                             
...
udp   UNCONN 0      0           10.8.1.2:53941      0.0.0.0:*    users:(("xray",pid=116591,fd=1283))                                                                            
udp   UNCONN 0      0           10.8.1.2:46786      0.0.0.0:*    users:(("xray",pid=116591,fd=1395))                                                                            
...
udp   UNCONN 0      0           10.8.1.2:54013      0.0.0.0:*    users:(("xray",pid=116591,fd=1298))                                                                            
udp   UNCONN 0      0           10.8.1.2:44903      0.0.0.0:*    users:(("xray",pid=116591,fd=326))                                                                             
udp   UNCONN 0      0           10.8.1.2:39817      0.0.0.0:*    users:(("xray",pid=116591,fd=35))                                                                              
udp   UNCONN 0      0            0.0.0.0:42910      0.0.0.0:*    users:(("openvpn",pid=827,fd=4))                                                                               
udp   UNCONN 0      0           10.8.1.2:37831      0.0.0.0:*    users:(("xray",pid=116591,fd=1409))                                                                            
udp   UNCONN 0      0           10.8.1.2:35796      0.0.0.0:*    users:(("xray",pid=116591,fd=998))                                                                             
udp   UNCONN 0      0           10.8.1.2:53229      0.0.0.0:*    users:(("xray",pid=116591,fd=1207))                                                                            
...
udp   UNCONN 0      0           10.8.1.2:50350      0.0.0.0:*    users:(("xray",pid=116591,fd=1004))                                                                            
udp   UNCONN 0      0           10.8.1.2:43196      0.0.0.0:*    users:(("xray",pid=116591,fd=1071))                                                                            
udp   UNCONN 0      0           10.8.1.2:33021      0.0.0.0:*    users:(("xray",pid=116591,fd=821))                                                                             
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 34749 bytes 3588472 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 34749 bytes 5947045 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 6738 bytes 414188 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 17119531 bytes 7824187991 accept
		ct state related,established counter packets 284438062 bytes 350200405902 accept
		ct state invalid counter packets 54597 bytes 2582581 jump ufw-logging-deny
		ct state invalid counter packets 54597 bytes 2582581 drop
		...
		ip protocol icmp icmp type echo-request counter packets 2292 bytes 114579 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 2120934 bytes 197204846 jump ufw-not-local
		...
		counter packets 2120934 bytes 197204846 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 17119531 bytes 7824187991 accept
		ct state related,established counter packets 227475257 bytes 210936311193 accept
		counter packets 2147943 bytes 750293885 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 4893418 bytes 3304806935 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 0 bytes 0 accept
		counter packets 29611 bytes 1786856 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 27063 bytes 2110914 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 5379 bytes 1231791 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 90 bytes 4108 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 602 bytes 28964 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 32642 bytes 10078984 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 208838 bytes 78345861 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 34749 bytes 1662094 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 34749 bytes 8699296 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 6740 bytes 414292 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 1411856 bytes 85594836 accept
		ip protocol udp ct state new counter packets 718879 bytes 663023670 accept
	}
	...
	chain INPUT {
		...
		counter packets 303735416 bytes 358224495899 jump ufw-before-logging-input
		counter packets 303735416 bytes 358224495899 jump ufw-before-input
		counter packets 729775 bytes 114496555 jump ufw-after-input
		counter packets 455161 bytes 22695933 jump ufw-after-logging-input
		counter packets 455161 bytes 22695933 jump ufw-reject-input
		counter packets 455161 bytes 22695933 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 246742731 bytes 219510793069 jump ufw-before-logging-output
		counter packets 246742731 bytes 219510793069 jump ufw-before-output
		counter packets 2147943 bytes 750293885 jump ufw-after-output
		counter packets 2147943 bytes 750293885 jump ufw-after-logging-output
		counter packets 2147943 bytes 750293885 jump ufw-reject-output
		counter packets 2147943 bytes 750293885 jump ufw-track-output
	}
	...
	chain FORWARD {
		...
		counter packets 4923029 bytes 3306593791 jump ufw-before-logging-forward
		counter packets 4923029 bytes 3306593791 jump ufw-before-forward
		counter packets 29611 bytes 1786856 jump ufw-after-forward
		counter packets 29611 bytes 1786856 jump ufw-after-logging-forward
		counter packets 29611 bytes 1786856 jump ufw-reject-forward
		counter packets 29611 bytes 1786856 jump ufw-track-forward
		iifname "wg-home" counter packets 29611 bytes 1786856 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 20410 bytes 993754 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 20410 bytes 993754 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 274614 bytes 91800622 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 1841512 bytes 105263820 return
		fib daddr type multicast counter packets 5557 bytes 177824 return
		fib daddr type broadcast counter packets 273865 bytes 91763202 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 1387437 bytes 82502998 accept
		tcp dport 80 counter packets 3437 bytes 187127 accept
		tcp dport 52000 counter packets 239 bytes 13908 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 44 | 2026-08-16 04:20:00 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-08-15 17:24:39 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 224K
drwxr-xr-x  2 root root   4.0K Aug 16 03:30 .
