...
Generated: 2026-08-02T04:20:02+03:00  
Host: hometele  
...
Linux hometele 6.8.0-136-generic #136-Ubuntu SMP PREEMPT_DYNAMIC Wed Jul  1 21:53:05 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:02 up 15 days, 59 min,  1 user,  load average: 0.00, 0.00, 0.00
```
...
udp   UNCONN 0      0           10.8.1.2:39547      0.0.0.0:*    users:(("xray",pid=261657,fd=114))                                                                                 
udp   UNCONN 0      0           10.8.1.2:56956      0.0.0.0:*    users:(("xray",pid=261657,fd=98))                                                                                  
udp   UNCONN 0      0           10.8.1.2:34452      0.0.0.0:*    users:(("xray",pid=261657,fd=90))                                                                                  
udp   UNCONN 0      0           10.8.1.2:52906      0.0.0.0:*    users:(("xray",pid=261657,fd=118))                                                                                 
...
udp   UNCONN 0      0           10.8.1.2:36674      0.0.0.0:*    users:(("xray",pid=261657,fd=102))                                                                                 
udp   UNCONN 0      0            0.0.0.0:58240      0.0.0.0:*    users:(("openvpn",pid=261409,fd=3))                                                                                
udp   UNCONN 0      0           10.8.1.2:52158      0.0.0.0:*    users:(("xray",pid=261657,fd=50))                                                                                  
udp   UNCONN 0      0           10.8.1.2:33832      0.0.0.0:*    users:(("xray",pid=261657,fd=92))                                                                                  
udp   UNCONN 0      0           10.8.1.2:46120      0.0.0.0:*    users:(("xray",pid=261657,fd=107))                                                                                 
...
udp   UNCONN 0      0           10.8.1.2:45167      0.0.0.0:*    users:(("xray",pid=261657,fd=73))                                                                                  
udp   UNCONN 0      0           10.8.1.2:55464      0.0.0.0:*    users:(("xray",pid=261657,fd=95))                                                                                  
udp   UNCONN 0      0           10.8.1.2:50365      0.0.0.0:*    users:(("xray",pid=261657,fd=56))                                                                                  
udp   UNCONN 0      0           10.8.1.2:53670      0.0.0.0:*    users:(("xray",pid=261657,fd=19))                                                                                  
...
udp   UNCONN 0      0           10.8.1.2:34282      0.0.0.0:*    users:(("xray",pid=261657,fd=26))                                                                                  
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 64989 bytes 7894125 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 64989 bytes 10797373 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 64957 bytes 3900241 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 28376836 bytes 13043606595 accept
		ct state related,established counter packets 414431421 bytes 507208230686 accept
		ct state invalid counter packets 140826 bytes 7070081 jump ufw-logging-deny
		ct state invalid counter packets 140826 bytes 7070081 drop
		...
		ip protocol icmp icmp type echo-request counter packets 4276 bytes 215497 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 3726632 bytes 380868610 jump ufw-not-local
		...
		counter packets 3726632 bytes 380868610 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 28376836 bytes 13043606595 accept
		ct state related,established counter packets 328108066 bytes 305571307437 accept
		counter packets 3259618 bytes 1088702797 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 10003111 bytes 6619563610 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 4 bytes 112 accept
		counter packets 527110 bytes 31664397 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 58670 bytes 4576105 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 10183 bytes 2332042 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 148 bytes 6716 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 1226 bytes 59424 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 78310 bytes 28795648 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 482711 bytes 164669007 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 64989 bytes 3155537 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 64989 bytes 16268460 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 64957 bytes 3900225 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 2234743 bytes 135675655 accept
		ip protocol udp ct state new counter packets 1002059 bytes 950384823 accept
	}
	...
	chain INPUT {
		...
		counter packets 446679991 bytes 520639991469 jump ufw-before-logging-input
		counter packets 446679991 bytes 520639991469 jump ufw-before-input
		counter packets 1416064 bytes 246179192 jump ufw-after-input
		counter packets 784816 bytes 45740250 jump ufw-after-logging-input
		counter packets 784816 bytes 45740250 jump ufw-reject-input
		counter packets 784816 bytes 45740250 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 359744520 bytes 319703616829 jump ufw-before-logging-output
		counter packets 359744520 bytes 319703616829 jump ufw-before-output
		counter packets 3259618 bytes 1088702797 jump ufw-after-output
		counter packets 3259618 bytes 1088702797 jump ufw-after-logging-output
		counter packets 3259618 bytes 1088702797 jump ufw-reject-output
		counter packets 3259618 bytes 1088702797 jump ufw-track-output
	}
	...
	chain FORWARD {
		...
		counter packets 10530225 bytes 6651228119 jump ufw-before-logging-forward
		counter packets 10530225 bytes 6651228119 jump ufw-before-forward
		counter packets 527110 bytes 31664397 jump ufw-after-forward
		counter packets 527110 bytes 31664397 jump ufw-after-logging-forward
		counter packets 527110 bytes 31664397 jump ufw-reject-forward
		counter packets 527110 bytes 31664397 jump ufw-track-forward
		iifname "wg-home" counter packets 522757 bytes 31403056 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 29055 bytes 1658334 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 29055 bytes 1658334 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 631248 bytes 200438942 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 3086504 bytes 180173827 return
		fib daddr type multicast counter packets 10391 bytes 332512 return
		fib daddr type broadcast counter packets 629737 bytes 200362271 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 2293328 bytes 133698234 accept
		tcp dport 80 counter packets 16723 bytes 944020 accept
		tcp dport 52000 counter packets 257 bytes 14932 accept
		udp dport 21195 counter packets 0 bytes 0 accept
		udp dport 21196 counter packets 0 bytes 0 accept
		tcp dport 21197 counter packets 62 bytes 3448 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 45 | 2026-08-02 04:20:00 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-08-01 17:31:25 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
