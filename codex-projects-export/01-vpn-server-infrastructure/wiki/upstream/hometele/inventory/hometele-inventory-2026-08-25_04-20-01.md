...
Generated: 2026-08-25T04:20:01+03:00  
Host: hometele  
...
Linux hometele 6.8.0-138-generic #138-Ubuntu SMP PREEMPT_DYNAMIC Fri Jul 31 22:41:49 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:01 up 4 days, 59 min,  0 user,  load average: 0.00, 0.00, 0.00
```
...
udp   UNCONN 0      0           10.8.1.2:34085      0.0.0.0:*    users:(("xray",pid=971,fd=1409))                                                                           
udp   UNCONN 0      0           10.8.1.2:50510      0.0.0.0:*    users:(("xray",pid=971,fd=1343))                                                                           
udp   UNCONN 0      0           10.8.1.2:51550      0.0.0.0:*    users:(("xray",pid=971,fd=1275))                                                                           
udp   UNCONN 0      0          127.0.0.1:10808      0.0.0.0:*    users:(("xray",pid=971,fd=7))                                                                              
udp   UNCONN 0      0           10.8.1.2:60990      0.0.0.0:*    users:(("xray",pid=971,fd=100))                                                                            
udp   UNCONN 0      0            0.0.0.0:51820      0.0.0.0:*                                                                                                               
udp   UNCONN 0      0           10.8.1.2:38510      0.0.0.0:*    users:(("xray",pid=971,fd=1067))                                                                           
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 17469 bytes 1805713 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 17469 bytes 4471092 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 3942 bytes 241240 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 8332146 bytes 3821735703 accept
		ct state related,established counter packets 148399744 bytes 190921744680 accept
		ct state invalid counter packets 15116 bytes 816291 jump ufw-logging-deny
		ct state invalid counter packets 15116 bytes 816291 drop
		...
		ip protocol icmp icmp type echo-request counter packets 1378 bytes 74671 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 1145499 bytes 125520386 jump ufw-not-local
		...
		counter packets 1145499 bytes 125520386 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 8332146 bytes 3821735703 accept
		ct state related,established counter packets 115233916 bytes 116446607934 accept
		counter packets 1167866 bytes 530000890 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 2023533 bytes 1296621629 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 0 bytes 0 accept
		counter packets 17640 bytes 1063264 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 38450 bytes 2999100 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 1910 bytes 437390 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 48 bytes 2140 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 282 bytes 13440 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 16354 bytes 5037032 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 103722 bytes 39031683 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 17469 bytes 787152 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 17469 bytes 6561499 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 3944 bytes 241344 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 707947 bytes 42857614 accept
		ip protocol udp ct state new counter packets 454232 bytes 486494894 accept
	}
	...
	chain INPUT {
		...
		counter packets 157893883 bytes 194869891731 jump ufw-before-logging-input
		counter packets 157893883 bytes 194869891731 jump ufw-before-input
		counter packets 468348 bytes 85158409 jump ufw-after-input
		counter packets 307582 bytes 37637624 jump ufw-after-logging-input
		counter packets 307582 bytes 37637624 jump ufw-reject-input
		counter packets 307582 bytes 37637624 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 124733928 bytes 120798344527 jump ufw-before-logging-output
		counter packets 124733928 bytes 120798344527 jump ufw-before-output
		counter packets 1167866 bytes 530000890 jump ufw-after-output
		counter packets 1167866 bytes 530000890 jump ufw-after-logging-output
		counter packets 1167866 bytes 530000890 jump ufw-reject-output
		counter packets 1167866 bytes 530000890 jump ufw-track-output
	}
	...
	chain FORWARD {
		...
		counter packets 2041173 bytes 1297684893 jump ufw-before-logging-forward
		counter packets 2041173 bytes 1297684893 jump ufw-before-forward
		counter packets 17640 bytes 1063264 jump ufw-after-forward
		counter packets 17640 bytes 1063264 jump ufw-after-logging-forward
		counter packets 17640 bytes 1063264 jump ufw-reject-forward
		counter packets 17640 bytes 1063264 jump ufw-track-forward
		iifname "wg-home" counter packets 17640 bytes 1063264 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 7912 bytes 441990 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 7912 bytes 441990 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 160766 bytes 47520785 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 982295 bytes 77927755 return
		fib daddr type multicast counter packets 2793 bytes 89376 return
		fib daddr type broadcast counter packets 160411 bytes 47503255 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 671435 bytes 40023285 accept
		tcp dport 80 counter packets 5653 bytes 332117 accept
		tcp dport 52000 counter packets 26 bytes 1356 accept
		udp dport 21195 counter packets 1 bytes 439 accept
		udp dport 21196 counter packets 1 bytes 439 accept
		tcp dport 21197 counter packets 14 bytes 740 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 48 | 2026-08-25 04:19:59 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-08-25 00:44:08 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 268K
drwxr-xr-x  2 root root   4.0K Aug 25 03:20 .
