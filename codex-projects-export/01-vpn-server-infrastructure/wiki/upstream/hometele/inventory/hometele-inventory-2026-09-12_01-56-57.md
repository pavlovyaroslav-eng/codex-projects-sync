...
Generated: 2026-09-12T01:56:57+03:00  
Host: hometele  
...
Linux hometele 6.8.0-139-generic #139-Ubuntu SMP PREEMPT_DYNAMIC Sat Aug  1 03:52:05 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 01:56:57 up 5 days, 22:36,  1 user,  load average: 0.03, 0.03, 0.00
```
...
udp   UNCONN 0      0                  *:49475            *:*    users:(("rw-core",pid=252273,fd=39))                                                                               
udp   UNCONN 0      0                  *:40269            *:*    users:(("rw-core",pid=252273,fd=37))                                                                               
udp   UNCONN 0      0               [::]:59810         [::]:*    users:(("amneziawg-go",pid=905,fd=15))                                                                             
udp   UNCONN 0      0                  *:443              *:*    users:(("rw-core",pid=252273,fd=7))                                                                                
udp   UNCONN 0      0                  *:37326            *:*    users:(("rw-core",pid=252273,fd=31))                                                                               
udp   UNCONN 0      0                  *:38376            *:*    users:(("rw-core",pid=252273,fd=9))                                                                                
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 25680 bytes 3262552 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 25680 bytes 2277959 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 25679 bytes 1541748 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 56330662 bytes 38339458394 accept
		ct state related,established counter packets 160963730 bytes 115171366221 accept
		ct state invalid counter packets 15359 bytes 942159 jump ufw-logging-deny
		ct state invalid counter packets 15359 bytes 942159 drop
		...
		ip protocol icmp icmp type echo-request counter packets 1943 bytes 101176 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 1359392 bytes 176136328 jump ufw-not-local
		...
		counter packets 1359392 bytes 176136328 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 56330662 bytes 38339458394 accept
		ct state related,established counter packets 129238979 bytes 117486795810 accept
		counter packets 1555224 bytes 190465580 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 3767922 bytes 2671907675 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 0 bytes 0 accept
		counter packets 190492 bytes 11436416 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 45308 bytes 3534024 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 2181 bytes 498855 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 97 bytes 4688 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 620 bytes 29256 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 24046 bytes 7405988 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 5 bytes 140 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 164864 bytes 59999888 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 25680 bytes 1202053 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 25679 bytes 2925620 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 25679 bytes 1541748 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 907608 bytes 54482651 accept
		ip protocol udp ct state new counter packets 629380 bytes 128718171 accept
	}
	...
	chain INPUT {
		...
		counter packets 218671086 bytes 153688004278 jump ufw-before-logging-input
		counter packets 218671086 bytes 153688004278 jump ufw-before-input
		counter packets 537053 bytes 85915274 jump ufw-after-input
		counter packets 299932 bytes 14442435 jump ufw-after-logging-input
		counter packets 299932 bytes 14442435 jump ufw-reject-input
		counter packets 299932 bytes 14442435 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 187124865 bytes 156016719784 jump ufw-before-logging-output
		counter packets 187124865 bytes 156016719784 jump ufw-before-output
		counter packets 1555224 bytes 190465580 jump ufw-after-output
		counter packets 1555224 bytes 190465580 jump ufw-after-logging-output
		counter packets 1555224 bytes 190465580 jump ufw-reject-output
		counter packets 1555224 bytes 190465580 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 4761048 bytes 2956825481 jump DOCKER-USER
		counter packets 4761048 bytes 2956825481 jump DOCKER-FORWARD
		...
		counter packets 3958414 bytes 2683344091 jump ufw-before-logging-forward
		counter packets 3958414 bytes 2683344091 jump ufw-before-forward
		counter packets 190492 bytes 11436416 jump ufw-after-forward
		counter packets 190492 bytes 11436416 jump ufw-after-logging-forward
		counter packets 190492 bytes 11436416 jump ufw-reject-forward
		counter packets 190492 bytes 11436416 jump ufw-track-forward
		iifname "wg-home" counter packets 190492 bytes 11436416 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 7878 bytes 465041 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 7878 bytes 465041 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 237121 bytes 71472839 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 1118955 bytes 104572997 return
		fib daddr type multicast counter packets 4107 bytes 131424 return
		fib daddr type broadcast counter packets 236330 bytes 71431907 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 481219 bytes 27270969 accept
		tcp dport 80 counter packets 269219 bytes 14923060 accept
		tcp dport 52000 counter packets 410 bytes 24052 accept
		...
		ip saddr 93.183.106.203 tcp dport 2222 counter packets 1066 bytes 63960 accept
		udp dport 443 counter packets 70368 bytes 47933927 accept
	}
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 48 | 2026-09-12 01:56:35 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-09-11 23:39:58 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
