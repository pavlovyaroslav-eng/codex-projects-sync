...
Generated: 2026-07-21T04:20:01+03:00  
Host: hometele  
...
Linux hometele 6.8.0-136-generic #136-Ubuntu SMP PREEMPT_DYNAMIC Wed Jul  1 21:53:05 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:01 up 3 days, 59 min,  1 user,  load average: 0.06, 0.03, 0.00
```
...
udp   UNCONN 0      0           10.8.1.2:43557      0.0.0.0:*    users:(("xray",pid=52736,fd=99))                                                                       
...
udp   UNCONN 0      0           10.8.1.2:43787      0.0.0.0:*    users:(("xray",pid=52736,fd=90))                                                                       
udp   UNCONN 0      0           10.8.1.2:56251      0.0.0.0:*    users:(("xray",pid=52736,fd=42))                                                                       
udp   UNCONN 0      0           10.8.1.2:60378      0.0.0.0:*    users:(("xray",pid=52736,fd=59))                                                                       
udp   UNCONN 0      0           10.8.1.2:48170      0.0.0.0:*    users:(("xray",pid=52736,fd=54))                                                                       
...
udp   UNCONN 0      0           10.8.1.2:54486      0.0.0.0:*    users:(("xray",pid=52736,fd=83))                                                                       
udp   UNCONN 0      0           10.8.1.2:41207      0.0.0.0:*    users:(("xray",pid=52736,fd=65))                                                                       
udp   UNCONN 0      0           10.8.1.2:46362      0.0.0.0:*    users:(("xray",pid=52736,fd=49))                                                                       
udp   UNCONN 0      0           10.8.1.2:40270      0.0.0.0:*    users:(("xray",pid=52736,fd=103))                                                                      
udp   UNCONN 0      0           10.8.1.2:38308      0.0.0.0:*    users:(("xray",pid=52736,fd=62))                                                                       
...
udp   UNCONN 0      0           10.8.1.2:53734      0.0.0.0:*    users:(("xray",pid=52736,fd=69))                                                                       
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 13149 bytes 1729190 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 13149 bytes 2132509 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 13117 bytes 787665 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 4534437 bytes 2069116589 accept
		ct state related,established counter packets 76680829 bytes 85350120897 accept
		ct state invalid counter packets 65636 bytes 3339640 jump ufw-logging-deny
		ct state invalid counter packets 65636 bytes 3339640 drop
		...
		ip protocol icmp icmp type echo-request counter packets 782 bytes 39879 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 696594 bytes 79077652 jump ufw-not-local
		...
		counter packets 696594 bytes 79077652 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 4534437 bytes 2069116589 accept
		ct state related,established counter packets 61780076 bytes 51276676700 accept
		counter packets 522452 bytes 178729413 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 2169789 bytes 1488993194 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 4 bytes 112 accept
		counter packets 110636 bytes 6641849 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 1098 bytes 85589 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 1460 bytes 334340 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 29 bytes 1324 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 203 bytes 9732 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 29076 bytes 13449500 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 102979 bytes 34050485 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 13149 bytes 633879 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 13149 bytes 3104167 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 13117 bytes 787649 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 345682 bytes 20894795 accept
		ip protocol udp ct state new counter packets 172395 bytes 157321078 accept
	}
	...
	chain INPUT {
		...
		counter packets 81978278 bytes 87501694657 jump ufw-before-logging-input
		counter packets 81978278 bytes 87501694657 jump ufw-before-input
		counter packets 323620 bytes 57218168 jump ufw-after-input
		counter packets 188775 bytes 9287198 jump ufw-after-logging-input
		counter packets 188775 bytes 9287198 jump ufw-reject-input
		counter packets 188775 bytes 9287198 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 66836965 bytes 53524522702 jump ufw-before-logging-output
		counter packets 66836965 bytes 53524522702 jump ufw-before-output
		counter packets 522452 bytes 178729413 jump ufw-after-output
		counter packets 522452 bytes 178729413 jump ufw-after-logging-output
		counter packets 522452 bytes 178729413 jump ufw-reject-output
		counter packets 522452 bytes 178729413 jump ufw-track-output
	}
	...
	chain FORWARD {
		...
		counter packets 2280429 bytes 1495635155 jump ufw-before-logging-forward
		counter packets 2280429 bytes 1495635155 jump ufw-before-forward
		counter packets 110636 bytes 6641849 jump ufw-after-forward
		counter packets 110636 bytes 6641849 jump ufw-after-logging-forward
		counter packets 110636 bytes 6641849 jump ufw-reject-forward
		counter packets 110636 bytes 6641849 jump ufw-track-forward
		iifname "wg-home" counter packets 106283 bytes 6380508 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 8496 bytes 464537 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 8496 bytes 464537 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 134845 bytes 47930970 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 559917 bytes 31093291 return
		fib daddr type multicast counter packets 2100 bytes 67200 return
		fib daddr type broadcast counter packets 134577 bytes 47917161 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 371180 bytes 21743767 accept
		tcp dport 80 counter packets 1473 bytes 79745 accept
		tcp dport 52000 counter packets 101 bytes 5980 accept
		udp dport 21195 counter packets 0 bytes 0 accept
		udp dport 21196 counter packets 0 bytes 0 accept
		tcp dport 21197 counter packets 22 bytes 1208 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 43 | 2026-07-21 04:19:58 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-07-21 04:15:48 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 104K
drwxr-xr-x  2 root root   4.0K Jul 21 03:20 .
