...
Generated: 2026-08-21T04:20:01+03:00  
Host: hometele  
...
Linux hometele 6.8.0-138-generic #138-Ubuntu SMP PREEMPT_DYNAMIC Fri Jul 31 22:41:49 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:01 up 59 min,  0 user,  load average: 0.00, 0.00, 0.00
```
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 189 bytes 23477 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 189 bytes 18649 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 47 bytes 2852 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 6484 bytes 2561499 accept
		ct state related,established counter packets 65194 bytes 56335721 accept
		ct state invalid counter packets 77 bytes 4658 jump ufw-logging-deny
		ct state invalid counter packets 77 bytes 4658 drop
		...
		ip protocol icmp icmp type echo-request counter packets 7 bytes 333 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 4722 bytes 630882 jump ufw-not-local
		...
		counter packets 4722 bytes 630882 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 6484 bytes 2561499 accept
		ct state related,established counter packets 64469 bytes 44619731 accept
		counter packets 1518 bytes 728603 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 44818 bytes 32910202 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 0 bytes 0 accept
		counter packets 150 bytes 9016 jump ufw-user-forward
	}
	...
	chain ufw-after-input {
		...
		fib daddr type broadcast counter packets 1052 bytes 397321 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 189 bytes 10043 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 189 bytes 30122 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 49 bytes 2956 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 311 bytes 18660 accept
		ip protocol udp ct state new counter packets 1195 bytes 708935 accept
	}
	...
	chain INPUT {
		...
		counter packets 76484 bytes 59533093 jump ufw-before-logging-input
		counter packets 76484 bytes 59533093 jump ufw-before-input
		counter packets 4256 bytes 600674 jump ufw-after-input
		counter packets 3016 bytes 147029 jump ufw-after-logging-input
		counter packets 3016 bytes 147029 jump ufw-reject-input
		counter packets 3016 bytes 147029 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 72471 bytes 47909833 jump ufw-before-logging-output
		counter packets 72471 bytes 47909833 jump ufw-before-output
		counter packets 1518 bytes 728603 jump ufw-after-output
		counter packets 1518 bytes 728603 jump ufw-after-logging-output
		counter packets 1518 bytes 728603 jump ufw-reject-output
		counter packets 1518 bytes 728603 jump ufw-track-output
	}
	...
	chain FORWARD {
		...
		counter packets 44968 bytes 32919218 jump ufw-before-logging-forward
		counter packets 44968 bytes 32919218 jump ufw-before-forward
		counter packets 150 bytes 9016 jump ufw-after-forward
		counter packets 150 bytes 9016 jump ufw-after-logging-forward
		counter packets 150 bytes 9016 jump ufw-reject-forward
		counter packets 150 bytes 9016 jump ufw-track-forward
		iifname "wg-home" counter packets 150 bytes 9016 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 65 bytes 3914 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 65 bytes 3914 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 1240 bytes 453645 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 3454 bytes 176341 return
		fib daddr type multicast counter packets 28 bytes 896 return
		fib daddr type broadcast counter packets 1240 bytes 453645 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 423 bytes 26468 accept
		tcp dport 80 counter packets 23 bytes 1336 accept
		tcp dport 52000 counter packets 2 bytes 120 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 45 | 2026-08-21 04:19:42 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-08-21 01:48:26 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 244K
drwxr-xr-x  2 root root   4.0K Aug 21 03:20 .
drwxrwxr-x 16 root syslog 4.0K Aug 21 03:20 ..
