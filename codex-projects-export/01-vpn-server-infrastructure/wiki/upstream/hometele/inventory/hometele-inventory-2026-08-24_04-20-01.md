...
Generated: 2026-08-24T04:20:01+03:00  
Host: hometele  
...
Linux hometele 6.8.0-138-generic #138-Ubuntu SMP PREEMPT_DYNAMIC Fri Jul 31 22:41:49 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:01 up 3 days, 59 min,  0 user,  load average: 0.00, 0.00, 0.00
```
...
udp   UNCONN 0      0           10.8.1.2:60378      0.0.0.0:*    users:(("xray",pid=971,fd=635))                                                                            
...
udp   UNCONN 0      0           10.8.1.2:60614      0.0.0.0:*    users:(("xray",pid=971,fd=743))                                                                            
udp   UNCONN 0      0           10.8.1.2:52581      0.0.0.0:*    users:(("xray",pid=971,fd=781))                                                                            
...
udp   UNCONN 0      0           10.8.1.2:53072      0.0.0.0:*    users:(("xray",pid=971,fd=450))                                                                            
udp   UNCONN 0      0           10.8.1.2:50076      0.0.0.0:*    users:(("xray",pid=971,fd=618))                                                                            
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 13149 bytes 1361988 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 13149 bytes 3411691 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 2983 bytes 182548 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 5865793 bytes 2693356557 accept
		ct state related,established counter packets 109619029 bytes 143445931479 accept
		ct state invalid counter packets 11048 bytes 597852 jump ufw-logging-deny
		ct state invalid counter packets 11048 bytes 597852 drop
		...
		ip protocol icmp icmp type echo-request counter packets 1084 bytes 60119 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 847667 bytes 99429905 jump ufw-not-local
		...
		counter packets 847667 bytes 99429905 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 5865793 bytes 2693356557 accept
		ct state related,established counter packets 85105146 bytes 81551442419 accept
		counter packets 860003 bytes 399336674 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 1429680 bytes 902796067 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 0 bytes 0 accept
		counter packets 12954 bytes 780920 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 28838 bytes 2249364 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 1430 bytes 327470 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 37 bytes 1644 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 226 bytes 10736 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 12308 bytes 3790864 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 79340 bytes 29569170 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 13149 bytes 594998 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 13149 bytes 5130751 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 2985 bytes 182652 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 515027 bytes 31093357 accept
		ip protocol udp ct state new counter packets 340794 bytes 367786892 accept
	}
	...
	chain INPUT {
		...
		counter packets 116344621 bytes 146239375912 jump ufw-before-logging-input
		counter packets 116344621 bytes 146239375912 jump ufw-before-input
		counter packets 369176 bytes 70689234 jump ufw-after-input
		counter packets 246997 bytes 34739986 jump ufw-after-logging-input
		counter packets 246997 bytes 34739986 jump ufw-reject-input
		counter packets 246997 bytes 34739986 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 91830942 bytes 84644135650 jump ufw-before-logging-output
		counter packets 91830942 bytes 84644135650 jump ufw-before-output
		counter packets 860003 bytes 399336674 jump ufw-after-output
		counter packets 860003 bytes 399336674 jump ufw-after-logging-output
		counter packets 860003 bytes 399336674 jump ufw-reject-output
		counter packets 860003 bytes 399336674 jump ufw-track-output
	}
	...
	chain FORWARD {
		...
		counter packets 1442634 bytes 903576987 jump ufw-before-logging-forward
		counter packets 1442634 bytes 903576987 jump ufw-before-forward
		counter packets 12954 bytes 780920 jump ufw-after-forward
		counter packets 12954 bytes 780920 jump ufw-after-logging-forward
		counter packets 12954 bytes 780920 jump ufw-reject-forward
		counter packets 12954 bytes 780920 jump ufw-track-forward
		iifname "wg-home" counter packets 12954 bytes 780920 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 6175 bytes 345563 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 6175 bytes 345563 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 122179 bytes 35949248 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 723668 bytes 63427255 return
		fib daddr type multicast counter packets 2102 bytes 67264 return
		fib daddr type broadcast counter packets 121897 bytes 35935386 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 473146 bytes 28423563 accept
		tcp dport 80 counter packets 5291 bytes 310997 accept
		tcp dport 52000 counter packets 19 bytes 984 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 47 | 2026-08-24 04:19:44 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-08-23 22:17:16 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 264K
drwxr-xr-x  2 root root   4.0K Aug 24 03:20 .
