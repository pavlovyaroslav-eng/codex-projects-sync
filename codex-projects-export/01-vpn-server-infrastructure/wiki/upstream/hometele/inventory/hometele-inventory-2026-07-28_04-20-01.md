...
Generated: 2026-07-28T04:20:01+03:00  
Host: hometele  
...
Linux hometele 6.8.0-136-generic #136-Ubuntu SMP PREEMPT_DYNAMIC Wed Jul  1 21:53:05 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:01 up 10 days, 59 min,  1 user,  load average: 0.00, 0.00, 0.00
```
...
udp   UNCONN 0      0           10.8.1.2:47819      0.0.0.0:*    users:(("xray",pid=172876,fd=68))                                                                              
...
udp   UNCONN 0      0           10.8.1.2:46042      0.0.0.0:*    users:(("xray",pid=172876,fd=61))                                                                              
...
udp   UNCONN 0      0           10.8.1.2:47234      0.0.0.0:*    users:(("xray",pid=172876,fd=282))                                                                             
udp   UNCONN 0      0           10.8.1.2:53415      0.0.0.0:*    users:(("xray",pid=172876,fd=46))                                                                              
udp   UNCONN 0      0           10.8.1.2:49329      0.0.0.0:*    users:(("xray",pid=172876,fd=22))                                                                              
udp   UNCONN 0      0           10.8.1.2:38259      0.0.0.0:*    users:(("xray",pid=172876,fd=25))                                                                              
...
udp   UNCONN 0      0           10.8.1.2:39409      0.0.0.0:*    users:(("xray",pid=172876,fd=75))                                                                              
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 43389 bytes 5242858 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 43389 bytes 7270281 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 43357 bytes 2603265 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 15234619 bytes 6963835463 accept
		ct state related,established counter packets 263192236 bytes 323113393857 accept
		ct state invalid counter packets 93135 bytes 4836459 jump ufw-logging-deny
		ct state invalid counter packets 93135 bytes 4836459 drop
		...
		ip protocol icmp icmp type echo-request counter packets 2952 bytes 149690 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 2262347 bytes 246301235 jump ufw-not-local
		...
		counter packets 2262347 bytes 246301235 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 15234619 bytes 6963835463 accept
		ct state related,established counter packets 205816082 bytes 194190750429 accept
		counter packets 2058579 bytes 691861074 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 6887222 bytes 4603699729 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 4 bytes 112 accept
		counter packets 354869 bytes 21307845 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 52344 bytes 4082742 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 6322 bytes 1447651 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 93 bytes 4204 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 784 bytes 38144 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 58071 bytes 22560350 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 344178 bytes 114117716 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 43389 bytes 2093707 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 43389 bytes 10780525 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 43357 bytes 2603249 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 1403922 bytes 84934165 accept
		ip protocol udp ct state new counter packets 640851 bytes 605364492 accept
	}
	...
	chain INPUT {
		...
		counter packets 280785289 bytes 330328516704 jump ufw-before-logging-input
		counter packets 280785289 bytes 330328516704 jump ufw-before-input
		counter packets 1019566 bytes 172872095 jump ufw-after-input
		counter packets 557774 bytes 30621288 jump ufw-after-logging-input
		counter packets 557774 bytes 30621288 jump ufw-reject-input
		counter packets 557774 bytes 30621288 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 223109280 bytes 201846446966 jump ufw-before-logging-output
		counter packets 223109280 bytes 201846446966 jump ufw-before-output
		counter packets 2058579 bytes 691861074 jump ufw-after-output
		counter packets 2058579 bytes 691861074 jump ufw-after-logging-output
		counter packets 2058579 bytes 691861074 jump ufw-reject-output
		counter packets 2058579 bytes 691861074 jump ufw-track-output
	}
	...
	chain FORWARD {
		...
		counter packets 7242095 bytes 4625007686 jump ufw-before-logging-forward
		counter packets 7242095 bytes 4625007686 jump ufw-before-forward
		counter packets 354869 bytes 21307845 jump ufw-after-forward
		counter packets 354869 bytes 21307845 jump ufw-after-logging-forward
		counter packets 354869 bytes 21307845 jump ufw-reject-forward
		counter packets 354869 bytes 21307845 jump ufw-track-forward
		iifname "wg-home" counter packets 350516 bytes 21046504 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 18723 bytes 1095290 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 18723 bytes 1095290 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 461792 bytes 142250807 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 1794572 bytes 103876754 return
		fib daddr type multicast counter packets 6938 bytes 222016 return
		fib daddr type broadcast counter packets 460837 bytes 142202465 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 1230398 bytes 72695542 accept
		tcp dport 80 counter packets 11969 bytes 692372 accept
		tcp dport 52000 counter packets 167 bytes 9722 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 44 | 2026-07-28 04:19:53 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-07-27 23:46:21 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 136K
drwxr-xr-x  2 root root   4.0K Jul 28 03:20 .
