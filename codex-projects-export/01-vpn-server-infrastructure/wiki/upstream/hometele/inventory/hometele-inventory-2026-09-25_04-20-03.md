...
Generated: 2026-09-25T04:20:03+03:00  
Host: hometele  
...
Linux hometele 6.8.0-142-generic #142-Ubuntu SMP PREEMPT_DYNAMIC Wed Sep  2 14:24:27 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:03 up 59 min,  0 user,  load average: 0.26, 0.13, 0.05
```
...
udp   UNCONN 0      0                  *:41265            *:*    users:(("rw-core",pid=2180,fd=15))                                                                     
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 189 bytes 34998 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 189 bytes 13323 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 188 bytes 11312 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 87788 bytes 118015751 accept
		ct state related,established counter packets 215353 bytes 188378784 accept
		ct state invalid counter packets 85 bytes 4056 jump ufw-logging-deny
		ct state invalid counter packets 85 bytes 4056 drop
		...
		ip protocol icmp icmp type echo-request counter packets 22 bytes 1330 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 4383 bytes 1039190 jump ufw-not-local
		...
		counter packets 4383 bytes 1039190 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 87788 bytes 118015751 accept
		ct state related,established counter packets 166096 bytes 186882242 accept
		counter packets 1682 bytes 127559 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 65958 bytes 52891952 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 0 bytes 0 accept
		counter packets 1349 bytes 80980 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 79 bytes 6162 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 15 bytes 3435 jump ufw-skip-to-policy-input
		...
		udp dport 67 counter packets 169 bytes 52052 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 0 bytes 0 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 1187 bytes 419407 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 189 bytes 12538 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 189 bytes 14112 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 188 bytes 11296 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 204 bytes 12240 accept
		ip protocol udp ct state new counter packets 1460 bytes 113999 accept
	}
	...
	chain INPUT {
		...
		counter packets 307631 bytes 307439111 jump ufw-before-logging-input
		counter packets 307631 bytes 307439111 jump ufw-before-input
		counter packets 3109 bytes 568548 jump ufw-after-input
		counter packets 1659 bytes 87492 jump ufw-after-logging-input
		counter packets 1659 bytes 87492 jump ufw-reject-input
		counter packets 1659 bytes 87492 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 255566 bytes 305025552 jump ufw-before-logging-output
		counter packets 255566 bytes 305025552 jump ufw-before-output
		counter packets 1682 bytes 127559 jump ufw-after-output
		counter packets 1682 bytes 127559 jump ufw-after-logging-output
		counter packets 1682 bytes 127559 jump ufw-reject-output
		counter packets 1682 bytes 127559 jump ufw-track-output
	}

	chain FORWARD {
		type filter hook forward priority filter; policy drop;
		counter packets 72993 bytes 54985944 jump DOCKER-USER
		counter packets 72993 bytes 54985944 jump DOCKER-FORWARD
		counter packets 67307 bytes 52972932 jump ufw-before-logging-forward
		counter packets 67307 bytes 52972932 jump ufw-before-forward
		counter packets 1349 bytes 80980 jump ufw-after-forward
		counter packets 1349 bytes 80980 jump ufw-after-logging-forward
		counter packets 1349 bytes 80980 jump ufw-reject-forward
		counter packets 1349 bytes 80980 jump ufw-track-forward
		iifname "wg-home" counter packets 1349 bytes 80980 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 33 bytes 1556 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 33 bytes 1556 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 1450 bytes 481056 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 2906 bytes 557316 return
		fib daddr type multicast counter packets 28 bytes 896 return
		fib daddr type broadcast counter packets 1449 bytes 480978 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 321 bytes 19176 accept
		tcp dport 80 counter packets 291 bytes 11888 accept
		...
		ip saddr 93.183.106.203 tcp dport 2222 counter packets 14 bytes 840 accept
		udp dport 443 counter packets 638 bytes 437446 accept
	}
	...
	chain DOCKER-FORWARD {
		counter packets 72993 bytes 54985944 jump DOCKER-CT
		counter packets 70523 bytes 53567386 jump DOCKER-INTERNAL
		counter packets 70523 bytes 53567386 jump DOCKER-BRIDGE
		iifname "br-be53f5c116d2" counter packets 3216 bytes 594454 accept
...
headscale   headscale/headscale:0.29.3   Up 59 minutes (healthy)   127.0.0.1:8081->8080/tcp, 127.0.0.1:9091->9090/tcp
remnanode   remnawave/node:3.4.1         Up 59 minutes             
```
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 47 | 2026-09-25 04:19:48 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-09-24 21:03:55 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 420K
drwxr-xr-x  2 root root   4.0K Sep 25 03:20 .
