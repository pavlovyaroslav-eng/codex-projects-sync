...
Generated: 2026-08-30T04:20:01+03:00  
Host: hometele  
...
Linux hometele 6.8.0-138-generic #138-Ubuntu SMP PREEMPT_DYNAMIC Fri Jul 31 22:41:49 UTC 2026 x86_64 x86_64 x86_64 GNU/Linux
 ...
 04:20:01 up 9 days, 59 min,  0 user,  load average: 0.00, 0.00, 0.00
```
...
udp   UNCONN 0      0           10.8.1.2:51171      0.0.0.0:*    users:(("xray",pid=971,fd=4371))                                                                               
...
udp   UNCONN 0      0           10.8.1.2:34099      0.0.0.0:*    users:(("xray",pid=971,fd=3231))                                                                               
udp   UNCONN 0      0           10.8.1.2:33155      0.0.0.0:*    users:(("xray",pid=971,fd=3171))                                                                               
udp   UNCONN 0      0           10.8.1.2:54664      0.0.0.0:*    users:(("xray",pid=971,fd=3646))                                                                               
udp   UNCONN 0      0           10.8.1.2:53812      0.0.0.0:*    users:(("xray",pid=971,fd=3067))                                                                               
udp   UNCONN 0      0          127.0.0.1:10808      0.0.0.0:*    users:(("xray",pid=971,fd=7))                                                                                  
udp   UNCONN 0      0            0.0.0.0:51820      0.0.0.0:*                                                                                                                   
udp   UNCONN 0      0           10.8.1.2:41653      0.0.0.0:*    users:(("xray",pid=971,fd=4273))                                                                               
...
udp   UNCONN 0      0           10.8.1.2:57260      0.0.0.0:*    users:(("xray",pid=971,fd=2948))                                                                               
...
table ip filter {
	chain ufw-before-logging-input {
		ct state new limit rate 3/minute burst 10 packets counter packets 39069 bytes 3938048 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-output {
		ct state new limit rate 3/minute burst 10 packets counter packets 39069 bytes 7941665 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-logging-forward {
		ct state new limit rate 3/minute burst 10 packets counter packets 9181 bytes 560956 log prefix "[UFW AUDIT] "
	}

	chain ufw-before-input {
		iifname "lo" counter packets 16833854 bytes 7654346790 accept
		ct state related,established counter packets 305553015 bytes 382255625834 accept
		ct state invalid counter packets 31301 bytes 1788651 jump ufw-logging-deny
		ct state invalid counter packets 31301 bytes 1788651 drop
		...
		ip protocol icmp icmp type echo-request counter packets 2820 bytes 153655 accept
		udp sport 67 udp dport 68 counter packets 0 bytes 0 accept
		counter packets 2472315 bytes 247854941 jump ufw-not-local
		...
		counter packets 2472315 bytes 247854941 jump ufw-user-input
	}

	chain ufw-before-output {
		oifname "lo" counter packets 16833854 bytes 7654346790 accept
		ct state related,established counter packets 235372837 bytes 235571081811 accept
		counter packets 2203614 bytes 897228644 jump ufw-user-output
	}

	chain ufw-before-forward {
		ct state related,established counter packets 5099398 bytes 3406363780 accept
		ip protocol icmp icmp type destination-unreachable counter packets 0 bytes 0 accept
		ip protocol icmp icmp type time-exceeded counter packets 0 bytes 0 accept
		ip protocol icmp icmp type parameter-problem counter packets 0 bytes 0 accept
		ip protocol icmp icmp type echo-request counter packets 0 bytes 0 accept
		counter packets 45484 bytes 2739600 jump ufw-user-forward
	}

	chain ufw-after-input {
		udp dport 137 counter packets 102381 bytes 7985711 jump ufw-skip-to-policy-input
		udp dport 138 counter packets 4196 bytes 960884 jump ufw-skip-to-policy-input
		tcp dport 139 counter packets 129 bytes 5968 jump ufw-skip-to-policy-input
		tcp dport 445 counter packets 664 bytes 32040 jump ufw-skip-to-policy-input
		udp dport 67 counter packets 36964 bytes 11483450 jump ufw-skip-to-policy-input
		udp dport 68 counter packets 1 bytes 28 jump ufw-skip-to-policy-input
		fib daddr type broadcast counter packets 227678 bytes 86715374 jump ufw-skip-to-policy-input
	}
	...
	chain ufw-after-logging-input {
		limit rate 3/minute burst 10 packets counter packets 39069 bytes 1768610 log prefix "[UFW BLOCK] "
	}

	chain ufw-after-logging-output {
		limit rate 3/minute burst 10 packets counter packets 39069 bytes 11364614 log prefix "[UFW ALLOW] "
	}

	chain ufw-after-logging-forward {
		limit rate 3/minute burst 10 packets counter packets 9183 bytes 561060 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-track-output {
		ip protocol tcp ct state new counter packets 1383292 bytes 84071073 accept
		ip protocol udp ct state new counter packets 806203 bytes 811523849 accept
	}
	...
	chain INPUT {
		...
		counter packets 324893305 bytes 390159769871 jump ufw-before-logging-input
		counter packets 324893305 bytes 390159769871 jump ufw-before-input
		counter packets 964635 bytes 158199145 jump ufw-after-input
		counter packets 592622 bytes 51015690 jump ufw-after-logging-input
		counter packets 592622 bytes 51015690 jump ufw-reject-input
		counter packets 592622 bytes 51015690 jump ufw-track-input
	}

	chain OUTPUT {
		type filter hook output priority filter; policy accept;
		counter packets 254410305 bytes 244122657245 jump ufw-before-logging-output
		counter packets 254410305 bytes 244122657245 jump ufw-before-output
		counter packets 2203614 bytes 897228644 jump ufw-after-output
		counter packets 2203614 bytes 897228644 jump ufw-after-logging-output
		counter packets 2203614 bytes 897228644 jump ufw-reject-output
		counter packets 2203614 bytes 897228644 jump ufw-track-output
	}
	...
	chain FORWARD {
		...
		counter packets 5144882 bytes 3409103380 jump ufw-before-logging-forward
		counter packets 5144882 bytes 3409103380 jump ufw-before-forward
		counter packets 45484 bytes 2739600 jump ufw-after-forward
		counter packets 45484 bytes 2739600 jump ufw-after-logging-forward
		counter packets 45484 bytes 2739600 jump ufw-reject-forward
		counter packets 45484 bytes 2739600 jump ufw-track-forward
		iifname "wg-home" counter packets 45484 bytes 2739600 accept
	...
	chain ufw-logging-deny {
		ct state invalid limit rate 3/minute burst 10 packets counter packets 15496 bytes 917585 log prefix "[UFW AUDIT INVALID] "
		limit rate 3/minute burst 10 packets counter packets 15496 bytes 917585 log prefix "[UFW BLOCK] "
	}
	...
	chain ufw-skip-to-policy-input {
		counter packets 372013 bytes 107183455 drop
	}
	...
	chain ufw-not-local {
		fib daddr type local counter packets 2094919 bytes 140515117 return
		fib daddr type multicast counter packets 6248 bytes 199936 return
		fib daddr type broadcast counter packets 371148 bytes 107139888 return
		limit rate 3/minute burst 10 packets counter packets 0 bytes 0 jump ufw-logging-deny
		counter packets 0 bytes 0 drop
	}

	chain ufw-user-input {
		tcp dport 443 counter packets 1377176 bytes 82111313 accept
		tcp dport 80 counter packets 130334 bytes 7531356 accept
		tcp dport 52000 counter packets 100 bytes 5428 accept
		udp dport 21195 counter packets 1 bytes 439 accept
		udp dport 21196 counter packets 1 bytes 439 accept
		tcp dport 21197 counter packets 42 bytes 2340 accept
		udp dport 21198 counter packets 1 bytes 439 accept
		udp dport 21199 counter packets 1 bytes 439 accept
		udp dport 51820 counter packets 24 bytes 3603 accept
...
| `/var/lib/hometele-monitor/command-agent.since` | regular file | root:root | 644 | 46 | 2026-08-30 04:19:50 |
...
| `/etc/letsencrypt` | directory | root:root | 755 | 4096 | 2026-08-30 01:04:32 |
| `/root/cert` | missing | - | - | - | - |
| `/etc/x-ui` | missing | - | - | - | - |
| `/etc/3x-ui` | missing | - | - | - | - |
...
total 296K
drwxr-xr-x  2 root root   4.0K Aug 30 03:30 .
drwxrwxr-x 16 root syslog 4.0K Aug 30 00:00 ..
